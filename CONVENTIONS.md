...
- **`!important` vain `@media (prefers-reduced-motion)` -lohkoissa.** Layout-/väri-`!important` = merkki cascade-ongelmasta → korjaa **`layer(components)`-järjestyksellä** tai spesifisyydellä, älä lisää uutta `!important`ia. **Varo:** `!important` *kääntää* layer-järjestyksen (matalan layerin `!important` voittaa korkeamman layerin normaalisäännön), joten `!important`ien poisto layeroinnin yhteydessä vaatii tapauskohtaisen analyysin — ei blanket-poistoa.
  - **Per-flip-purkutapa (L-V400):** poista yksi cascade-konflikti kerrallaan, todista 0-pikselidiffi `tests/e2e-visual-layer.spec.js`-harnessilla (29 pintaa × mobile/desktop) ennen committia. Yleisin turvallinen tapaus on **redundantti** `!important` jonka voittajasääntö voittaa jo specificityllä/source-orderilla (esim. `.foo[hidden]` 0,2,0 > `.foo` 0,1,0, tai `body:has(...) .x` 1,2,1 > `.x` 0,1,0, tai suora arvo > periytyvä) — silloin pelkkä `!important`-poisto on behavior-preserving. Harness-katteen ulkopuoliset pinnat (digikirja-lesson, profile-menu/teaching-panel auki-tila) vaativat oman computed-style-probe/clickthrough-todistuksen — älä poista pelkän specificity-matikan varassa jos pinta ei ole harnessissa (L-V390 oli juuri tällainen off-harness-`[hidden]`-regressio).
