// Waitlist modal, FR landing. Extracted from inline <script> in ranska.html
...
(function () {
  ...
  var countEl = document.getElementById('wl-count-fr');
  if (countEl) {
    fetch('/api/onboarding/waitlist/count?language=fr')
      .then(function (r) { return r.ok ? r.json() : Promise.reject(); })
      .then(function (d) {
        if (d && typeof d.count === 'number' && d.count > 0) {
          countEl.textContent = d.count + ' odottaa ranskan kurssia •';
  ...
  form.addEventListener('submit', function (e) {
    ...
    fetch('/api/onboarding/waitlist', {
      ...
      body: JSON.stringify({ email: email, language: 'fr', level: 'lyhyt' })
    })
      .then(function (r) {
      ...
      .then(function () {
        statusEl.textContent = 'Olet waitlistillä! Saat sähköpostin kun ranskan Puheo avautuu.';
