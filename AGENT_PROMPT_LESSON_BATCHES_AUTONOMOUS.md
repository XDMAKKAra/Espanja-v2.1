# L-LESSON-BATCHES — autonominen orchestrator/worker lesson-generointi (Batch 2 → Batch 7)

> Liimaa tämä Claude Coden tuoreeseen istuntoon repon juuressa. Agentti pyörii **autonomisesti**: generoi → review-subagentti → korjaa → seuraava batch. Älä pysähdy kysymään käyttäjältä mitään ennen kuin viimeinen batch on valmis.
>
> Käyttäjä on poissa. Hän palaa kun viimeinen batch on shipattu ja `npm run validate:lessons` on exit 0 KAIKKIIN tiedostoihin. Tämä prompti on suunniteltu niin että käyttäjä voi `/clear` tarvittaessa kesken — `AGENT_STATE.md` säilyttää tilan.
...
Task({ model: "sonnet", description: "Generate K5 lessons 1-11", prompt: "..." })
Task({ model: "sonnet", description: "Generate K6 lessons 1-6", prompt: "..." })
```
...
3. Lue `data/courses/`-kansion sisältö — onko batchit tehty vai ei
...
Generoi 85 oppituntia 6 batchissa, jokaisen batchin jälkeen pakollinen review-subagent-tarkistus, korjaa kaikki review-löydökset ennen seuraavaan batchiin siirtymistä.
...
Kurssi 1 oppitunnit 1–5 ovat jo olemassa (Batch 1 hotfixin jäljiltä). Älä koske niihin paitsi jos review löytää epäjohdonmukaisuuksia ja silloinkin vain konsultoinnin jälkeen.

K2–K8 lesson_1.json -tiedostot ovat **placeholdereita** L-COURSE-1:stä. Yliaja ne osana batchia 2/3/4/5/6/7 omiensa kurssien kanssa.
...
```
PER BATCH N:
  1. PLAN — lue lib/curriculumData.js JOS et ole vielä lukenut. Suunnittele cross-pollination batchin sisällä (mitä sanoja toistuu, mitä rakenteita rakentaa edelliselle).
  2. GENERATE — spawnaa YKSI worker-subagent Sonnetilla joka generoi koko batchin (ei yksi-per-lesson):
     Task({
       subagent_type: "general-purpose",
       model: "sonnet",         ← PAKOLLINEN
       description: "Generate batch N (lessons)",
       prompt: "[koko worker-prompti, listaa batchin lessonit, kerro mitä skill-tiedostoja lukea, anna L-tason taulukot, vaadi npm run validate:lessons lopuksi]"
     })
     ...
     - YO-relevance-kenttä jokaisessa
     - Suomi-copy puheo-finnish-voice-skillin mukaan
     - distractor-laatu error-analysis-protocol-skillin mukaan
  3. VALIDATE — npm run validate:lessons. Jos exit != 0, korjaa schema-virheet itse, älä raportoi käyttäjälle.
  ...
  6. RE-VALIDATE — npm run validate:lessons uudelleen → exit 0
  7. CHECKPOINT — päivitä AGENT_STATE.md:
     - Last completed batch: N
     - Next batch: N+1
     - Files generated this batch: [polut]
     - Review findings count: P0=X, P1=Y, P2=Z, kaikki P0/P1 korjattu
     - Memorized files (älä lue uudestaan seuraavissa batcheissa): schemas/lesson.json, lib/curriculumData.js, PROMPT_GENERATE_LESSON.md, .claude/skills/puheo-ai-prompt/SKILL.md, .claude/skills/puheo-finnish-voice/SKILL.md, education-skillit (luettelo)
  8. RECORD — IMPROVEMENTS.md +1 rivi formaatti: `- [YYYY-MM-DD L-LESSON-BATCH-N] Generoitu N oppituntia (kurssit X). Review: P0=A, P1=B korjattu. Validate: exit 0.`
  9. NEXT — siirry välittömästi batchiin N+1. ÄLÄ PYSÄHDY KYSYMÄÄN KÄYTTÄJÄLTÄ MITÄÄN.
```

**Pysähdy vain jos:**
- npm run validate:lessons palauttaa exit != 0 etkä saa korjattua kahdessa yrityksessä
...
5. `data/courses/kurssi_1/lesson_1.json` ja `lesson_2.json` — referenssi mitä laatua olemassa olevat lessonit ovat (Batch 1:n hotfixin jälkeen)
...
7. `.claude/skills/puheo-finnish-voice/SKILL.md`
8. `.claude/skills/education/practice-problem-sequence-designer/SKILL.md`
9. `.claude/skills/education/retrieval-practice-generator/SKILL.md`
10. `.claude/skills/education/cognitive-load-analyser/SKILL.md`
11. `.claude/skills/education/criterion-referenced-rubric-generator/SKILL.md`
12. `.claude/skills/education/error-analysis-protocol/SKILL.md`
13. `.claude/skills/education/explicit-instruction-sequence-builder/SKILL.md`
14. `.claude/skills/education/spaced-practice-scheduler/SKILL.md`
15. `.claude/skills/education/adaptive-hint-sequence-designer/SKILL.md`
16. `.claude/skills/education/variation-theory-task-designer/SKILL.md`
17. `.claude/skills/education/formative-assessment-loop-designer/SKILL.md`
18. `.claude/skills/education/self-efficacy-builder-sequence/SKILL.md`

**Skillit jotka REVIEW-agentin PITÄÄ lukea (kaikki yllä olevat 6–18 PLUS näitä):**

19. `.claude/skills/ui-ux-pro-max/SKILL.md` — frontend-laadun arvio
20. `.claude/skills/frontend-design/SKILL.md` — visuaalinen monipuolisuus, UX-flow
21. `.claude/skills/design-taste-frontend/SKILL.md` (jos olemassa) — typografia, visuaalinen rytmi
...
- **YO-koe-fokus pakollinen.** meta.yo_relevance-kenttä jokaisessa.
- **Sanat aiheessa.** Vocab-oppitunnin sanat matchaa title-aiheeseen, ei satunnaisia.
- **Suomi-copy** puheo-finnish-voice-skillistä. Ei englanninkielisiä termejä suomenkielisessä tekstissä. Ei shame, ei superlativisointia.
...
- recall_typed_es_to_fi: 10-12
- recall_typed_fi_to_es: 10-12
...
Lopputarkistus:
1. `npm run validate:lessons` exit 0
2. `find data/courses -name "lesson_*.json" | wc -l` → pitäisi palauttaa 90 (5 K1 + 10 K2 + 11 K3 + 12 K4 + 11 K5 + 12 K6 + 12 K7 + 12 K8 — wait, total = 5+10+11+12+11+12+12+12 = 85 + Batch 1 olemassa olevat 5 K1 = 90)

Oikeasti: K1 = 10 tunnetiedostoa (5 olemassa + 5 generoitu = 10), K2 = 10 (10 generoitu, korvaa lesson_1 placeholderin), K3-K8 = ks. taulukko. Yhteensä **90 lesson-tiedostoa**.

3. AGENT_STATE.md "Last completed loop: L-LESSON-BATCH-7. All 90 lessons generated and validated."
4. IMPROVEMENTS.md: yhteenveto-rivi 7 batchin jälkeen
5. Käyttäjälle reply: max 200 sanaa, formaatti:
   - "Generoitu X oppituntia 6 batchissa, kaikki schema-validia."
   ...
   - "Seuraava askel käyttäjälle: USE_PREGENERATED_LESSONS=true Vercel-dashiin, testaa Pro-tilillä K1L1, K2L1, K5L1, K8L1 (eri tasoja)."
...
- ÄLÄ deploy:aa
- ÄLÄ koske olemassa olevia K1 lesson_1.json - lesson_5.json -tiedostoja paitsi jos review löytää epäjohdonmukaisuuden — silloinkin pyydä lupaa kysymällä käyttäjältä siinä tilanteessa
...
- Lue 2-3 referenssiä netistä mitä YO-koe testaa (etsi "yo-koe lyhyt espanja kulttuuri" tms.) — älä kopioi, vain inspiraatioksi
- Lue saman kurssin grammar_focus tarkemmin → johtanut sanasto sieltä
- Reading-oppituntien teksteissä voi käyttää uutisteemoja (ympäristö, teknologia, kulttuuri-tapahtumat)
...
Lue ensin `AGENT_STATE.md`. Jos se sanoo että tämä on tuore aloitus → mene kohtaan "Pakolliset luettavat tiedostot" yllä, lue koko luettelo 1–18 (kaikki worker-skillit pakolliset), aloita Batch 2.
...
| 3 | recall_typed_es→fi | keskitaso | A, B, C, M, E, L |
| 4 | recall_typed_fi→es | keskitaso, distractor lähempi | B, C, M, E, L |
| 5 | application_gap_fill helppo | scaffold (infinitiivi annettu, vain konjugointi) | B, C, M, E, L |
...
- Vaiheet 1–3: distractor on **selvästi väärä** semanttinen kategoria (esim. *libro* kun oikea on *trabajo*)
- Vaiheet 4–6: distractor on **läheinen** semanttinen kategoria (esim. *empleo* vs *trabajo*)
- Vaiheet 7–10: distractor on **lähes oikea** — oikea verbi mutta väärä aikamuoto, oikea sana mutta väärä taivutus (esim. *hablaré* vs *hablaría* vs *hablará*)
...
- Vaiheet 7–10: oppikirjan ulkopuolista mutta YO-kokeessa esiintyvää (synonyymit, idiomit, kulttuurispesifit ilmaisut)
...
1. **Lesson runner** lukee `phases[].items[]` ja `phases[].kind` — varmista että jokainen kind on tuettu typessä `js/lib/lessonAdapter.js`
2. **side-panel** -sisältö (`teaching_panel` -kenttä jos schema:ssa) ei ole tyhjä eikä > 600 sanaa → cognitive overload
3. **typed-input** -tehtävien `expected_answer` ei sisällä monitulkintaisia muotoja jotka frontti hylkäisi (esim. piste lopussa, isoa kirjainta)
4. **Mobile-näkymä** (lessonRunner.css media-queryt): pitkät espanjan lauseet eivät katkea kesken sanan, painikkeet ≥44px korkeat, ei vaakasrollia
5. **Vaiheiden visuaalinen rytmi:** ei viittä peräkkäistä saman tyypin tehtävää (käyttäjä turtuu) — vaihtelu on pakollinen
6. **Kirjoitustehtävien palaute** (synthesis_short_writing, writing): rubriikki-kentät ovat olemassa että AI-grading toimii
...
```
Olet review-agent Puheo-projektissa (YO-koe Spanish-harjoittelu suomenkielisille lukio­laisille).

Repon juuri: C:\Users\marce\OneDrive\Documents\espanja paska

**Tarkistettavat tiedostot:**
[ORCHESTRATOR LIITTÄÄ POLUT TÄHÄN]
...
- AGENT_PROMPT_LESSON_BATCHES_AUTONOMOUS.md (tämä — etsi "Adaptiivinen vaikeus" ja "Frontend-renderöinti" -lohkot)
- js/screens/lessonRunner.js
- js/lib/lessonAdapter.js
- css/components/lesson-runner.css

**Vaihe 0 ennen tarkistusta (PAKOLLINEN):**
- Aja `npm run validate:lessons` ja varmista exit 0. Jos != 0 → raportoi P0-listana ja pysähdy ennen muita tarkistuksia.
- Avaa PROMPT_GENERATE_LESSON.md L-tason taulukot näkyviin viittausta varten.

**Tarkistus-checklist (per tiedosto, MAX 5 löydöstä per tiedosto):**

A. Kielelliset virheet (P0)
- Espanjan oikeinkirjoitus: acentos (á/é/í/ó/ú), ñ, b/v, c/qu/k, h-äänteettomyys
- Sukupäätteet (el/la), monikot, verbien konjugaatiot per pronomini
- Suomen kielioppi: sijapäätteet, omistuspäätteet, johdokset
- Käännösten oikeellisuus FI↔ES kumpaankin suuntaan
- Distractorit ovat oikeita espanjan sanoja, eivät keksittyjä

B. Tehtävämuotojen monipuolisuus (P1)
- Per oppitunti useita tyyppejä: recognition_mc, recognition_match, recall_typed_es_to_fi, recall_typed_fi_to_es, application_gap_fill, application_sentence_build, synthesis_translate, synthesis_short_writing
- Ei viittä peräkkäistä samaa typea
- Käännöstä molempiin suuntiin (es→fi JA fi→es)
...
- Lauseet aiheessa, ei "Pedro come pan" -tasoa
- YO-tyyppinen konteksti
- Ei sanaston identtistä toistoa kurssin sisällä

D. Adaptiivinen vaikeus (P0 jos rikki — KRIITTINEN)
Vertaa AGENT_PROMPT_LESSON_BATCHES_AUTONOMOUS.md "Adaptiivinen vaikeus" -taulukkoon:
- Vaiheittainen vaikeus nousee oikein (1→10)
- L:n vaiheet 8–10 aidosti vaikeampia kuin M:n viimeisin
- B/C scaffold (infinitiivi annettu); L:n synthesis ei apua
- Distractor-taso vaiheittain: helpoissa selvästi väärä, vaikeissa lähes oikea
- Lauseiden pituus skaalautuu (4–6 → 10–18 sanaa)
- mastery_threshold I 0.55 → L 0.95

E. Frontend-renderöinti (P0 jos rikki, P1 muuten)
Vertaa AGENT_PROMPT_LESSON_BATCHES_AUTONOMOUS.md "Frontend-renderöinti" -lohkoon:
- Kaikki phases[].kind tuettu lessonAdapter.js:ssä
- side-panel ei tyhjä eikä > 600 sanaa
- typed-input expected_answer rendaa oikein
- Pitkät lauseet eivät riko mobile-näkymää
- Ei viittä peräkkäistä saman tyypin tehtävää
- writing/synthesis: rubriikki-kentät olemassa AI-gradinguun

F. YO-koe-kytkös (P1)
- meta.yo_relevance oppitunti-spesifinen, ei copy-paste
- Kuvaa miten tämä auttaa nimenomaan korkeampia arvosanoja
- target_grades-lista oikein

**Output-formaatti:**

Per tiedosto MAX 5 riviä:
### kurssi_X/lesson_N.json
- P0 rivi 234: "se gusta" → "le gusta" (kielioppivirhe)
...
```
Olet lesson-worker Puheo-projektissa (YO-koe Spanish, suomenkielinen UI).

Repon juuri: C:\Users\marce\OneDrive\Documents\espanja paska

**Tehtävä:** generoi [KURSSI X oppitunnit Y–Z] kohteeseen data/courses/kurssi_X/lesson_N.json.

**Lue ensin (PAKOLLISET):**
1. PROMPT_GENERATE_LESSON.md
2. schemas/lesson.json
3. lib/curriculumData.js (etsi "kurssi_X" -rivit)
4. AGENT_PROMPT_LESSON_BATCHES_AUTONOMOUS.md — etsi "Adaptiivinen vaikeus" ja "Frontend-renderöinti" -lohkot
5. data/courses/kurssi_4/lesson_1.json + lesson_5.json (referenssi)
6. .claude/skills/puheo-finnish-voice/SKILL.md
7. .claude/skills/puheo-ai-prompt/SKILL.md
8. .claude/skills/education/practice-problem-sequence-designer/SKILL.md
9. .claude/skills/education/retrieval-practice-generator/SKILL.md
10. .claude/skills/education/cognitive-load-analyser/SKILL.md
11. .claude/skills/education/criterion-referenced-rubric-generator/SKILL.md
...
- skip_for_targets vaiheittain (ks. AGENT_PROMPT_LESSON_BATCHES_AUTONOMOUS.md "skip_for_targets — esimerkki vocab-oppitunnista")
- meta.level yksi kirjain (I/A/B/C/M/E/L), EI CEFR
- meta.yo_relevance oppitunti-spesifinen
- **Adaptiivinen vaikeus** (KRIITTINEN): L:n vaiheet 8–10 aidosti vaikeampia kuin M:n viimeisin — pidempiä lauseita (10–18 sanaa), lähes-oikeita distractoreita (oikea verbi väärä aikamuoto), ei scaffoldia
- mastery_threshold per target_grade: I 0.55, A 0.60, B 0.65, C 0.70, M 0.80, E 0.85, L 0.95
- Cross-pollination: käytä saman kurssin aiempien oppituntien sanoja distractoreina
- Sanat aiheessa, ei satunnaisia
- Suomi-copy luonnollista (ei käännöskoneellista)
- **Tehtävämuotojen monipuolisuus pakollinen:** käännös molempiin suuntiin (es→fi JA fi→es), gap_fill, sentence_build, synthesis. Ei viittä peräkkäistä saman tyypin tehtävää.

**Validoi lopuksi:** npm run validate:lessons → exit 0. Korjaa schema-virheet itse.
...
### 1. Tarkista USE_PREGENERATED_LESSONS-flag

Lue `routes/curriculum.js` ja varmista että `USE_PREGENERATED_LESSONS=true` -reitti lukee oikeasti `data/courses/`-tiedostoja. Jos näet että fallback OpenAI-reittiin on yhä mahdollinen vaikka JSON-tiedosto on olemassa → kirjaa `BUGS.md`:hen P0-merkinnällä:

```markdown
## P0 — fallback OpenAI runtime path triggered despite JSON existing
- File: routes/curriculum.js, function readPregeneratedLesson() / loadLesson()
- Symptom: K1L7 in production showed "AI-pyyntö aikakatkaistiin (30s)" 
  even though data/courses/kurssi_1/lesson_7.json was generated
- Root cause hypothesis: env flag not read OR JSON path mismatch OR cache
...
1. Vercel Dashboard → Settings → Environment Variables → 
   USE_PREGENERATED_LESSONS = true (Production)
2. Trigger redeploy: git commit -m "feat: lessons batch 2-7 generated" --allow-empty
3. Testaa appissa: K1L1 (A), K3L1 (B), K5L1 (C), K7L1 (M), K8L1 (E)
4. Jos joku näyttää "AI-pyyntö aikakatkaistiin" -virheen → flag ei vaikuta, 
...
```markdown
## Tunnetut UI-bugit (havaittu 2026-05-04 käyttäjän testiltä, korjataan parannus-orchestrator-vaiheessa)

- **P0** Lesson runner side-panel: "Opetussivu" aukeaa vasempaan reunaan, 
  pitäisi olla keskellä tai oikealla. Sijainti rikkoutui L-MERGE-DASH-PATH 
  (#20) jälkeen.
- **P1** Lesson runner skeleton-loader: pieni vasemmalla, vaikka renderöityvä 
  sisältö on full-width. Skeleton-elementti käyttää vanhaa positiointia.
- **P0** K1L7 (mahdollisesti muut) lataa OpenAI runtime-reittiä vaikka JSON 
  on generoitu. Ks. yllä kohdat 1.

Näiden korjaaminen on Ruflo-parannus-orchestratorin tehtävä — sisältö-vaihe 
keskittyy vain JSON-generointiin.
```
...
```
Generoitu X oppituntia 6 batchissa, kaikki schema-validia. 
Yhteensä X tiedostoa data/courses/ -kansiossa.
...
Käyttäjän tehtävät: kirjattu ylle (USE_PREGENERATED_LESSONS=true + redeploy + testaa).

Seuraava askel: Ruflo-orchestrator alkuun kun käyttäjä on testannut 
ja deployannut sisällön.
```
