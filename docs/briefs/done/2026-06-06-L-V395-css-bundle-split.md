...
## TULOS (2026-06-06) — Approach C: EI shipata

Mitattu, ei muutettu. Kaksi löydöstä:

**1. esbuild ei tue CSS-code-splittiä (Approach A infeasible).**
POC (`splitting:true`, dynaaminen `import()` joka tuo CSS:n): esbuild emittoi chunk-CSS:n (`chunks/lazy-*.css`) MUTTA hoistaa saman CSS:n SILTI entry-bundleen — `entry.css` sisälsi sekä `.shared`:n että `.lazy`:n. Mikään runtime ei myöskään auto-lataa chunk-CSS:ää. → kriittinen `app.bundle.css` ei pienenisi Approach A:lla. Vain manuaalinen Approach B (erillinen `app-screens.css` + lazy `<link>`-injektio) toimisi, ja se kantaa tämän briefin #1-riskin (cascade rikkoutuu kun lazy-CSS latautuu ajonaikaisesti `app-old-spain.css`:n JÄLKEEN).

**2. Hyötykatto mitattu — kohinatasolla.**
Trimmattu bundle (digikirja.css + lesson-runner.css pois) vs full:

| | raw | gzip |
|---|---|---|
| full | 429 650 | 69 881 |
| trimmed | 373 827 | 61 131 |
| delta | -55 823 (-13%) | -8 750 (-12.5%) |

FCP (Playwright route-swap, app.html, n=15, localhost → transfer ~0 → parse-aika eristettynä):
- FULL: mediaani 208ms, mean 213.9, range 188–264
- TRIMMED: mediaani 200ms, mean 197.9, range 180–220
- **Delta: 8ms mediaani (3.8%), jakaumat menevät vahvasti päällekkäin.**

**Päätös:** 8ms parse-aikasäästö (kohinarajalla) + 8.75KB gzip ei oikeuta Approach B:n cascade-riskiä eikä build/runtime-koneistoa. CSS jätetään ennalleen. Mittausartefaktit (trimmed-bundle, FCP-skripti) poistettu — luvut tässä + IMPROVEMENTS.md ovat pysyvä jälki. Jos joskus halutaan uudelleen: vain Approach B on teknisesti mahdollinen, ja se vaatii screenshot-diffin joka siirretystä screenistä cascade-vuotojen varalta.
