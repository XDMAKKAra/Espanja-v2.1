...
- **✅ PÄÄTÖS + TOTEUTUS (2026-06-06): poista end-to-end.** Poistettu: `js/screens/learningPath.js` (koko tiedosto), `/api/mastery-test/start` + `/submit` + `/mixed-review` (routes/exercises.js), `screen-mastery-intro/result` (app.html), main.js-wiring + vocab.js `endBatch`-mastery-haara, `tests/training/adaptive-routing.test.js`, trimmattu lib/learningPath.js-import. SÄILYTETTY: `/api/learning-path` (live, profile.js käyttää), `lib/learningPath.js` (getUserPath jne.), user_mastery/masteryThreshold/diagnostic-seed (eri "mastery"-konsepti, elävä), DB-taulut (ei dropattu). Verify: 1286 unit-testiä PASS, build PASS, e2e 6/6.
- **✅ curriculum.js loadCurriculum-zombie POISTETTU (L-V394d).** Subagentti mappasi call-graphin: `loadCurriculum` + koko `#screen-path` render-chain (renderSkeleton/renderKurssitList/renderCard/renderBento*/renderPathTOC/ensurePathRoot + orvot helperit) reachable VAIN kuolleesta koodista; `openLesson`-ketju (openLesson/renderLessonPage/ctaLabel/startExercises/goBack + jaetut escapeHtml+renderMarkdown) live. Poistettu ~587 riviä (curriculum.js 932→345). Ainoa cross-chain-kytkös: `goBack` kutsui poistettua `loadCurriculum`:ia → kirjoitettu uudelleen `location.hash="#/oppimispolku"`. Poistettu main.js:n käyttämätön `loadCurriculum`-import. Verify: build PASS, node --check PASS, 1286 unit-testiä PASS, e2e lesson-shell 4/4 (myös vahvistaa että V394b-sidebar-muutos EI rikkonut book-mode-piilotusta) + path-nav 3/3 + sidebar 3/3. `openLesson` staattisesti verifioitu (build resolvoi kaikki viittaukset, kaikki ketjun helperit KEEP, goBack korjattu, ei top-level-sivuvaikutuksia).
...
- **5.3 — V1: sidebar `display: !important`-sota → ✅ KORJATTU.** Juurisyy oli load-order: `sidebar-shell.css` (single-source-of-truth, ladataan VIIMEISENÄ) asetti `.app-sidebar { display:grid }` kaikille leveyksille → pakotti `off-canvas-nav.css:48`:n käyttämään `!important`:ia voittaakseen mobiilidrawerin, mikä taas pakotti `app-shell.css:69` auth-mode-säännön eskaloitumaan `!important`:iin (L-V378-kommentti dokumentoi aiemman auth-vuotobugin tästä). **Korjaus:** `sidebar-shell` display:grid skoupattu `@media (min-width:1024px)` (desktop omistaa gridin), off-canvas omistaa mobiilidrawerin ilman `!important`:ia, auth-mode-sääntö voittaa luonnollisella spesifisyydellä (0,2,0 > 0,1,0) ilman `!important`:ia. Poistettu 2 `!important` + 1 redundantti sääntö (`app-shell:56`). **Verify:** `tests/e2e-v394-sidebar-display.spec.js` pinnaa computed-`display`:n 5 tilassa (desktop=grid, mobile-closed/open=flex, auth-desktop/mobile=none ml. L-V378-vuotoguard); ennen JA jälkeen identtiset; visuaalinen vahvistus desktop-grid + mobiilidrawer-screenshoteista. Muutos liikkuu POIS L-V378-bugiluokasta (vähemmän `!important` = JS-inline-tila voittaa kuten on tarkoitus).
