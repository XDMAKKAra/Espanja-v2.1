...
**Current state:** L-LANDING-REBUILD-AND-BUGFIX-1 ✓ shipped. Landing palautettu dbce727-rakenteesta (254 → 923 r) 3-kielisenä: NAV+HERO(browser-frame+dashboard-1440.png)+PROBLEM+3 PILLARS+LANG CARDS+COURSES(8 kurssia A→E)+HOW IT WORKS+AI GRADING(YTL 13/20)+3-tier PRICING(Free/Treeni/Mestari)+FAQ(7 q)+FINAL CTA+FOOTER. Dashboard KPI-rivi näkyy nyt vaakagridissä (root cause: `app.bundle.css` oli L-FRONTEND-POLISH-1:n jälkeen stale; bundle rebuildattu). Curriculum-fetch: `?lang=es` eksplisiittisesti + tarkat virheviestit + poistettu `state.language = "spanish"` -bugi `curriculum.js:555` + `lessonResults.js:253` (aiheutti coming-soon-redirectin lesson-startin jälkeen). Reading-exercise mobile-padding + short-answer/tf-btn fallbacks yhtenäistetty `.ex-option`-canonin kanssa. Bug-scan 38 PASS / 0 FAIL / 0 P0-P2 löydetty 17 view × 2 viewportilla. SW v141→v142. **NEXT:** käyttäjän (a) selaintesti `/index.html` + `/app.html#kotinakyma` + `/app.html#oppimispolku`; (b) Vercel-deploy.
