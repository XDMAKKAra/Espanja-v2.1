...
import { GRADES, GRADE_ORDER, DAY_MS, WEEK_MS } from "./constants.js";
...
// The codebase carries two conventions: full words ("spanish"/"french"/
// "german" — exercise_bank, sr_cards, dashboard `language` param, client
// state.language) and short ISO codes ("es"/"fr"/"de" — curriculum/digikirja
// routes). Progress tables (exercise_logs, user_mistakes, exam_sessions,
// diagnostic_results) store the full word to stay consistent with the
// dashboard param they are filtered against. Normalize anything to the full
// word here so a stray "es" can never silently miss a "spanish" row. Defaults
// to "spanish" for any unknown/empty input (today the only language with
// content), so existing behaviour is unchanged.
const _LANG_FULL = {
...
export function normalizeLang(input) {
  return _LANG_FULL[String(input || "").trim().toLowerCase()] || "spanish";
}
...
// ─── L-V340 load-test hooks (inert unless LOAD_TEST=1) ──────────────────────
// LOAD_TEST_AI=stub  → return a deterministic grading-shaped object with no
//   network call. Used for the bulk concurrency ramp so we measure route +
//   middleware capacity, never OpenAI speed or cost.
// LOAD_TEST_AI=real  → hit the real API but enforce a hard cumulative budget
//   ceiling (LOAD_TEST_AI_BUDGET_EUR, default 0.10). Cost is computed from the
//   actual usage tokens of each completed call; once the running total reaches
//   the cap the NEXT call throws before any spend. Every call logs its cost +
//   the cumulative total. This is the only path in the whole load test that
//   spends money.
const _LT = process.env.LOAD_TEST === "1";
const _LT_AI = process.env.LOAD_TEST_AI || (_LT ? "stub" : "");
const _LT_BUDGET_EUR = Number(process.env.LOAD_TEST_AI_BUDGET_EUR) || 0.10;
// gpt-4o-mini pricing (USD per 1M tokens) — treated 1:1 as EUR for a
// conservative ceiling (EUR call is cheaper than USD, so we abort early).
const _PRICE_IN = 0.15 / 1e6;
const _PRICE_OUT = 0.60 / 1e6;
let _ltSpend = 0;
let _ltCalls = 0;
function _ltCost(usage) {
  return (usage.inputTokens || 0) * _PRICE_IN + (usage.outputTokens || 0) * _PRICE_OUT;
}
function _ltStubResult() {
  // Shape satisfies lib/writingGrading.js processGradingResult (four rubric
  // dimensions) so the grade-writing route returns 200 under load.
  return {
    viestinnallisyys: { score: 3, feedback_fi: "Load-test stub." },
    kielen_rakenteet: { score: 3, feedback_fi: "Load-test stub." },
    sanasto: { score: 3, feedback_fi: "Load-test stub." },
    kokonaisuus: { score: 3, feedback_fi: "Load-test stub." },
    corrected_text: "Load-test stub corrected text.",
    errors: [],
    annotations: [],
    overall_feedback_fi: "Load-test stub feedback.",
    next_action_fi: "Load-test stub.",
    _usage: { inputTokens: 0, outputTokens: 0, totalTokens: 0 },
  };
}

export async function callOpenAI(prompt, maxTokens = 2000, opts = {}) {
  if (_LT_AI === "stub") {
    return _ltStubResult();
  }
  if (_LT_AI === "real") {
    if (_ltSpend >= _LT_BUDGET_EUR) {
      console.error(
        `[load-test] AI budget cap hit: €${_ltSpend.toFixed(4)} >= €${_LT_BUDGET_EUR} after ${_ltCalls} real calls — refusing further OpenAI spend.`
      );
      throw new Error("LOAD_TEST_BUDGET_EXCEEDED");
    }
  }
  ...
  // Model override (opts.model) lets the validation harness swap in a different
  // grading model (e.g. gpt-5.4-mini) without touching the production default.
  // Production callers omit opts.model → OPENAI_MODEL (gpt-4o-mini) as before.
  const model = opts.model || OPENAI_MODEL;

  // Check cache — SHA-256 of full prompt + maxTokens + model
  const cacheKey = getCacheKey(prompt, maxTokens, model);
  ...
  // and only accept the default temperature (1). Branch on the model family so a
  // harness model swap can't fail on those API differences. gpt-4o-mini (prod)
  // keeps the legacy params untouched.
  ...
  try {
    ...
    // Load-test budget accounting (real-AI burst only): tally actual cost from
    // usage tokens and log the running total so the cap is auditable.
    if (_LT_AI === "real") {
      const cost = _ltCost(parsed._usage);
      _ltSpend += cost;
      _ltCalls += 1;
      console.log(
        `[load-test] real OpenAI call #${_ltCalls}: €${cost.toFixed(5)} (tokens in=${parsed._usage.inputTokens} out=${parsed._usage.outputTokens}); cumulative €${_ltSpend.toFixed(5)} / cap €${_LT_BUDGET_EUR}`
      );
    }
...
// ─── Shared constants ───────────────────────────────────────────────────────
// Defined in lib/constants.js (single source of truth); imported above and
// re-exported here so existing call-sites that import them from lib/openai.js
// keep working unchanged.
export { GRADES, GRADE_ORDER, DAY_MS, WEEK_MS };
