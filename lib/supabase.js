...
import { LOAD_TEST, createStubClient } from "./load-test-stubs.js";
dotenv.config();
...
// Under LOAD_TEST every client is an in-memory fake (see load-test-stubs.js):
// guarantees the load driver can never reach the production Supabase project.
export const adminClient = LOAD_TEST
  ? createStubClient()
  : url
...
export const authClient = LOAD_TEST
  ? createStubClient()
  : url
  ? createClient(url, serviceRoleKey, {
      auth: { autoRefreshToken: false, persistSession: false },
...
export function createUserClient(jwt) {
  if (LOAD_TEST) return adminClient;
