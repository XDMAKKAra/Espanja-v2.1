 ...
 *
 * L-LANG-INFRA-1: readLessonFile() added for multi-language file loading.
 * readPregeneratedLesson() in routes/curriculum.js delegates here so that
 * the data/courses/${lang}/ path is centralised in one place.
 */

import { readFile } from "node:fs/promises";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { PRODUCT_LANGS } from "./constants.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = path.resolve(__dirname, "..");

const SUPPORTED_LANGS = PRODUCT_LANGS;
// fr/de shipped 2026-05-25 (90 lessons each under data/courses/{fr,de}). Gate
// opened L-V339 — all three languages now serve pre-generated lessons.
const LANGS_WITH_CONTENT = new Set(["es", "de", "fr"]);

/**
 * Read a pre-generated lesson JSON from data/courses/${lang}/${courseKey}/lesson_${index}.json.
 *
 * Returns:
 * - Parsed lesson object if found and not a placeholder.
 * - null if file is missing or is a placeholder (caller falls through to runtime generation).
 * - { available: false, language: lang, message: string } if the language has no content yet.
 *
 * @param {string} courseKey - e.g. "kurssi_1"
 * @param {number} lessonIndex
 * @param {string} [lang="es"]
 */
export async function readLessonFile(courseKey, lessonIndex, lang = "es") {
  const safeLang = SUPPORTED_LANGS.has(lang) ? lang : "es";
  if (!LANGS_WITH_CONTENT.has(safeLang)) {
    return {
      available: false,
      language: safeLang,
  ...
  try {
    const filePath = path.join(REPO_ROOT, "data", "courses", safeLang, courseKey, `lesson_${lessonIndex}.json`);
    ...
    if (err.code !== "ENOENT") console.error("readLessonFile error:", err.message);
    return null;
  }
}
