...
/**
 * Infer the target language ("spanish" | "german" | "french") from kurssi
 * metadata when language is not passed in explicitly. Uses grammar_focus keys
 * and the kurssi title — both are language-specific enough to distinguish
 * all three supported lyhyt-oppimäärä languages unambiguously.
 */
function inferKurssiLanguage(kurssi) {
  const focuses = (kurssi.grammar_focus || []).join(" ");
  const title = kurssi.title || "";
  if (
    focuses.includes("praeteritum") ||
    focuses.includes("perfekt") ||
    focuses.includes("konjunktiv") ||
    focuses.includes("dative") ||
    title.includes("Alltag") ||
    title.includes("Abitur") ||
    title.includes("Wer bin")
  ) return "german";
  if (
    focuses.includes("passe_compose") ||
    focuses.includes("subjonctif") ||
    focuses.includes("imparfait") ||
    focuses.includes("conditionnel") ||
    focuses.includes("plus_que_parfait") ||
    title.includes("quotidienne") ||
    title.includes("bac") ||
    title.includes("Qui je")
  ) return "french";
  return "spanish";
}

/** Maps internal language string → fi_to_<code> direction value. */
const LANG_TO_DIRECTION = {
  spanish: "fi_to_es",
  german:  "fi_to_de",
  french:  "fi_to_fr",
};
 ...
 * Spans every grammar_focus[] topic of the kurssi evenly, and enforces
 * a mandatory item-type mix so the AI cannot return 100% multiple-choice.
 *
 * @param {object} ctx - lessonCtx from resolveLessonContext
 * @param {string} [language] - optional explicit language ("spanish"|"german"|"french");
 *   if omitted the language is inferred from kurssi metadata.
 */
export function curriculumTestInstruction(ctx, language) {
  if (!ctx) return "";
  ...
  // Resolve fi_to_<lang> direction for typed/translate items.
  const lang = language || inferKurssiLanguage(kurssi);
  const direction = LANG_TO_DIRECTION[lang] || "fi_to_es";

  // Pre-compute approximate item counts for the mix directive.
  const mcCount     = Math.round(n * 0.47);
  const gapCount    = Math.round(n * 0.23);
  const typedCount  = Math.round(n * 0.17);
  const transCount  = Math.round(n * 0.10);
  // match gets whatever is left (target ~3 %, min 1).
  const matchCount  = Math.max(1, n - mcCount - gapCount - typedCount - transCount);
  ...
  return [
    ...
    "",
    "TÄRKEÄ — TEHTÄVÄTYYPPIEN MONIPUOLISUUS (PAKOLLINEN):",
    `Generoi yhteensä ${n} kysymystä jakauttuneena tarkasti näin (pyöristä tarvittaessa lähimpään kokonaislukuun, ja lisää erotus aukkotehtäviin):`,
    `- mc (monivalinta): ~47 % → ${mcCount} kpl`,
    `- gap_fill (aukkotehtävä, käytä {1}-syntaksia): ~23 % → ${gapCount} kpl`,
    `- typed (lyhyt käännös FI→kohdekieli, 4–7 sanaa, direction: "${direction}"): ~17 % → ${typedCount} kpl`,
    `- translate (pidempi käännös, 8–14 sanaa, direction: "${direction}"): ~10 % → ${transCount} kpl`,
    `- match (sanaparituselementti, 4 paria): ~3 % → ${matchCount} kpl (vähintään 1 per testi)`,
    "",
    "ÄLÄ KOSKAAN palauta yli 60 % saman tyypin kysymyksiä. Jos teet niin, palautus hylätään.",
    "",
    "Jokaisessa kysymyksessä TÄYTYY olla \"item_type\"-kenttä, ja sen arvon TÄYTYY olla tarkasti yksi seuraavista: \"mc\" | \"gap_fill\" | \"typed\" | \"translate\" | \"match\".",
    "",
    "Itemin minimivaatimukset:",
    "- mc: { item_type, stem, choices[4], correct_index, explanation, topic_key }",
    "- gap_fill: { item_type, sentence_template (sisältää {1} tai {2}), answers[][] (yksi sisempi tarkasti per blank), word_bank|null, topic_key }",
    `- typed: { item_type, direction ("${direction}"), prompt, accept[] (2–4 varianttia, sis. aksentittomat), topic_key }`,
    `- translate: { item_type, direction ("${direction}"), source, accept[] (2–4 varianttia), topic_key }`,
    "- match: { item_type, pairs[4] ({left,right}), topic_key }",
    levelDirective,
  ].filter(Boolean).join("\n");
}
