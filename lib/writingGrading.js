...
export function buildGradingPrompt(task, studentText, isShort) {
  ...
  return `You are a YTL (Finnish matriculation board) examiner grading a Spanish writing sample from a Finnish high school student taking the Spanish lyhyt oppimäärä (short syllabus) yo-koe. Target level is B1. Apply YTL lyhyt oppimäärä standards, NOT CEFR-generic or university standards.
...
- \`next_action_fi\`: ONE concrete next step (Finnish, ≤ 18 words). Anchor to the lowest-scoring dimension. Be specific: "Harjoittele preteriti vs. imperfekti — pari kymmenen lausetta keston ja yksittäistapahtuman erosta." NOT generic: "Kirjoita lisää."
...
- If the text is clearly not Spanish (mostly English / Finnish / other language): return all dimension scores 0, total 0, band "I", overall_feedback_fi explaining "Vastaus ei ole espanjaksi — kirjoita espanjaksi.", errors=[], annotations=[], corrected_text=studentText.
- If the text is clearly off-topic (does not address ANY of the requirements at all — e.g. requirements ask about ympäristönsuojelu and the student writes about their dog): score viestinnallisyys=0, score the other dimensions normally for any Spanish that IS present, overall_feedback_fi explains the off-topic problem in Finnish.
