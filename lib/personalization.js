...
// ─── Exercise-pool weighting (L-V315b Task 2) ──────────────────────────────

/**
 * Painotuskerroin yhdelle aiheelle. skillProfile-rekisteröity aihe saa
 * 3.0 jos taso ≤ 2 (heikko), 0.3 jos taso ≥ 4 (vahva), 1.0 muutoin.
 * Jos aihe puuttuu profilesta, fallback: kurssi-tason painotus
 * courseWeights × TOPIC_TO_PUHEO_COURSE[lang][topic] -mappauksen kautta.
 */
function topicWeight(topic, skillProfile, courseWeights, lang) {
  const entry = skillProfile?.[topic];
  if (entry && Number.isFinite(entry.level)) {
    if (entry.level <= 2) return 3.0;
    if (entry.level >= 4) return 0.3;
    return 1.0;
  }
  if (courseWeights && lang && TOPIC_TO_PUHEO_COURSE?.[lang]?.[topic]) {
    const courses = TOPIC_TO_PUHEO_COURSE[lang][topic];
    if (Array.isArray(courses) && courses.length) {
      const ws = courses
        .map(c => courseWeights[c])
...
/**
 * Sample yksi aihe `availableTopics`-listasta painotetusti.
 * Painotus: heikkojen aiheiden frequency ≥ ~10x vahvojen, normalisoiu
 * kokonaisuudeksi roulette-wheel-samplingilla. Jos profilea ei ole tai
 * kaikki painot ovat nollia, palautuu uniform-sample (non-personalized
 * käyttö, admin-testaus, anonyymi käyttäjä).
 *
 * @param {Object|null} skillProfile  - buildSkillProfile().skillProfile tai null
 * @param {Object|null} courseWeights - buildSkillProfile().courseWeights tai null
 * @param {string[]} availableTopics  - lista topic-keystä joista valita
 * @param {string} lang               - 'es' | 'de' | 'fr' (kurssi-fallback)
 * @param {Function} [rng]            - testattavuuden vuoksi injektoitavissa
 * @returns {string|null}
 */
export function selectWeightedTopic(skillProfile, courseWeights, availableTopics, lang, rng = Math.random) {
  if (!Array.isArray(availableTopics) || availableTopics.length === 0) return null;

  const hasProfile = skillProfile && typeof skillProfile === "object" && Object.keys(skillProfile).length > 0;
  const hasWeights = courseWeights && typeof courseWeights === "object" && Object.keys(courseWeights).length > 0;

  if (!hasProfile && !hasWeights) {
    return availableTopics[Math.floor(rng() * availableTopics.length)];
  }

  const weights = availableTopics.map(t => topicWeight(t, skillProfile, courseWeights, lang));
  ...
  for (let i = 0; i < availableTopics.length; i++) {
    acc += weights[i];
    if (r <= acc) return availableTopics[i];
  }
  return availableTopics[availableTopics.length - 1];
}
