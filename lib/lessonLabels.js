...
export function getLessonLabel(sourceKey) {
  ...
  if (sourceKey.endsWith("_review")) {
    ...
    const num = kurssiNumberFromKey(kurssiKey);
    if (!kurssi) return num != null ? `Kurssi ${num} · yleiskertaus` : "Yleiskertaus";
  ...
  if (!kurssi) {
    const num = kurssiNumberFromKey(kurssiKey);
    return num != null ? `Kurssi ${num} · oppitunti ${lessonIdx}` : `Oppitunti ${lessonIdx}`;
  }
