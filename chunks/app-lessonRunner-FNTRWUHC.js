import{a as M,b as _}from"./app-chunk-EVWRWXV5.js";import{b as T}from"./app-chunk-BSBZXMKL.js";import{b as j}from"./app-chunk-AE7C6F2Z.js";import"./app-chunk-2SASPNNN.js";import{a as E,c as S,e as A,j as O}from"./app-chunk-T52YLBP4.js";import{b as C}from"./app-chunk-PXMVMW5B.js";function B(t,e){let s=t?.mastery_threshold;return s?typeof s[e]=="number"?s[e]:s.B??.7:.7}function q(t,e){let s=t?.skip_for_targets;return Array.isArray(s)&&s.includes(e)}var y="lesson-runner-root",V="puheo:lessonProgress:";function N(t){let e=t&&typeof t.language=="string"&&t.language||"es",s=t&&t.kurssiKey?encodeURIComponent(t.kurssiKey):"",a=s?`#/oppimispolku/${e}/${s}`:`#/oppimispolku?lang=${e}`;location.hash!==a&&(location.hash=a)}var v=new Map;async function z(t){if(!t)return[];if(v.has(t))return v.get(t);try{let{API:e,isLoggedIn:s,authHeader:a,apiFetch:n}=await import("./app-api-GLA7PASW.js"),i=await n(`${e}/api/curriculum/${encodeURIComponent(t)}`,{headers:{...s()?a():{}}});if(!i.ok)return v.set(t,[]),[];let r=await i.json(),l=Array.isArray(r.lessons)?r.lessons.slice().sort((o,c)=>(o.sortOrder||0)-(c.sortOrder||0)):[];return v.set(t,l),l}catch{return[]}}function x(t){let e=Array.isArray(t.phases)?t.phases:[];if(e.length===0)return`<aside class="lr-toc" aria-label="Teht\xE4v\xE4t">
  ...
  </aside>`}function w(t,e){t.querySelectorAll(".lr-toc__item:not(.is-current)").forEach(s=>{let a=()=>{let n=Number(s.dataset.phase);!Number.isFinite(n)||n===e.currentPhaseIdx||(k(e),e.currentPhaseIdx=n,e.currentItemIdx=0,e.correctInPhase=0,e.answeredInPhase=0,k(e),b(t,e))};s.addEventListener("click",a),s.addEventListener("keydown",n=>{(n.key==="Enter"||n.key===" ")&&(n.preventDefault(),a())})})}function R(t){let e=t.lesson.meta||{},s=e.kurssi_title||e.course_title||e.course_key||"Kurssi",a=e.lesson_index||t.lessonIndex,n=e.title||"Oppitunti";return`<nav class="lr-breadcrumb" aria-label="Sijainti">
  ...
  </nav>`}function P(t,e){return`${V}${t}:${e}`}function k(t){if(!(!t||!t.kurssiKey||t.finished)&&!(t.currentPhaseIdx===0&&t.currentItemIdx===0&&t.answeredInPhase===0))try{sessionStorage.setItem(P(t.kurssiKey,t.lessonIndex),JSON.stringify({currentPhaseIdx:t.currentPhaseIdx,currentItemIdx:t.currentItemIdx,correctInPhase:t.correctInPhase,answeredInPhase:t.answeredInPhase,phaseResults:t.phaseResults,targetGrade:t.targetGrade,phaseSignature:t.phases.map(e=>`${e.phase_id}:${e.items.length}`).join("|"),savedAt:Date.now()}))}catch{}}function J(t,e,s){try{let a=sessionStorage.getItem(P(t,e));if(!a)return null;let n=JSON.parse(a);if(!n||typeof n!="object")return null;let i=s.phases.map(r=>`${r.phase_id}:${r.items.length}`).join("|");return n.phaseSignature!==i||n.savedAt&&Date.now()-n.savedAt>1440*60*1e3?null:n}catch{return null}}function Y(t,e){try{sessionStorage.removeItem(P(t,e))}catch{}}function u(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function $(t){return String(t||"").toLowerCase().trim().normalize("NFD").replace(/[̀-ͯ]/g,"").replace(/\s+/g," ")}function H(t,e,s="es"){let a=e||[],n=$(t);if(n&&a.some(i=>$(i)===n))return{ok:!0,hint:null};for(let i of a){let r=j(t,i,s);if(r.ok)return r}return{ok:!1,hint:null}}function U(t,e,s,a){let n=(t.phases||[]).filter(i=>!q(i,e));return{lesson:t,kurssiKey:s,lessonIndex:a,targetGrade:e,phases:n,currentPhaseIdx:0,currentItemIdx:0,correctInPhase:0,answeredInPhase:0,phaseResults:[],sidePanelOpen:!1,sidePanelOpenMs:0,sidePanelOpenedAt:0,startedAt:Date.now(),finished:!1}}function Ie(t,e,s,a){let n=t.pregenerated||t;W(),C("screen-lesson");let i=document.getElementById(y);if(!i)return;let r=U(n,a||"B",e,s);try{sessionStorage.setItem("currentLesson",JSON.stringify({kurssiKey:e,lessonIndex:s,lessonFocus:n.meta?.title||"",lessonType:n.meta?.lesson_type||"",targetGrade:r.targetGrade,isPregenerated:!0}));let o=n.teaching||{},c=String(o.intro_md||"").trim(),d=Array.isArray(o.key_points)?o.key_points:[],p=[c,d.length?`## Avainkohdat
`+d.map(h=>`- ${h}`).join(`
`):""].filter(Boolean).join(`

`);p?sessionStorage.setItem("currentLessonTeachingMd",p):sessionStorage.removeItem("currentLessonTeachingMd")}catch{}r._courseLessons=[],z(e).then(o=>{r._courseLessons=o;let c=document.querySelector(".lr-shell .lr-toc");if(c&&c.parentElement){let d=document.createElement("div");d.innerHTML=x(r),c.parentElement.replaceChild(d.firstElementChild,c),w(document.getElementById(y),r)}});let l=J(e,s,r);if(l){r.currentPhaseIdx=l.currentPhaseIdx,r.currentItemIdx=l.currentItemIdx,r.correctInPhase=l.correctInPhase||0,r.answeredInPhase=l.answeredInPhase||0,r.phaseResults=Array.isArray(l.phaseResults)?l.phaseResults:[],r.startedAt=Date.now(),b(i,r);return}G(i,r)}function W(){let t=document.getElementById("screen-lesson");t||(t=document.createElement("div"),t.id="screen-lesson",t.className="screen",document.querySelector(".app-main")?.appendChild(t)||document.body.appendChild(t)),document.getElementById(y)||(t.innerHTML=`<div id="${y}"></div>`)}function G(t,e){let s=e.lesson.meta||{},a=e.lesson.teaching||{},n=K(a.intro_md||""),i=Array.isArray(a.key_points)?a.key_points:[],r=i.length?`<ul class="lr-keypoints">${i.map(l=>`<li>${u(l)}</li>`).join("")}</ul>`:"";t.innerHTML=`
    ...
    </div>`,w(t,e),document.getElementById("lr-back")?.addEventListener("click",()=>{N(e)}),document.getElementById("lr-start")?.addEventListener("click",()=>{e.startedAt=Date.now(),b(t,e)})}function b(t,e){let s=e.phases[e.currentPhaseIdx];if(!s)return D(t,e);let a=e.phases.length,n=X(e),i=s.items[e.currentItemIdx];if(!i)return L(t,e,s,"completed");t.innerHTML=`
    <div class="lr-shell lr-shell--exercise lr-shell--three">
      ...
      ${pe(e)}
    </div>`,ae(t,e,i),w(t,e)}function X(t){return`<span class="lr-stepper" aria-hidden="true">${t.phases.map((s,a)=>a<t.currentPhaseIdx?'<span class="lr-step lr-step--done" aria-hidden="true">\u25CF</span>':a===t.currentPhaseIdx?'<span class="lr-step lr-step--current" aria-hidden="true">\u25CF</span>':'<span class="lr-step" aria-hidden="true">\u25CB</span>').join("")}</span>`}function Q(t,e){switch(t.item_type){case"mc":return Z(t);case"typed":return F(t);case"translate":return ee(t);case"match":return te(t);case"gap_fill":return se(t);case"writing":return ne(t);case"reading_mc":return re(t);default:return`<p class="lr-unsupported">Teht\xE4v\xE4tyyppi\xE4 "${u(t.item_type||"?")}" ei tueta, ohitetaan.</p>
