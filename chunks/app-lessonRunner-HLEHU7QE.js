import{a as H,b as E,c as N,d as A}from"./app-chunk-XSJXYF7Z.js";import{b as R}from"./app-chunk-BSBZXMKL.js";import{b as B}from"./app-chunk-AE7C6F2Z.js";import{c as y}from"./app-chunk-2SASPNNN.js";import{b as S}from"./app-chunk-CTTO4TQX.js";import{a as _,c as k,e as w,j as x}from"./app-chunk-T52YLBP4.js";function F(t,e){let s=t?.mastery_threshold;return s?typeof s[e]=="number"?s[e]:s.B??.7:.7}function D(t,e){let s=t?.skip_for_targets;return Array.isArray(s)&&s.includes(e)}function v(t){return String(t||"").toLowerCase().trim().normalize("NFD").replace(/[̀-ͯ]/g,"").replace(/\s+/g," ")}function O(t,e,s="es"){let a=e||[],n=v(t);if(n&&a.some(i=>v(i)===n))return{ok:!0,hint:null};for(let i of a){let r=B(t,i,s);if(r.ok)return r}return{ok:!1,hint:null}}var $="lesson-runner-root",G="puheo:lessonProgress:";function K(t){let e=t&&typeof t.language=="string"&&t.language||"es",s=t&&t.kurssiKey?encodeURIComponent(t.kurssiKey):"",a=s?`#/oppimispolku/${e}/${s}`:`#/oppimispolku?lang=${e}`;location.hash!==a&&(location.hash=a)}var P=new Map;async function Q(t){if(!t)return[];if(P.has(t))return P.get(t);try{let{API:e,isLoggedIn:s,authHeader:a,apiFetch:n}=await import("./app-api-GLA7PASW.js"),i=await n(`${e}/api/curriculum/${encodeURIComponent(t)}`,{headers:{...s()?a():{}}});if(!i.ok)return P.set(t,[]),[];let r=await i.json(),l=Array.isArray(r.lessons)?r.lessons.slice().sort((d,c)=>(d.sortOrder||0)-(c.sortOrder||0)):[];return P.set(t,l),l}catch{return[]}}function T(t){let e=Array.isArray(t.phases)?t.phases:[];if(e.length===0)return`<aside class="lr-toc" aria-label="Teht\xE4v\xE4t">
  ...
  </aside>`}function j(t,e){t.querySelectorAll(".lr-toc__item:not(.is-current)").forEach(s=>{let a=()=>{let n=Number(s.dataset.phase);!Number.isFinite(n)||n===e.currentPhaseIdx||(I(e),e.currentPhaseIdx=n,e.currentItemIdx=0,e.correctInPhase=0,e.answeredInPhase=0,I(e),g(t,e))};s.addEventListener("click",a),s.addEventListener("keydown",n=>{(n.key==="Enter"||n.key===" ")&&(n.preventDefault(),a())})})}function V(t){let e=t.lesson.meta||{},s=e.kurssi_title||e.course_title||e.course_key||"Kurssi",a=e.lesson_index||t.lessonIndex,n=e.title||"Oppitunti";return`<nav class="lr-breadcrumb" aria-label="Sijainti">
  ...
  </nav>`}function M(t,e){return`${G}${t}:${e}`}function I(t){if(!(!t||!t.kurssiKey||t.finished)&&!(t.currentPhaseIdx===0&&t.currentItemIdx===0&&t.answeredInPhase===0))try{sessionStorage.setItem(M(t.kurssiKey,t.lessonIndex),JSON.stringify({currentPhaseIdx:t.currentPhaseIdx,currentItemIdx:t.currentItemIdx,correctInPhase:t.correctInPhase,answeredInPhase:t.answeredInPhase,phaseResults:t.phaseResults,targetGrade:t.targetGrade,phaseSignature:t.phases.map(e=>`${e.phase_id}:${e.items.length}`).join("|"),savedAt:Date.now()}))}catch{}}function X(t,e,s){try{let a=sessionStorage.getItem(M(t,e));if(!a)return null;let n=JSON.parse(a);if(!n||typeof n!="object")return null;let i=s.phases.map(r=>`${r.phase_id}:${r.items.length}`).join("|");return n.phaseSignature!==i||n.savedAt&&Date.now()-n.savedAt>1440*60*1e3?null:n}catch{return null}}function Z(t,e){try{sessionStorage.removeItem(M(t,e))}catch{}}function u(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function z(t,e,s,a){let n=(t.phases||[]).filter(i=>!D(i,e));return{lesson:t,kurssiKey:s,lessonIndex:a,targetGrade:e,language:typeof y.language=="string"&&y.language||"es",phases:n,currentPhaseIdx:0,currentItemIdx:0,correctInPhase:0,answeredInPhase:0,gradedItems:[],phaseResults:[],sidePanelOpen:!1,sidePanelOpenMs:0,sidePanelOpenedAt:0,startedAt:Date.now(),finished:!1}}async function ee(t){try{let e=await x(`${_}/api/curriculum/review-queue?lang=${encodeURIComponent(t||"es")}&limit=8`,{headers:{...w()}});if(!e||!e.ok)return null;let s=await e.json();return s&&!s.locked&&Array.isArray(s.items)&&s.items.length?s:null}catch{return null}}async function te(t){try{if(typeof k=="function"&&!k())return;let e=await Promise.race([ee(t.language),new Promise(s=>setTimeout(()=>s(null),1500))]);e&&Array.isArray(e.items)&&e.items.length&&(t.phases=[A(e),...t.phases])}catch{}}async function Ce({items:t,lang:e}){let s={meta:{title:"Kertaus",lesson_type:"review",course_key:"kertaus",lesson_index:0},phases:[A({items:Array.isArray(t)?t:[]})]};J(),S("screen-lesson");let a=document.getElementById($);if(!a)return;let n=z(s,"B","kertaus",0);n._isReviewSession=!0,n.language=e||typeof y.language=="string"&&y.language||"es",g(a,n)}async function Te(t,e,s,a){let n=t.pregenerated||t;J(),S("screen-lesson");let i=document.getElementById($);if(!i)return;let r=z(n,a||"B",e,s);try{sessionStorage.setItem("currentLesson",JSON.stringify({kurssiKey:e,lessonIndex:s,lessonFocus:n.meta?.title||"",lessonType:n.meta?.lesson_type||"",targetGrade:r.targetGrade,isPregenerated:!0}));let d=n.teaching||{},c=String(d.intro_md||"").trim(),o=Array.isArray(d.key_points)?d.key_points:[],p=[c,o.length?`## Avainkohdat
`+o.map(h=>`- ${h}`).join(`
`):""].filter(Boolean).join(`

`);p?sessionStorage.setItem("currentLessonTeachingMd",p):sessionStorage.removeItem("currentLessonTeachingMd")}catch{}r._courseLessons=[],Q(e).then(d=>{r._courseLessons=d;let c=document.querySelector(".lr-shell .lr-toc");if(c&&c.parentElement){let o=document.createElement("div");o.innerHTML=T(r),c.parentElement.replaceChild(o.firstElementChild,c),j(document.getElementById($),r)}}),await te(r);let l=X(e,s,r);if(l){r.currentPhaseIdx=l.currentPhaseIdx,r.currentItemIdx=l.currentItemIdx,r.correctInPhase=l.correctInPhase||0,r.answeredInPhase=l.answeredInPhase||0,r.phaseResults=Array.isArray(l.phaseResults)?l.phaseResults:[],r.startedAt=Date.now(),g(i,r);return}se(i,r)}function J(){let t=document.getElementById("screen-lesson");t||(t=document.createElement("div"),t.id="screen-lesson",t.className="screen",document.querySelector(".app-main")?.appendChild(t)||document.body.appendChild(t)),document.getElementById($)||(t.innerHTML=`<div id="${$}"></div>`)}function se(t,e){let s=e.lesson.meta||{},a=e.lesson.teaching||{},n=Y(a.intro_md||""),i=Array.isArray(a.key_points)?a.key_points:[],r=i.length?`<ul class="lr-keypoints">${i.map(l=>`<li>${u(l)}</li>`).join("")}</ul>`:"";t.innerHTML=`
    ...
    </div>`,j(t,e),document.getElementById("lr-back")?.addEventListener("click",()=>{K(e)}),document.getElementById("lr-start")?.addEventListener("click",()=>{e.startedAt=Date.now(),g(t,e)})}function g(t,e){let s=e.phases[e.currentPhaseIdx];if(!s)return W(t,e);let a=e.phases.length,n=ne(e),i=s.items[e.currentItemIdx];if(!i)return q(t,e,s,"completed");t.innerHTML=`
    <div class="lr-shell lr-shell--exercise lr-shell--three">
      ...
      ${ye(e)}
    </div>`,ue(t,e,i),j(t,e)}function ne(t){return`<span class="lr-stepper" aria-hidden="true">${t.phases.map((s,a)=>a<t.currentPhaseIdx?'<span class="lr-step lr-step--done" aria-hidden="true">\u25CF</span>':a===t.currentPhaseIdx?'<span class="lr-step lr-step--current" aria-hidden="true">\u25CF</span>':'<span class="lr-step" aria-hidden="true">\u25CB</span>').join("")}</span>`}function re(t,e){switch(t.item_type){case"mc":return ae(t);case"typed":return U(t);case"translate":return ie(t);case"match":return le(t);case"gap_fill":return ce(t);case"writing":return oe(t);case"reading_mc":return de(t);default:return`<p class="lr-unsupported">Teht\xE4v\xE4tyyppi\xE4 "${u(t.item_type||"?")}" ei tueta, ohitetaan.</p>
    ...
    <div class="lr-typed">
      ...
      <div class="lr-feedback" id="lr-feedback" hidden></div>
    </div>`}function ie(t){return U({...t,prompt:t.source})}function le(t){let e=(t.pairs||[]).map(a=>a.left),s=be((t.pairs||[]).map(a=>a.right));return`
