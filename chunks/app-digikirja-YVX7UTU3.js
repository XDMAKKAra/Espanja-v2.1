import{a as ot}from"./app-chunk-XYSTN2TU.js";import{b as V}from"./app-chunk-AE7C6F2Z.js";import{a as S,c as q,d as N,e as I,j as A}from"./app-chunk-ECRDZOTG.js";import{b as rt}from"./app-chunk-PXMVMW5B.js";function Qt(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function b(t){let e=Qt(t);return e=e.replace(/`([^`]+)`/g,'<code class="dk__teoria-code">$1</code>'),e=e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),e=e.replace(/\*([^*\n]+)\*/g,"<em>$1</em>"),e=e.replace(/(^|\s)_([^_\n]+)_(?=\s|[.,;:!?]|$)/g,"$1<em>$2</em>"),e}function ct(t){return/^\s*\|?\s*:?-{2,}:?(\s*\|\s*:?-{2,}:?\s*)+\|?\s*$/.test(t)}function dt(t){return t.trim().replace(/^\|/,"").replace(/\|$/,"").split("|").map(s=>s.trim())}function te(t,e){let s=dt(t),n=e.map(dt),a=s.map(o=>`<th>${b(o)}</th>`).join(""),i=n.map(o=>`<tr>${o.map(c=>`<td>${b(c)}</td>`).join("")}</tr>`).join("");return`<div class="dk__bilingual${s.length===2?" dk__bilingual--2col":""}"><table><thead><tr>${a}</tr></thead><tbody>${i}</tbody></table></div>`}function ee(t){return`
    ...
    </aside>`}function se(t){return`<ul class="dk__teoria-ul">${t.map(s=>`<li>${b(s.replace(/^[-*]\s+/,""))}</li>`).join("")}</ul>`}function lt(t){if(!t||typeof t!="string")return"";let e=t.replace(/\r\n?/g,`
`).split(`
`),s=[],n=0,a=!1;for(;n<e.length;){let i=e[n];if(/^\s*$/.test(i)){n++;continue}let r=/^(#{1,6})\s+(.*)$/.exec(i);if(r){let l=r[1].length,c=r[2].trim();if(l===1&&!a){a=!0,n++;continue}l===2?s.push(`<h3 class="dk__teoria-h2">${b(c)}</h3>`):l===3?s.push(`<h4 class="dk__teoria-h3">${b(c)}</h4>`):s.push(`<h${Math.min(l+1,6)} class="dk__teoria-h">${b(c)}</h${Math.min(l+1,6)}>`),n++;continue}if(/^\s*>\s?/.test(i)){let l=[];for(;n<e.length&&/^\s*>\s?/.test(e[n]);)l.push(e[n]),n++;s.push(ee(l));continue}if(/^\s*\|/.test(i)&&n+1<e.length&&ct(e[n+1])){let l=i,c=[];for(n+=2;n<e.length&&/^\s*\|/.test(e[n]);)c.push(e[n]),n++;s.push(te(l,c));continue}if(/^\s*[-*]\s+/.test(i)){let l=[];for(;n<e.length&&/^\s*[-*]\s+/.test(e[n]);)l.push(e[n]),n++;s.push(se(l));continue}let o=[i];for(n++;n<e.length&&!/^\s*$/.test(e[n])&&!/^(#{1,6})\s+/.test(e[n])&&!/^\s*>\s?/.test(e[n])&&!/^\s*[-*]\s+/.test(e[n])&&!(/^\s*\|/.test(e[n])&&n+1<e.length&&ct(e[n+1]));)o.push(e[n]),n++;s.push(`<p class="dk__teoria-p">${b(o.join(" "))}</p>`)}return s.join(`
`)}var ft="puheo:dk:sidemenu",E="open",$="collapsed",ne="puheo:dk:progress";function vt(){return`${ne}:${p.lang}:${p.kurssiKey}:${p.lessonIndex}`}function B(){try{return JSON.parse(localStorage.getItem(vt())||"{}")}catch{return{}}}function gt(t){try{localStorage.setItem(vt(),JSON.stringify(t))}catch{}}function x(t){if(!t)return;let e=B();e[t]!=="done"&&(e[t]="done",gt(e),q()&&A(`${S}/api/digikirja/progress`,{method:"POST",headers:{"Content-Type":"application/json",...I()},body:JSON.stringify({lang:p.lang,kurssi:p.kurssiKey,lesson:p.lessonIndex,sivuId:t})}).catch(()=>{}),Q())}function Q(){let t=B(),e=document.getElementById("dk-sidemenu-list");e&&e.querySelectorAll(".dk__row").forEach(n=>{n.classList.toggle("is-done",t[n.dataset.sivu]==="done")});let s=document.getElementById("dk-progress-chip");if(s){let n=f.filter(a=>t[a.id]==="done").length;s.textContent=`${n} / ${f.length} valmis`,s.dataset.full=n>=f.length?"true":"false"}}var ae=[[/^YO-tyylinen käännös$/i,"K\xE4\xE4nn\xE4 lauseet"],[/^YO-tason käännös$/i,"K\xE4\xE4nn\xE4 lauseet"],[/^Lyhyt kirjoitus$/i,"Kirjoita lyhyt teksti"],[/^Lauseiden rakentelua$/i,"K\xE4\xE4nn\xE4 lauseet"],[/^Lauseen rakentelua$/i,"K\xE4\xE4nn\xE4 lauseet"],[/Lauseen täydennys\s*[—-]\s*/g,"Lauseen t\xE4ydennys, "],[/Tunnista\s*[—-]\s*/g,"Tunnista "],[/Tuota\s*[—-]\s*/g,"Tuota "],[/Yhdistä\s*[—-]\s*/g,"Yhdist\xE4 "],[/Käännä\s*[—-]\s*/g,"K\xE4\xE4nn\xE4 "]];function ie(t){if(!t)return"";let e=String(t);for(let[s,n]of ae)e=e.replace(s,n);return e=e.replace(/\s+—\s+/g,", "),e}var re={teoria:"Opetus",tehtava:"Harjoitukset",flashcards:"Kortit",testi:"Testit",itsearviointi:"Itsearvio"};var yt=5,oe="puheo:dk:flashcards",ut={},bt=!1,p={lang:"es",kurssiKey:"kurssi_2",lessonIndex:3,sivuId:"teoria"},g=null,f=[],D="";function d(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function ce(t){let e=[...t];for(let s=e.length-1;s>0;s--){let n=Math.floor(Math.random()*(s+1));[e[s],e[n]]=[e[n],e[s]]}return e}function de(){try{return localStorage.getItem(ft)===E?E:$}catch{return E}}function le(t){try{localStorage.setItem(ft,t)}catch{}}function ue(t){return t==="fr"?"Ranska":t==="de"?"Saksa":"Espanja"}function pe(t){let e=/^kurssi_(\d+)$/.exec(String(t||""));return e?`Kurssi ${e[1]}`:t||""}function _e(t){return`/data/courses/${encodeURIComponent(t.lang)}/${encodeURIComponent(t.kurssiKey)}/lesson_${encodeURIComponent(t.lessonIndex)}.json`}async function ke(t){let e=_e(t),s=await fetch(e,{headers:{accept:"application/json"}});if(!s.ok)throw new Error(`lesson fetch ${s.status}`);return s.json()}async function he(t){if(q())try{let e=`lang=${encodeURIComponent(t.lang)}&kurssi=${encodeURIComponent(t.kurssiKey)}&lesson=${encodeURIComponent(t.lessonIndex)}`,[s,n]=await Promise.all([A(`${S}/api/digikirja/progress?${e}`,{headers:I()}).catch(()=>null),A(`${S}/api/digikirja/itsearvio?${e}`,{headers:I()}).catch(()=>null)]);if(s?.ok){let{sivut:a}=await s.json().catch(()=>({sivut:{}}));if(a&&typeof a=="object"){let i=B();for(let r of Object.keys(a))i[r]||(i[r]="done");gt(i)}}if(n?.ok){let{itsearvio:a}=await n.json().catch(()=>({itsearvio:null}));a?.ratings&&(et()||Et({ratings:a.ratings,submittedAt:a.submittedAt,lang:t.lang,kurssiKey:t.kurssiKey,lessonIndex:t.lessonIndex}))}}catch{}}function me(t){let e=[];e.push({id:"teoria",kind:"teoria",num:"",title:t?.meta?.title||"Opetus",meta:"Opetussivu"}),(Array.isArray(t?.phases)?t.phases:[]).forEach((o,l)=>{let c=String(l+1),u=ie(o.title)||`Vaihe ${c}`,_=Array.isArray(o.items)?o.items.length:0;e.push({id:`phase-${l}`,kind:"tehtava",num:c,title:u,meta:_?`${_} kohtaa`:"Teht\xE4v\xE4"})});let n=t?.meta?.lesson_type||"vocab",a=new Set(["vocab","mixed"]),i=Array.isArray(t?.vocab)?t.vocab:[],r=Math.min(i.length,yt);return r>0&&a.has(n)&&e.push({id:"kortit-1",kind:"flashcards",num:"",title:`K\xE4\xE4nt\xF6kortit \xB7 ${t?.meta?.title||""}`.trim(),meta:`${r} korttia`}),e}function y(t){let e=f.findIndex(s=>s.id===t);return e>=0?e:0}function fe(){let e=(g?.meta||{}).title||"Oppitunti";return`
    ...
    <aside class="dk__sidemenu" id="dk-sidemenu" aria-label="Oppitunnin sis\xE4llys">
      <div class="dk__sidemenu-top">
        ...
        <div class="dk__sidemenu-course">${d(ue(p.lang))} \xB7 ${d(pe(p.kurssiKey))}</div>
      </div>
    ...
    <p class="dk__teoria-p">T\xE4ll\xE4 oppitunnilla ei ole viel\xE4 opetusmateriaalia. Voit siirty\xE4 suoraan harjoituksiin sivuvalikosta.</p>`,a=s.length?`<aside class="dk__key-points" aria-label="Ydinpoiminnat">
       ...
       </aside>`:"";return n+a}function xt(t){let e=ut[t.kind]||ut.flashcards;return`
    <div class="dk__placeholder" data-kind="${d(t.kind)}">
      <p class="dk__placeholder-kind">${d(e.label)}</p>
      <p>${d(e.body)}</p>
    </div>`}var Y=new Map;function wt(t){let e=t?.testDef;return e?((Array.isArray(g?.phases)?g.phases:[])[e.sourcePhase]?.items||[]).slice(0,e.count).filter(a=>je.has(a.item_type)):[]}function St(t,e){let s=Y.get(t);return s||(s={submitted:!1,answers:e.map(n=>n.item_type==="mc"?null:n.item_type==="gap_fill"?new Array((String(n.sentence_template||"").match(/\{\d+\}/g)||[]).length).fill(""):""),results:e.map(()=>null),scoreCorrect:0},Y.set(t,s)),s}function It(t){let e=wt(t);if(e.length===0)return`
      <div class="dk__placeholder" data-kind="testi">
        <p>T\xE4ll\xE4 testill\xE4 ei ole viel\xE4 kohtia.</p>
      </div>`;let s=St(t.id,e),n=t.testDef?.label||t.title||"Testi",a=e.map((o,l)=>be(o,l,s)).join(""),i=s.submitted?$e(e,s):"",r=s.submitted?`<button type="button" class="dk__btn dk__btn--ghost" id="dk-testi-reset">Tee uudelleen</button>
    ...
    <section class="dk__testi" data-sivu="${d(t.id)}">
      ...
      <div class="dk__exercise-actions dk__testi-actions">${r}</div>
        ...
        <div class="dk__input-row">
         ...
         ${a?.correct?"":`<p class="dk__feedback-expected"><span>Oikea vastaus:</span> ${d(Wt(t)||"")}</p>`}
    ...
    <div class="dk__testi-summary" aria-live="polite">
    ...
    </div>`}function _t(){let t=y(p.sivuId),e=f[t],s=document.querySelector(".dk__content .dk__arvio");if(!s)return;let n=document.createElement("div");n.innerHTML=jt(e),s.replaceWith(n.firstElementChild),Tt()}function Tt(){let t=document.querySelector(".dk__arvio");if(!t)return;let e=t.dataset.sivu,s=Lt(e);t.querySelectorAll(".dk__arvio-btn").forEach(n=>{n.addEventListener("click",()=>{let a=n.dataset.statement,i=Number(n.dataset.value);s[a]=i,t.querySelector(`.dk__arvio-row[data-statement="${a}"]`)?.querySelectorAll(".dk__arvio-btn").forEach(l=>{let c=Number(l.dataset.value)===i;l.classList.toggle("is-chosen",c),l.setAttribute("aria-pressed",c?"true":"false")});let o=document.getElementById("dk-arvio-submit");if(o){let l=K.every(c=>Number.isInteger(s[c.id])&&s[c.id]>0);o.disabled=!l}})}),document.getElementById("dk-arvio-submit")?.addEventListener("click",()=>{let n={ratings:{...s},submittedAt:new Date().toISOString(),lang:p.lang,kurssiKey:p.kurssiKey,lessonIndex:p.lessonIndex};Et(n),x(e),_t(),q()&&A(`${S}/api/digikirja/itsearvio`,{method:"POST",headers:{"Content-Type":"application/json",...I()},body:JSON.stringify({lang:p.lang,kurssi:p.kurssiKey,lesson:p.lessonIndex,ratings:n.ratings})}).catch(()=>{})}),document.getElementById("dk-arvio-reset")?.addEventListener("click",()=>{try{localStorage.removeItem(tt())}catch{}J.delete(e),_t()}),document.getElementById("dk-arvio-back")?.addEventListener("click",()=>{location.hash=`#/oppimispolku/${p.lang}/${encodeURIComponent(p.kurssiKey)}`})}var G=new Map,L="know",Ct="again";function z(t){return(Array.isArray(t?.vocab)?t.vocab:[]).slice(0,yt)}function st(){return`${oe}:${p.lang}:${p.kurssiKey}:${p.lessonIndex}`}function nt(){try{let t=localStorage.getItem(st());return t?JSON.parse(t):{}}catch{return{}}}function Ae(t,e){try{let s=nt();s[t]=e,localStorage.setItem(st(),JSON.stringify(s))}catch{}}function Ee(){try{localStorage.removeItem(st())}catch{}}function Mt(t){let e=G.get(t);return e||(e={cardIndex:0,flipped:!1},G.set(t,e)),e}function j(t,e){return t?.es?`${e}:${t.es}`:`${e}`}function qt(t){let e=z(g);if(e.length===0)return'<div class="dk__placeholder"><p>T\xE4m\xE4n oppitunnin sanasto on tyhj\xE4.</p></div>';let s=nt(),n=e.filter((h,k)=>s[j(h,k)]===L).length;if(n===e.length)return`
      ...
      </section>`;let i=Mt(t.id),r=Math.min(i.cardIndex,e.length-1),l=Kt(e,s,r)[0]??r,c=e[l],u=j(c,l),_=s[u]||null;return`
    ...
    </section>`}function Kt(t,e,s){let n=[];for(let a=s;a<t.length;a++){let i=j(t[a],a);e[i]!==L&&n.push(a)}for(let a=0;a<s;a++){let i=j(t[a],a);e[i]!==L&&n.push(a)}return n}function Le(t,e,s,n){let a=t.gender?`<span class="dk__flashcard-tag">${d(t.gender==="m"?"Maskuliini":t.gender==="f"?"Feminiini":t.gender)}</span>`:"",i=n===Ct?'<span class="dk__flashcard-tag dk__flashcard-tag--again">Harjoittelussa</span>':n===L?'<span class="dk__flashcard-tag dk__flashcard-tag--know">Hallinnassa</span>':"";return`
    ...
    <div class="dk__exercise-actions dk__flash-actions">
    ...
    </div>`}function F(){let t=y(p.sivuId),e=f[t],s=document.querySelector(".dk__content .dk__flashpack");if(!s)return;let n=document.createElement("div");n.innerHTML=qt(e),s.replaceWith(n.firstElementChild),Rt()}function Rt(){let t=document.querySelector(".dk__flashpack");if(!t)return;let e=t.dataset.sivu;if(document.getElementById("dk-flash-reset")?.addEventListener("click",()=>{Ee(),G.delete(e),F()}),document.getElementById("dk-flash-next-sivu")?.addEventListener("click",()=>{let l=y(p.sivuId),c=f[l+1];c&&w(c.id)}),t.dataset.done==="true")return;let s=Mt(e),n=Number(t.dataset.index),a=z(g)[n];if(!a)return;let i=j(a,n),r=()=>{s.flipped=!s.flipped,F()};document.getElementById("dk-flashcard")?.addEventListener("click",r),document.getElementById("dk-flashcard")?.addEventListener("keydown",l=>{(l.key===" "||l.key==="Enter")&&(l.preventDefault(),r())}),document.getElementById("dk-flash-flip")?.addEventListener("click",r);let o=l=>{Ae(i,l),s.flipped=!1;let c=nt(),u=z(g);u.length>0&&u.every((k,m)=>c[j(k,m)]===L)&&x(e);let h=Kt(u,c,n+1<u.length?n+1:0);s.cardIndex=h[0]??n,F()};document.getElementById("dk-flash-again")?.addEventListener("click",()=>o(Ct)),document.getElementById("dk-flash-know")?.addEventListener("click",()=>o(L))}var kt=new Map,je=new Set(["mc","typed","gap_fill","translate"]),Te=new Set(["mc","typed","gap_fill","translate","match"]);function T(t){if(!t||t.kind!=="tehtava")return null;let e=/^phase-(\d+)$/.exec(t.id);if(!e)return null;let s=Number(e[1]);return(Array.isArray(g?.phases)?g.phases:[])[s]||null}function Ht(t,e){let s=kt.get(t);return s||(s={itemIndex:0,answered:new Array(e.length).fill(null),scoreCorrect:0,scoreTotal:0},kt.set(t,s)),s}function Ot(t){let e=T(t);if(!e)return xt(t);let s=Array.isArray(e.items)?e.items:[];if(s.length===0)return'<div class="dk__placeholder"><p>T\xE4ll\xE4 vaiheella ei ole teht\xE4vi\xE4.</p></div>';let n=s[0].item_type;if(n==="writing")return Pt(t,e,s);if(n==="reading_mc")return Dt(t,e,s);if(!Te.has(n))return`
      ...
      </div>`;let a=Ht(t.id,s),i=Math.min(a.itemIndex,s.length-1),r=s[i],o=a.answered[i];return`
    ...
    <div class="dk__input-row">
      ...
      <ul class="dk__match-results">${i}</ul>`}let n=ce(s.map(a=>a.right));return`
    ...
    <div class="dk" id="dk-root" data-sidemenu="open">
    ...
    </div>`}function R(t){let e=document.getElementById("dk-root");if(!e)return;e.dataset.sidemenu=t;let s=document.getElementById("dk-toggle-sidemenu");s&&s.setAttribute("aria-pressed",t===$?"true":"false")}function Ye(){let t=document.getElementById("dk-toggle-sidemenu"),e=document.getElementById("dk-sidemenu-backdrop");t?.addEventListener("click",()=>{let s=document.getElementById("dk-root");if(window.matchMedia("(max-width: 1023px)").matches){let a=s.dataset.sidemenu===E?$:E;R(a)}else{let a=s.dataset.sidemenu===$?E:$;R(a),le(a)}}),e?.addEventListener("click",()=>{R($)})}function w(t){if(!t||t===p.sivuId||f.findIndex(r=>r.id===t)<0)return;p.sivuId=t;let s=`#/oppitunti/${p.lang}/${encodeURIComponent(p.kurssiKey)}/${p.lessonIndex}/${encodeURIComponent(t)}`;location.hash!==s&&history.replaceState(null,"",s);let n=document.getElementById("dk-root");if(!n)return;let a=n.querySelector(".dk__sidemenu-list");a&&a.querySelectorAll(".dk__row").forEach(r=>{let o=r.dataset.sivu===t;r.classList.toggle("is-active",o),r.setAttribute("aria-current",o?"page":"false")});let i=n.querySelector(".dk__content");if(i){let r=document.createElement("div");r.innerHTML=Yt(),i.replaceWith(r.firstElementChild),Gt()}window.matchMedia("(max-width: 1023px)").matches&&R($),Jt(),$t(),Q(),document.getElementById("dk-content")?.focus({preventScroll:!1})}function Je(){document.getElementById("dk-sidemenu-list")?.addEventListener("click",t=>{let e=t.target.closest(".dk__row");e&&w(e.dataset.sivu)})}function Jt(){let t=document.getElementById("dk-sidemenu-list");if(!t)return;let e=t.querySelector(".dk__row.is-active");e&&requestAnimationFrame(()=>{try{e.scrollIntoView({block:"nearest",inline:"nearest",behavior:"auto"})}catch{let s=e.offsetTop-t.clientHeight/2+e.clientHeight/2;t.scrollTop=Math.max(0,s)}})}function Gt(){document.querySelectorAll(".dk__content .dk__prevnext-btn[data-sivu]").forEach(t=>{t.addEventListener("click",()=>w(t.dataset.sivu))}),Ut(),Rt(),At(),Tt(),Nt(),Ft()}function Ge(){bt=!0}async function ze(t={}){bt||Ge(),p.lang=t.lang||p.lang,p.kurssiKey=t.kurssiKey||p.kurssiKey,p.lessonIndex=Number(t.lessonIndex)||p.lessonIndex,p.sivuId=t.sivuId||p.sivuId||"teoria";let e=document.getElementById("screen-digikirja");if(!e)return;e.innerHTML=We(),rt("screen-digikirja"),import("./app-sidebarShell-O3IKFPA5.js").then(n=>n.setSidebarMode("book"));let s=`${p.lang}/${p.kurssiKey}/${p.lessonIndex}`;D=s;try{let[n]=await Promise.all([ke(p),he(p)]);if(D!==s)return;g=n,f=me(n),f.some(r=>r.id===p.sivuId)||(p.sivuId=f[0]?.id||"teoria");let i=window.matchMedia("(max-width: 1023px)").matches?$:de();e.innerHTML=`
      ...
      </div>`,R(i),Ye(),Je(),ge(),Gt(),Jt(),$t(),Q()}catch(n){if(D!==s)return;e.innerHTML=Ue(n)}}function ns(t){let e=/^#\/oppitunti\/([a-z]{2})\/([^/?#]+)\/(\d+)\/([^/?#]+)/i.exec(t||location.hash);return e?(ze({lang:e[1].toLowerCase(),kurssiKey:decodeURIComponent(e[2]),lessonIndex:Number(e[3]),sivuId:decodeURIComponent(e[4])}),!0):!1}export{Ge as initDigikirja,ze as showDigikirja,ns as tryRouteDigikirja};
//# sourceMappingURL=app-digikirja-YVX7UTU3.js.map
