import{a as C,c as T}from"./app-chunk-N47QBJIH.js";import"./app-chunk-BKOFN7BD.js";import{a as E,c as S,e as A,j as O}from"./app-chunk-NZWTLFMY.js";import{b as j}from"./app-chunk-3WC2U67L.js";function M(t,e){let s=t?.mastery_threshold;return s?typeof s[e]=="number"?s[e]:s.B??.7:.7}function B(t,e){let s=t?.skip_for_targets;return Array.isArray(s)&&s.includes(e)}function q(t,e,{minChars:s=0,maxChars:a=0}={}){if(!t||!e)return{stop(){}};let n=s||a?`${s}\u2013${a}`:"";function i(){let r=(t.value||"").length;e.textContent=n?`${r} / ${n} merkki\xE4`:`${r} merkki\xE4`,e.classList.toggle("is-met",s>0&&r>=s),e.classList.toggle("is-too-long",a>0&&r>a)}return t.addEventListener("input",i),i(),{stop(){t.removeEventListener("input",i)}}}function _(t){return Math.round((t||0)*6)}var y="lesson-runner-root",J="puheo:lessonProgress:",v=new Map;async function V(t){if(!t)return[];if(v.has(t))return v.get(t);try{let{API:e,isLoggedIn:s,authHeader:a,apiFetch:n}=await import("./app-api-54FCZYC2.js"),i=await n(`${e}/api/curriculum/${encodeURIComponent(t)}`,{headers:{...s()?a():{}}});if(!i.ok)return v.set(t,[]),[];let r=await i.json(),l=Array.isArray(r.lessons)?r.lessons.slice().sort((o,c)=>(o.sortOrder||0)-(c.sortOrder||0)):[];return v.set(t,l),l}catch{return[]}}function w(t){let e=Array.isArray(t.phases)?t.phases:[];if(e.length===0)return`<aside class="lr-toc" aria-label="Teht\xE4v\xE4t">
  ...
  </aside>`}function x(t,e){t.querySelectorAll(".lr-toc__item:not(.is-current)").forEach(s=>{let a=()=>{let n=Number(s.dataset.phase);!Number.isFinite(n)||n===e.currentPhaseIdx||(g(e),e.currentPhaseIdx=n,e.currentItemIdx=0,e.correctInPhase=0,e.answeredInPhase=0,g(e),b(t,e))};s.addEventListener("click",a),s.addEventListener("keydown",n=>{(n.key==="Enter"||n.key===" ")&&(n.preventDefault(),a())})})}function N(t){let e=t.lesson.meta||{},s=e.kurssi_title||e.course_title||e.course_key||"Kurssi",a=e.lesson_index||t.lessonIndex,n=e.title||"Oppitunti";return`<nav class="lr-breadcrumb" aria-label="Sijainti">
  ...
  </nav>`}function L(t,e){return`${J}${t}:${e}`}function g(t){if(!(!t||!t.kurssiKey||t.finished)&&!(t.currentPhaseIdx===0&&t.currentItemIdx===0&&t.answeredInPhase===0))try{sessionStorage.setItem(L(t.kurssiKey,t.lessonIndex),JSON.stringify({currentPhaseIdx:t.currentPhaseIdx,currentItemIdx:t.currentItemIdx,correctInPhase:t.correctInPhase,answeredInPhase:t.answeredInPhase,phaseResults:t.phaseResults,targetGrade:t.targetGrade,phaseSignature:t.phases.map(e=>`${e.phase_id}:${e.items.length}`).join("|"),savedAt:Date.now()}))}catch{}}function z(t,e,s){try{let a=sessionStorage.getItem(L(t,e));if(!a)return null;let n=JSON.parse(a);if(!n||typeof n!="object")return null;let i=s.phases.map(r=>`${r.phase_id}:${r.items.length}`).join("|");return n.phaseSignature!==i||n.savedAt&&Date.now()-n.savedAt>1440*60*1e3?null:n}catch{return null}}function K(t,e){try{sessionStorage.removeItem(L(t,e))}catch{}}function u(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function $(t){return String(t||"").toLowerCase().trim().normalize("NFD").replace(/[̀-ͯ]/g,"").replace(/\s+/g," ")}function H(t,e,s="es"){let a=e||[],n=$(t);if(n&&a.some(i=>$(i)===n))return{ok:!0,hint:null};for(let i of a){let r=C(t,i,s);if(r.ok)return r}return{ok:!1,hint:null}}function Y(t,e,s,a){let n=(t.phases||[]).filter(i=>!B(i,e));return{lesson:t,kurssiKey:s,lessonIndex:a,targetGrade:e,phases:n,currentPhaseIdx:0,currentItemIdx:0,correctInPhase:0,answeredInPhase:0,phaseResults:[],sidePanelOpen:!1,sidePanelOpenMs:0,sidePanelOpenedAt:0,startedAt:Date.now(),finished:!1}}function Ie(t,e,s,a){let n=t.pregenerated||t;W(),j("screen-lesson");let i=document.getElementById(y);if(!i)return;let r=Y(n,a||"B",e,s);try{sessionStorage.setItem("currentLesson",JSON.stringify({kurssiKey:e,lessonIndex:s,lessonFocus:n.meta?.title||"",lessonType:n.meta?.lesson_type||"",targetGrade:r.targetGrade,isPregenerated:!0}));let o=n.teaching||{},c=String(o.intro_md||"").trim(),d=Array.isArray(o.key_points)?o.key_points:[],p=[c,d.length?`## Avainkohdat
`+d.map(h=>`- ${h}`).join(`
`):""].filter(Boolean).join(`

`);p?sessionStorage.setItem("currentLessonTeachingMd",p):sessionStorage.removeItem("currentLessonTeachingMd")}catch{}r._courseLessons=[],V(e).then(o=>{r._courseLessons=o;let c=document.querySelector(".lr-shell .lr-toc");if(c&&c.parentElement){let d=document.createElement("div");d.innerHTML=w(r),c.parentElement.replaceChild(d.firstElementChild,c),x(document.getElementById(y),r)}});let l=z(e,s,r);if(l){r.currentPhaseIdx=l.currentPhaseIdx,r.currentItemIdx=l.currentItemIdx,r.correctInPhase=l.correctInPhase||0,r.answeredInPhase=l.answeredInPhase||0,r.phaseResults=Array.isArray(l.phaseResults)?l.phaseResults:[],r.startedAt=Date.now(),b(i,r);return}G(i,r)}function W(){let t=document.getElementById("screen-lesson");t||(t=document.createElement("div"),t.id="screen-lesson",t.className="screen",document.querySelector(".app-main")?.appendChild(t)||document.body.appendChild(t)),document.getElementById(y)||(t.innerHTML=`<div id="${y}"></div>`)}function G(t,e){let s=e.lesson.meta||{},a=e.lesson.teaching||{},n=D(a.intro_md||""),i=Array.isArray(a.key_points)?a.key_points:[],r=i.length?`<ul class="lr-keypoints">${i.map(l=>`<li>${u(l)}</li>`).join("")}</ul>`:"";t.innerHTML=`
    ...
    </div>`,x(t,e),document.getElementById("lr-back")?.addEventListener("click",()=>{import("./app-curriculum-76KX7QYP.js").then(l=>l.loadCurriculum())}),document.getElementById("lr-start")?.addEventListener("click",()=>{e.startedAt=Date.now(),b(t,e)})}function b(t,e){let s=e.phases[e.currentPhaseIdx];if(!s)return R(t,e);let a=e.phases.length,n=U(e),i=s.items[e.currentItemIdx];if(!i)return P(t,e,s,"completed");t.innerHTML=`
    <div class="lr-shell lr-shell--exercise lr-shell--three">
      ...
      <div class="lr-item" id="lr-item">${X(i,e)}</div>
      ${ue(e)}
    </div>`,re(t,e,i),x(t,e)}function U(t){return`<span class="lr-stepper" aria-hidden="true">${t.phases.map((s,a)=>a<t.currentPhaseIdx?'<span class="lr-step lr-step--done" aria-hidden="true">\u25CF</span>':a===t.currentPhaseIdx?'<span class="lr-step lr-step--current" aria-hidden="true">\u25CF</span>':'<span class="lr-step" aria-hidden="true">\u25CB</span>').join("")}</span>`}function X(t,e){switch(t.item_type){case"mc":return Q(t);case"typed":return F(t);case"translate":return Z(t);case"match":return ee(t);case"gap_fill":return te(t);case"writing":return se(t);case"reading_mc":return ne(t);default:return`<p class="lr-unsupported">Teht\xE4v\xE4tyyppi\xE4 "${u(t.item_type||"?")}" ei tueta, ohitetaan.</p>
    ...
    <div class="lr-typed">
      ...
      <div class="lr-feedback" id="lr-feedback" hidden></div>
    </div>`}function Z(t){return F({...t,prompt:t.source})}function ee(t){let e=(t.pairs||[]).map(a=>a.left),s=he((t.pairs||[]).map(a=>a.right));return`
    <div class="lr-match" data-match>
      ...
      <div class="lr-feedback" id="lr-feedback" hidden></div>
    </div>`}function te(t){let e=t.sentence_template||"",s=(t.answers||t.accepts||[]).reduce((r,l)=>Math.max(r,String(l||"").length),8),a=Math.min(Math.max(s,6),18),n=u(e).replace(/\{(\d+)\}/g,(r,l)=>`<input type="text" class="lr-gap-input" data-gap="${l}" size="${a}" autocomplete="off" autocapitalize="off" spellcheck="false" aria-label="T\xE4yt\xE4 kohta ${l}" />`),i=Array.isArray(t.word_bank)&&t.word_bank.length?`<div class="lr-gap-bank" role="group" aria-label="Sanapankki">
    ...
    <div class="lr-gap">
      ...
      <div class="lr-feedback" id="lr-feedback" hidden></div>
    </div>`}function se(t){let e=Number(t.min_chars)||_(t.min_words),s=Number(t.max_chars)||_(t.max_words);return`
    <div class="lr-writing">
      ...
      <div class="lr-feedback" id="lr-feedback" hidden></div>
    </div>`}function ne(t){let e=u(t.passage||"").replace(/\n/g,"<br>"),s=(t.questions||[]).map((a,n)=>`
  ...
  `).join("");return`
    <div class="lr-reading">
      ...
      <div class="lr-feedback" id="lr-feedback" hidden></div>
    </div>`}function re(t,e,s){t.__lrState=e,T(t,e.language),document.getElementById("lr-help-toggle")?.addEventListener("click",()=>pe(t,e)),document.getElementById("lr-skip")?.addEventListener("click",()=>oe(t,e)),document.getElementById("lr-exit-lesson")?.addEventListener("click",async()=>{let{confirmDialog:n}=await import("./app-confirmDialog-46K22AFY.js");g(e),await n({title:"Lopetetaanko oppitunti?",body:"Tallennamme kohdan johon p\xE4\xE4sit, voit jatkaa t\xE4\xE4lt\xE4 my\xF6hemmin.",confirmLabel:"L\xE4hde takaisin",cancelLabel:"Jatka oppituntia"})&&(await import("./app-curriculum-76KX7QYP.js")).loadCurriculum()}),t.querySelector("[data-lr-skip-item]")?.addEventListener("click",()=>I(t,e,!0));let a=typeof e.language=="string"&&e.language||"es";if(s.item_type==="mc")t.querySelectorAll(".lr-mc-choice").forEach(n=>{n.addEventListener("click",()=>{let i=Number(n.dataset.mcIdx),r=i===Number(s.correct_index);m(t,r,s.explanation||"",{hint:null,waitForClick:!r}),ce(t,s.correct_index,i),k(e,r),r&&f(t,e)})});else if(s.item_type==="typed"||s.item_type==="translate"){let n=document.getElementById("lr-typed-input"),i=document.getElementById("lr-typed-submit"),r=()=>{let l=s.accept||[],{ok:o,hint:c}=H(n.value,l,a),d=l[0]||"";m(t,o,o?"":`Oikea vastaus: ${d}`,{hint:c,waitForClick:!o}),k(e,o),o&&f(t,e)};i?.addEventListener("click",r),n?.addEventListener("keydown",l=>{l.key==="Enter"&&r()}),n?.focus()}else if(s.item_type==="match")ae(t,e,s);else if(s.item_type==="gap_fill"){let n=null,i=t.querySelectorAll(".lr-gap-input");i.forEach(r=>{r.addEventListener("focus",()=>{n=r})}),i[0]?.focus(),t.querySelectorAll(".lr-gap-chip").forEach(r=>{r.addEventListener("click",()=>{let l=r.dataset.word||r.textContent||"",o=n&&n.value===""?n:Array.from(i).find(c=>!c.value)||n||i[0];o&&(o.value=l,o.classList.add("is-filled"),r.classList.add("is-used"),(Array.from(i).find(d=>!d.value)||o).focus())})}),i.forEach(r=>{r.addEventListener("input",()=>{r.classList.toggle("is-filled",!!r.value),t.querySelectorAll(".lr-gap-chip.is-used").forEach(l=>{let o=l.dataset.word||l.textContent||"",c=Array.from(i).some(d=>d.value===o);l.classList.toggle("is-used",c)})})}),document.getElementById("lr-gap-submit")?.addEventListener("click",()=>{let r=!0,l=[],o=null;i.forEach((c,d)=>{let p=s.answers&&s.answers[d]||[],h=H(c.value,p,a);h.ok||(r=!1),h.hint&&!o&&(o=h.hint),l.push(p[0]||"?"),c.classList.toggle("is-wrong",!h.ok),c.classList.toggle("is-correct",h.ok)}),m(t,r,r?"":`Oikeat vastaukset: ${l.join(", ")}`,{hint:o,waitForClick:!r}),k(e,r),r&&f(t,e)})}else if(s.item_type==="writing"){let n=document.getElementById("lr-writing-input"),i=document.getElementById("lr-writing-char-counter"),r=Number(n?.dataset.minChars)||0,l=Number(n?.dataset.maxChars)||0;n&&i&&q(n,i,{minChars:r,maxChars:l}),document.getElementById("lr-writing-submit")?.addEventListener("click",()=>{let c=(n?.value||"").length,d=c>=r,p=d?"Kirjoituksesi on tallennettu.":`Merkkim\xE4\xE4r\xE4 on ${c}, tavoite v\xE4hint\xE4\xE4n ${r} merkki\xE4. Jatka kirjoitusta.`;m(t,d,p,{hint:null,waitForClick:!d}),k(e,d),d&&f(t,e)})}else s.item_type==="reading_mc"&&document.getElementById("lr-reading-submit")?.addEventListener("click",()=>{let n=0,i=(s.questions||[]).length;(s.questions||[]).forEach((l,o)=>{let c=t.querySelector(`input[name="lr-q-${o}"]:checked`);c&&Number(c.value)===Number(l.correct_index)&&n++});let r=n===i;m(t,r,r?"Kaikki oikein.":`${n}/${i} oikein.`),e.correctInPhase+=n/i,e.answeredInPhase+=1,f(t,e,!1)})}function ae(t,e,s){let a=null,n=new Set,i=(s.pairs||[]).length,r=0;t.querySelectorAll(".lr-match-cell").forEach(l=>{l.addEventListener("click",()=>{if(n.has(l))return;if(l.dataset.side==="left")t.querySelectorAll('.lr-match-cell[data-side="left"]').forEach(c=>c.classList.remove("is-active")),l.classList.add("is-active"),a=l;else if(a){let c=Number(a.dataset.idx),d=s.pairs[c]?.right,p=l.dataset.val;d&&p&&$(d)===$(p)?(a.classList.add("is-matched"),l.classList.add("is-matched"),n.add(a),n.add(l),a=null,r++,r===i&&(m(t,!0,"Kaikki parit oikein."),k(e,!0),f(t,e))):(l.classList.add("is-wrong"),setTimeout(()=>l.classList.remove("is-wrong"),600))}})})}var ie='<svg class="lr-feedback__icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 6 9 17l-5-5"/></svg>',le='<svg class="lr-feedback__icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>';function m(t,e,s,a={}){let n=t.querySelector("#lr-feedback");if(!n)return;let{hint:i=null,waitForClick:r=!1}=a;n.hidden=!1,n.className=`lr-feedback ${e?"is-correct lr-feedback--correct":"is-wrong lr-feedback--wrong"}`,n.setAttribute("role","status"),n.setAttribute("aria-live",e?"polite":"assertive");let l=e?s||"Hyvin meni!":s||"Melkein, yrit\xE4 uudelleen.",o=i?`<p class="lr-feedback__accent-hint">${u(i)}</p>`:"",c=r?'<button type="button" class="lr-feedback__next" id="lr-feedback-next">Seuraava \u2192</button>':"";n.innerHTML=`${e?ie:le}<div class="lr-feedback__body-wrap"><p class="lr-feedback__title ${e?"is-correct":"is-wrong"}">${e?"Oikein":"V\xE4\xE4rin"}</p><p class="lr-feedback__body">${u(l)}</p>${o}${c}</div>`,n.classList.remove("lr-feedback--animate"),n.offsetWidth,n.classList.add("lr-feedback--animate"),r&&n.querySelector("#lr-feedback-next")?.addEventListener("click",()=>{let p=t.__lrState;p&&I(t,p,!1)},{once:!0})}function ce(t,e,s){t.querySelectorAll(".lr-mc-choice").forEach(a=>{let n=Number(a.dataset.mcIdx);a.disabled=!0,n===e&&a.classList.add("is-correct"),n===s&&s!==e&&a.classList.add("is-wrong")})}function k(t,e){t.answeredInPhase+=1,e&&(t.correctInPhase+=1)}function f(t,e,s=!0){setTimeout(()=>I(t,e,!1),1100)}function I(t,e,s){let a=e.phases[e.currentPhaseIdx];if(e.currentItemIdx+=1,g(e),e.currentItemIdx>=a.items.length)return P(t,e,a,"completed");b(t,e)}function P(t,e,s,a){let n=e.answeredInPhase||s.items.length,i=Math.round(e.correctInPhase),r=n>0?e.correctInPhase/n:0,l=M(s,e.targetGrade),o=a==="skipped",c=!o&&r>=l;e.phaseResults.push({phaseId:s.phase_id,title:s.title,correct:i,total:n,pct:r,threshold:l,mastered:c,skipped:o});let d,p;o?(d="Vaihe ohitettu",p="Sanat palaavat kertaussessioon my\xF6hemmin, niit\xE4 ei j\xE4tet\xE4 unohduksiin."):c?(d="Hallitset t\xE4m\xE4n",p=`Sait ${i} / ${n} oikein, jatketaan seuraavaan vaiheeseen.`):r>=.5?(d="L\xE4hell\xE4, viel\xE4 yksi pyyhk\xE4isy",p=`${i} / ${n} oikein. Sanat joissa horjuit palaavat seuraavissa vaiheissa.`):(d="T\xE4m\xE4 kaipaa toistoa",p=`${i} / ${n} oikein. Et ole yksin, t\xE4m\xE4 rakenne vaatii toistoa, ei eri s\xE4\xE4nt\xF6\xE4.`);let h=e.currentPhaseIdx+1>=e.phases.length;t.innerHTML=`
    ...
    </div>`,document.getElementById("lr-next")?.addEventListener("click",()=>{e.currentPhaseIdx+=1,e.currentItemIdx=0,e.correctInPhase=0,e.answeredInPhase=0,e.currentPhaseIdx>=e.phases.length?R(t,e):(g(e),b(t,e))})}function oe(t,e){if(!window.confirm("Ohita t\xE4m\xE4 vaihe? Sanat palaavat kertaussessioon my\xF6hemmin."))return;let a=e.phases[e.currentPhaseIdx];P(t,e,a,"skipped")}function R(t,e){if(e.finished)return;e.finished=!0,K(e.kurssiKey,e.lessonIndex);let s=e.phaseResults.reduce((c,d)=>c+(d.skipped?0:d.correct),0),a=e.phaseResults.reduce((c,d)=>c+(d.skipped?0:d.total),0),n=Math.max(1,Math.round((Date.now()-e.startedAt)/6e4)),i=e.targetGrade,r=de(i,e.phaseResults),l=e.lesson.meta?.yo_relevance||"",o=e.phaseResults.map(c=>{let d=c.skipped?"lr-result-skipped":c.mastered?"lr-result-mastered":"lr-result-almost",p=c.skipped?"Ohitettu":c.mastered?"Hallitset":"L\xE4hell\xE4";return`<li class="lr-result-row ${d}">
