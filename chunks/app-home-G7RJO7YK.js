import{a as u,c as h,e as p,j as m}from"./app-chunk-NZWTLFMY.js";import{b as f}from"./app-chunk-3WC2U67L.js";var $=["pro","treeni","lifetime","trialing","active"];function g(e){let a=(e?.subscription_status||window._userProfile?.subscription_status||"").toLowerCase();return $.some(t=>a.includes(t))}var j=[{code:"es",label:"Espanja",flag:"\u{1F1EA}\u{1F1F8}"},{code:"fr",label:"Ranska",flag:"\u{1F1EB}\u{1F1F7}"},{code:"de",label:"Saksa",flag:"\u{1F1E9}\u{1F1EA}"}],w="puheo:enabled-langs";function b(){try{let e=localStorage.getItem(w);if(e){let a=JSON.parse(e);if(Array.isArray(a)&&a.length)return a}}catch{}return["es"]}var k="puheo:lang",c=null;function r(e){return String(e??"").replace(/[&<>"']/g,a=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[a])}function A(){let e=b();try{let a=localStorage.getItem(k);if(a&&e.includes(a))return a}catch{}return e[0]||"es"}function L(e){try{localStorage.setItem(k,e)}catch{}}async function S(){if(c&&Date.now()-c.ts<6e4)return c.payload;try{let e=await m(`${u}/api/dashboard/v2`,{headers:{...h()?p():{}}});if(!e.ok)return null;let a=await e.json();return c={ts:Date.now(),payload:a},a}catch{return null}}function E(e){let a=b(),t=j.filter(o=>a.includes(o.code));return t.length<2?"":t.map(o=>`
    <button type="button"
  ...
  `).join("")}function d({label:e,value:a,locked:t}){return`
    ...
    <section class="ohjaamo" aria-label="Ohjaamo">
      ...
      ${l}
    </section>`}var v=[{id:"path",title:"Oppimispolku",body:"Vaiheittainen kurssi sanasto- ja kielioppiteht\xE4vineen.",chip:"8 kurssia \xB7 80 oppituntia",freeBadge:"Yksi oppitunti per p\xE4iv\xE4",locked:!1,target:e=>`#/oppimispolku?lang=${e}`},{id:"writing",title:"Kirjoitusteht\xE4v\xE4",body:"AI arvioi tuotoksesi YTL-rubriikilla ja antaa konkreettiset korjaukset.",chip:"Lyhyt + pitk\xE4",freeBadge:"3 teht\xE4v\xE4\xE4 per kuukausi",locked:!1,target:e=>`#/kirjoitus?lang=${e}`},{id:"reading",title:"Luetun ymm\xE4rt\xE4minen",body:"Aitoja YO-tyylisi\xE4 tekstej\xE4 monivalintateht\xE4vineen.",chip:"180 teksti\xE4",freeBadge:"5 teksti\xE4 per p\xE4iv\xE4",locked:!1,target:e=>`#/luetun?lang=${e}`},{id:"exam",title:"Koeharjoitus",body:"Koko YO-koe simulaationa. AI-arvio kaikkiin osa-alueisiin.",chip:"T\xE4ysi YO-simulaatio",freeBadge:"Avaa Treeni",locked:!0,target:e=>`#/koeharjoitus?lang=${e}`}];function O(e,a,t){let o=e.locked&&!t,i=["home-mode",`home-mode--${e.id}`,o?"is-locked":""].filter(Boolean).join(" "),s=t?"":`<span class="home-mode__badge ${o?"is-lock":""}">${o?"\u{1F512} ":""}${r(e.freeBadge)}</span>`;return`
    <a class="${i}" href="${e.target(a)}" data-mode="${e.id}" ${o?'aria-disabled="true"':""}>
      ...
      ${s}
    </a>`}function P(e,a,t){let o=E(e),i=v.map(_=>O(_,e,t)).join(""),s=a?.profile?.profile?.preferred_name||a?.profile?.profile?.full_name||window._userProfile?.preferred_name||"",n=new Date().getHours(),l=n<11?"Huomenta":n<18?"P\xE4iv\xE4\xE4":"Iltaa",y=s?`${l}, ${s}.`:`${l}.`;return`
