...
`)}var $e="puheo:dk:sidemenu",T="open",A="collapsed",dt="puheo:dk:progress";function xe(){return`${dt}:${p.lang}:${p.kurssiKey}:${p.lessonIndex}`}function B(){try{return JSON.parse(localStorage.getItem(xe())||"{}")}catch{return{}}}function we(e){try{localStorage.setItem(xe(),JSON.stringify(e))}catch{}}function E(e){if(!e)return;pt(e);let t=B();t[e]!=="done"&&(t[e]="done",we(t),b()&&w(`${$}/api/digikirja/progress`,{method:"POST",headers:{"Content-Type":"application/json",...x()},body:JSON.stringify({lang:p.lang,kurssi:p.kurssiKey,lesson:p.lessonIndex,sivuId:e})}).catch(()=>{}),se())}function se(){let e=B(),t=document.getElementById("dk-sidemenu-list");t&&t.querySelectorAll(".dk__row").forEach(n=>{n.classList.toggle("is-done",e[n.dataset.sivu]==="done")});let s=document.getElementById("dk-progress-chip");if(s){let n=f.filter(a=>e[a.id]==="done").length;s.textContent=`${n} / ${f.length} valmis`,s.dataset.full=n>=f.length?"true":"false",s.style.setProperty("--dk-prog",f.length?String(n/f.length):"0")}}var me="",W=null;async function lt(e){try{let t=await w(`${$}/api/curriculum/review-queue?lang=${encodeURIComponent(e||"es")}&limit=8`,{headers:{...x()}});if(!t||!t.ok)return null;let s=await t.json();return s&&!s.locked&&Array.isArray(s.items)&&s.items.length?s:null}catch{return null}}async function ut(e,t){try{if(typeof b=="function"&&!b())return;if(me!==t){me=t;let s=await Promise.race([lt(p.lang),new Promise(i=>setTimeout(()=>i(null),1500))]),n=new Set(["mc","typed","gap_fill","translate"]),a=(s&&Array.isArray(s.items)?s.items:[]).filter(i=>n.has(i.item_type));W=a.length?le({...s,items:a}):null}W&&(e.phases=[W,...Array.isArray(e.phases)?e.phases:[]])}catch{}}function pt(e){if(!b||!b())return;let t=f.find(r=>r.id===e);if(!t||t.kind!=="tehtava")return;let s=L(t),n=Q.get(e);if(!s||!n||!Array.isArray(n.answered))return;let a=Array.isArray(s.items)?s.items:[],i=[];for(let r=0;r<a.length;r++){let o=n.answered[r];!a[r]||!o||i.push(ue(a[r],o,s))}i.length&&w(`${$}/api/curriculum/capture`,{method:"POST",headers:{"Content-Type":"application/json",...x()},body:JSON.stringify({lang:p.lang||"es",gradedItems:i})}).catch(()=>{})}var _t=[[/^YO-tyylinen käännös$/i,"K\xE4\xE4nn\xE4 lauseet"],[/^YO-tason käännös$/i,"K\xE4\xE4nn\xE4 lauseet"],[/^Lyhyt kirjoitus$/i,"Kirjoita lyhyt teksti"],[/^Lauseiden rakentelua$/i,"K\xE4\xE4nn\xE4 lauseet"],[/^Lauseen rakentelua$/i,"K\xE4\xE4nn\xE4 lauseet"],[/Lauseen täydennys\s*[—-]\s*/g,"Lauseen t\xE4ydennys, "],[/Tunnista\s*[—-]\s*/g,"Tunnista "],[/Tuota\s*[—-]\s*/g,"Tuota "],[/Yhdistä\s*[—-]\s*/g,"Yhdist\xE4 "],[/Käännä\s*[—-]\s*/g,"K\xE4\xE4nn\xE4 "]];function kt(e){if(!e)return"";let t=String(e);for(let[s,n]of _t)t=t.replace(s,n);return t=t.replace(/\s+—\s+/g,", "),t}var ht={teoria:"Opetus",tehtava:"Harjoitukset",flashcards:"Kortit",testi:"Testit",itsearviointi:"Itsearvio"};var Se=5,mt="puheo:dk:flashcards",fe={},Ie=!1,p={lang:"es",kurssiKey:"kurssi_2",lessonIndex:3,sivuId:"teoria"},v=null,f=[],H="";function d(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function ft(e){let t=[...e];for(let s=t.length-1;s>0;s--){let n=Math.floor(Math.random()*(s+1));[t[s],t[n]]=[t[n],t[s]]}return t}function gt(){try{return localStorage.getItem($e)===T?T:A}catch{return T}}function vt(e){try{localStorage.setItem($e,e)}catch{}}function yt(e){return e==="fr"?"Ranska":e==="de"?"Saksa":"Espanja"}function Ae(e){let t=/^kurssi_(\d+)$/.exec(String(e||""));return t?`Kurssi ${t[1]}`:e||""}function bt(e){return`/data/courses/${encodeURIComponent(e.lang)}/${encodeURIComponent(e.kurssiKey)}/lesson_${encodeURIComponent(e.lessonIndex)}.json`}async function $t(e){let t=bt(e),s=await fetch(t,{headers:{accept:"application/json"}});if(!s.ok)throw new Error(`lesson fetch ${s.status}`);return s.json()}async function xt(e){if(b())try{let t=`lang=${encodeURIComponent(e.lang)}&kurssi=${encodeURIComponent(e.kurssiKey)}&lesson=${encodeURIComponent(e.lessonIndex)}`,[s,n]=await Promise.all([w(`${$}/api/digikirja/progress?${t}`,{headers:x()}).catch(()=>null),w(`${$}/api/digikirja/itsearvio?${t}`,{headers:x()}).catch(()=>null)]);if(s?.ok){let{sivut:a}=await s.json().catch(()=>({sivut:{}}));if(a&&typeof a=="object"){let i=B();for(let r of Object.keys(a))i[r]||(i[r]="done");we(i)}}if(n?.ok){let{itsearvio:a}=await n.json().catch(()=>({itsearvio:null}));a?.ratings&&(ae()||qe({ratings:a.ratings,submittedAt:a.submittedAt,lang:e.lang,kurssiKey:e.kurssiKey,lessonIndex:e.lessonIndex}))}}catch{}}function wt(e){let t=[];t.push({id:"teoria",kind:"teoria",num:"",title:e?.meta?.title||"Opetus",meta:"Opetussivu"}),(Array.isArray(e?.phases)?e.phases:[]).forEach((o,l)=>{let c=String(l+1),u=kt(o.title)||`Vaihe ${c}`,_=Array.isArray(o.items)?o.items.length:0;t.push({id:`phase-${l}`,kind:"tehtava",num:c,title:u,meta:_?`${_} kohtaa`:"Teht\xE4v\xE4"})});let n=e?.meta?.lesson_type||"vocab",a=new Set(["vocab","mixed"]),i=Array.isArray(e?.vocab)?e.vocab:[],r=Math.min(i.length,Se);return r>0&&a.has(n)&&t.push({id:"kortit-1",kind:"flashcards",num:"",title:`K\xE4\xE4nt\xF6kortit \xB7 ${e?.meta?.title||""}`.trim(),meta:`${r} korttia`}),t}function y(e){let t=f.findIndex(s=>s.id===e);return t>=0?t:0}function St(){let t=(v?.meta||{}).title||"Oppitunti";return`
    ...
    <section class="dk__testi" data-sivu="${d(e.id)}">
      ...
      <div class="dk__exercise-actions dk__testi-actions">${r}</div>
        ...
        <ol class="dk__choices" role="radiogroup" aria-label="Vaihtoehdot">
          ${(e.choices||[]).map((_,k)=>{let h=c===k,m=k===u,g=["dk__choice"];return n?h&&m?g.push("is-correct"):h&&!m?g.push("is-wrong"):m&&g.push("is-revealed"):h&&g.push("is-selected"),`
              <li>
        ...
        </div>`;break}case"gap_fill":{let c=String(e.sentence_template||""),u=n?a?.userAnswer||[]:s.answers[t]||[],_=0,k=d(c).replace(/\{(\d+)\}/g,()=>{let m=u[_]||"",g=`dk-testi-${t}-gap-${_}`,q=_;return _++,`<input id="${g}" type="text" class="dk__input dk__input--gap"
    ...
    <div class="dk__testi-summary" aria-live="polite">
    ...
    </div>`}function Tt(e,t){return e.map((n,a)=>{switch(n.item_type){case"mc":return t.answers[a];case"typed":case"translate":{let i=document.getElementById(`dk-testi-input-${a}`);return i?i.value:""}case"gap_fill":return[...document.querySelectorAll(`[data-testi-item="${a}"][data-testi-gap]`)].map(r=>r.value);default:return null}})}function ge(){let e=y(p.sivuId),t=f[e],s=document.querySelector(".dk__content .dk__testi");if(!s)return;let n=document.createElement("div");n.innerHTML=Ce(t),s.replaceWith(n.firstElementChild),Me()}function Me(){let e=document.querySelector(".dk__testi");if(!e)return;let t=e.dataset.sivu,s=f.find(i=>i.id===t),n=je(s),a=Te(t,n);e.querySelectorAll(".dk__choice[data-testi-item]").forEach(i=>{i.addEventListener("click",()=>{if(a.submitted)return;let r=Number(i.dataset.testiItem),o=Number(i.dataset.choice);a.answers[r]=o,e.querySelector(`.dk__testi-item[data-testi-item="${r}"]`)?.querySelectorAll(".dk__choice").forEach(c=>{c.classList.toggle("is-selected",Number(c.dataset.choice)===o)})})}),document.getElementById("dk-testi-submit")?.addEventListener("click",()=>{if(a.submitted)return;a.answers=Tt(n,a);let i=0;a.results=n.map((r,o)=>{let l=te(r,a.answers[o]);return l.correct&&i++,l}),a.scoreCorrect=i,a.submitted=!0,E(t),ge(),requestAnimationFrame(()=>{document.querySelector(".dk__testi-summary")?.scrollIntoView({block:"start",behavior:"smooth"})})}),document.getElementById("dk-testi-reset")?.addEventListener("click",()=>{G.delete(t),ge()}),document.getElementById("dk-testi-next-sivu")?.addEventListener("click",()=>{let i=y(p.sivuId),r=f[i+1];r&&j(r.id)})}var Ct="puheo:dk:itsearvio",K=[{id:"vocab",text:"Hallitsen t\xE4m\xE4n oppitunnin sanaston."},{id:"grammar",text:"Pystyn k\xE4ytt\xE4m\xE4\xE4n uutta kielioppia omissa lauseissani."},{id:"input",text:"Ymm\xE4rr\xE4n aiheen teksti\xE4 ja keskusteluja."},{id:"output",text:"Voin puhua ja kirjoittaa t\xE4st\xE4 aiheesta espanjaksi."}],Mt=["heikko","vajaa","kohtuu","vahva","hallitsen"];function ne(){return`${Ct}:${p.lang}:${p.kurssiKey}:${p.lessonIndex}`}function ae(){try{return JSON.parse(localStorage.getItem(ne())||"null")}catch{return null}}function qe(e){try{localStorage.setItem(ne(),JSON.stringify(e))}catch{}}var z=new Map;function Pe(e){let t=z.get(e);return t||(t={...ae()?.ratings||{}},z.set(e,t)),t}function Ke(e){let t=Pe(e.id),s=ae(),n=!!s,a=K.map(l=>{let c=n?s.ratings?.[l.id]??0:t[l.id]??0,u=[1,2,3,4,5].map(_=>`
      <button type="button"
      ...
      </div>`}).join(""),i=K.every(l=>Number.isInteger(t[l.id])&&t[l.id]>0),r=n?`<button type="button" class="dk__btn dk__btn--ghost" id="dk-arvio-reset">P\xE4ivit\xE4 arvio</button>
    ...
    </section>`}function qt(e){let t=e?.ratings||{},s=K.map(i=>t[i.id]).filter(Number.isInteger);if(s.length===0)return"";let n=s.reduce((i,r)=>i+r,0)/s.length,a=n>=4?"Olet vahvalla pohjalla.":n>=3?"Hyv\xE4, suuntaa ty\xF6 heikoimpiin kohtiin.":"Kannattaa kerrata oppitunti ennen seuraavaa.";return`
    <div class="dk__testi-summary" aria-live="polite">
      <div class="dk__testi-summary-score">
    ...
    </div>`}function ve(){let e=y(p.sivuId),t=f[e],s=document.querySelector(".dk__content .dk__arvio");if(!s)return;let n=document.createElement("div");n.innerHTML=Ke(t),s.replaceWith(n.firstElementChild),Re()}function Re(){let e=document.querySelector(".dk__arvio");if(!e)return;let t=e.dataset.sivu,s=Pe(t);e.querySelectorAll(".dk__arvio-btn").forEach(n=>{n.addEventListener("click",()=>{let a=n.dataset.statement,i=Number(n.dataset.value);s[a]=i,e.querySelector(`.dk__arvio-row[data-statement="${a}"]`)?.querySelectorAll(".dk__arvio-btn").forEach(l=>{let c=Number(l.dataset.value)===i;l.classList.toggle("is-chosen",c),l.setAttribute("aria-pressed",c?"true":"false")});let o=document.getElementById("dk-arvio-submit");if(o){let l=K.every(c=>Number.isInteger(s[c.id])&&s[c.id]>0);o.disabled=!l}})}),document.getElementById("dk-arvio-submit")?.addEventListener("click",()=>{let n={ratings:{...s},submittedAt:new Date().toISOString(),lang:p.lang,kurssiKey:p.kurssiKey,lessonIndex:p.lessonIndex};qe(n),E(t),ve(),b()&&w(`${$}/api/digikirja/itsearvio`,{method:"POST",headers:{"Content-Type":"application/json",...x()},body:JSON.stringify({lang:p.lang,kurssi:p.kurssiKey,lesson:p.lessonIndex,ratings:n.ratings})}).catch(()=>{})}),document.getElementById("dk-arvio-reset")?.addEventListener("click",()=>{try{localStorage.removeItem(ne())}catch{}z.delete(t),ve()}),document.getElementById("dk-arvio-back")?.addEventListener("click",()=>{location.hash=`#/oppimispolku/${p.lang}/${encodeURIComponent(p.kurssiKey)}`})}var X=new Map,C="know",Oe="again";function Z(e){return(Array.isArray(e?.vocab)?e.vocab:[]).slice(0,Se)}function ie(){return`${mt}:${p.lang}:${p.kurssiKey}:${p.lessonIndex}`}function re(){try{let e=localStorage.getItem(ie());return e?JSON.parse(e):{}}catch{return{}}}function Pt(e,t){try{let s=re();s[e]=t,localStorage.setItem(ie(),JSON.stringify(s))}catch{}}function Kt(){try{localStorage.removeItem(ie())}catch{}}function He(e){let t=X.get(e);return t||(t={cardIndex:0,flipped:!1},X.set(e,t)),t}function M(e,t){return e?.es?`${t}:${e.es}`:`${t}`}function Ne(e){let t=Z(v);if(t.length===0)return'<div class="dk__placeholder"><p>T\xE4m\xE4n oppitunnin sanasto on tyhj\xE4.</p></div>';let s=re(),n=t.filter((k,h)=>s[M(k,h)]===C).length;if(n===t.length)return`
...
//# sourceMappingURL=app-digikirja-G5SHLHHM.js.map
