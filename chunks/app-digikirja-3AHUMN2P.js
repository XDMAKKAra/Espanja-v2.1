...
`)}var me="puheo:dk:sidemenu",E="open",$="collapsed",nt="puheo:dk:progress";function fe(){return`${nt}:${p.lang}:${p.kurssiKey}:${p.lessonIndex}`}function N(){try{return JSON.parse(localStorage.getItem(fe())||"{}")}catch{return{}}}function ve(e){try{localStorage.setItem(fe(),JSON.stringify(e))}catch{}}function x(e){if(!e)return;let t=N();t[e]!=="done"&&(t[e]="done",ve(t),K()&&A(`${S}/api/digikirja/progress`,{method:"POST",headers:{"Content-Type":"application/json",...I()},body:JSON.stringify({lang:p.lang,kurssi:p.kurssiKey,lesson:p.lessonIndex,sivuId:e})}).catch(()=>{}),Q())}function Q(){let e=N(),t=document.getElementById("dk-sidemenu-list");t&&t.querySelectorAll(".dk__row").forEach(n=>{n.classList.toggle("is-done",e[n.dataset.sivu]==="done")});let s=document.getElementById("dk-progress-chip");if(s){let n=k.filter(i=>e[i.id]==="done").length;s.textContent=`${n} / ${k.length} valmis`,s.dataset.full=n>=k.length?"true":"false"}}var at=[[/^YO-tyylinen käännös$/i,"K\xE4\xE4nn\xE4 lauseet"],[/^YO-tason käännös$/i,"K\xE4\xE4nn\xE4 lauseet"],[/^Lyhyt kirjoitus$/i,"Kirjoita lyhyt teksti"],[/^Lauseiden rakentelua$/i,"K\xE4\xE4nn\xE4 lauseet"],[/^Lauseen rakentelua$/i,"K\xE4\xE4nn\xE4 lauseet"],[/Lauseen täydennys\s*[—-]\s*/g,"Lauseen t\xE4ydennys, "],[/Tunnista\s*[—-]\s*/g,"Tunnista "],[/Tuota\s*[—-]\s*/g,"Tuota "],[/Yhdistä\s*[—-]\s*/g,"Yhdist\xE4 "],[/Käännä\s*[—-]\s*/g,"K\xE4\xE4nn\xE4 "]];function it(e){if(!e)return"";let t=String(e);for(let[s,n]of at)t=t.replace(s,n);return t=t.replace(/\s+—\s+/g,", "),t}var rt={teoria:"Opetus",tehtava:"Harjoitukset",flashcards:"Kortit",testi:"Testit",itsearviointi:"Itsearvio"};var ge=5,ot="puheo:dk:flashcards",le={},ye=!1,p={lang:"es",kurssiKey:"kurssi_2",lessonIndex:3,sivuId:"teoria"},g=null,k=[],F="";function l(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function ct(){try{return localStorage.getItem(me)===E?E:$}catch{return E}}function dt(e){try{localStorage.setItem(me,e)}catch{}}function lt(e){return e==="fr"?"Ranska":e==="de"?"Saksa":"Espanja"}function ut(e){let t=/^kurssi_(\d+)$/.exec(String(e||""));return t?`Kurssi ${t[1]}`:e||""}function pt(e){return`/data/courses/${encodeURIComponent(e.lang)}/${encodeURIComponent(e.kurssiKey)}/lesson_${encodeURIComponent(e.lessonIndex)}.json`}async function _t(e){let t=pt(e),s=await fetch(t,{headers:{accept:"application/json"}});if(!s.ok)throw new Error(`lesson fetch ${s.status}`);return s.json()}async function kt(e){if(K())try{let t=`lang=${encodeURIComponent(e.lang)}&kurssi=${encodeURIComponent(e.kurssiKey)}&lesson=${encodeURIComponent(e.lessonIndex)}`,[s,n]=await Promise.all([A(`${S}/api/digikirja/progress?${t}`,{headers:I()}).catch(()=>null),A(`${S}/api/digikirja/itsearvio?${t}`,{headers:I()}).catch(()=>null)]);if(s?.ok){let{sivut:i}=await s.json().catch(()=>({sivut:{}}));if(i&&typeof i=="object"){let a=N();for(let r of Object.keys(i))a[r]||(a[r]="done");ve(a)}}if(n?.ok){let{itsearvio:i}=await n.json().catch(()=>({itsearvio:null}));i?.ratings&&(te()||Ae({ratings:i.ratings,submittedAt:i.submittedAt,lang:e.lang,kurssiKey:e.kurssiKey,lessonIndex:e.lessonIndex}))}}catch{}}function ht(e){let t=[];t.push({id:"teoria",kind:"teoria",num:"",title:e?.meta?.title||"Opetus",meta:"Opetussivu"}),(Array.isArray(e?.phases)?e.phases:[]).forEach((o,d)=>{let c=String(d+1),u=it(o.title)||`Vaihe ${c}`,_=Array.isArray(o.items)?o.items.length:0;t.push({id:`phase-${d}`,kind:"tehtava",num:c,title:u,meta:_?`${_} kohtaa`:"Teht\xE4v\xE4"})});let n=e?.meta?.lesson_type||"vocab",i=new Set(["vocab","mixed"]),a=Array.isArray(e?.vocab)?e.vocab:[],r=Math.min(a.length,ge);return r>0&&i.has(n)&&t.push({id:"kortit-1",kind:"flashcards",num:"",title:`K\xE4\xE4nt\xF6kortit \xB7 ${e?.meta?.title||""}`.trim(),meta:`${r} korttia`}),t}function y(e){let t=k.findIndex(s=>s.id===e);return t>=0?t:0}function mt(){let t=(g?.meta||{}).title||"Oppitunti";return`
    ...
    </header>`}function ft(){let e=[],t=null;for(let o of k){let d=rt[o.kind]||"Muut";d!==t&&(e.push({title:d,items:[]}),t=d),e[e.length-1].items.push(o)}let s=N(),n=e.map(o=>{let d=`<span class="dk__group-title">${l(o.title)}</span>`,c=o.items.map(u=>{let _=u.id===p.sivuId,f=s[u.id]==="done",h=["dk__row"];_&&h.push("is-active"),f&&h.push("is-done");let m=u.num?`${l(u.num)} `:"";return`
        <button type="button"
                class="${h.join(" ")}"
                data-sivu="${l(u.id)}"
                data-kind="${l(u.kind)}"
    ...
    <aside class="dk__sidemenu" id="dk-sidemenu" aria-label="Oppitunnin sis\xE4llys">
      <div class="dk__sidemenu-top">
        ...
        <div class="dk__sidemenu-course">${l(lt(p.lang))} \xB7 ${l(ut(p.kurssiKey))}</div>
      </div>
    ...
    </section>`}function yt(e,t,s){let n=s.submitted,i=s.results[t],a=e.item_type==="translate"?e.source||"":e.item_type==="typed"?e.prompt||"":e.item_type==="gap_fill"?null:e.stem||"",r=n?`<span class="dk__feedback-chip ${i?.correct?"is-correct":"is-wrong"}">${i?.correct?"Oikein":"Viel\xE4 ei"}</span>`:`<span class="dk__testi-itemnum">${t+1}</span>`,o="";switch(e.item_type){case"mc":{let c=n?i?.choiceIndex:s.answers[t]===null?-1:s.answers[t],u=e.correct_index;o=`
    ...
    </li>`}function bt(e,t){let s=e.length,n=t.scoreCorrect,i=s?Math.round(n/s*100):0,a=i>=80?"Hyvin meni.":i>=50?"Hyv\xE4 alku. Kertaa virheelliset kohdat.":"Kertaa viel\xE4 ja yrit\xE4 uudelleen.";return`
    <div class="dk__testi-summary" aria-live="polite">
    ...
    </div>`}function $t(e,t){return e.map((n,i)=>{switch(n.item_type){case"mc":return t.answers[i];case"typed":case"translate":{let a=document.getElementById(`dk-testi-input-${i}`);return a?a.value:""}case"gap_fill":return[...document.querySelectorAll(`[data-testi-item="${i}"][data-testi-gap]`)].map(r=>r.value);default:return null}})}function ue(){let e=y(p.sivuId),t=k[e],s=document.querySelector(".dk__content .dk__testi");if(!s)return;let n=document.createElement("div");n.innerHTML=Se(t),s.replaceWith(n.firstElementChild),Ie()}function Ie(){let e=document.querySelector(".dk__testi");if(!e)return;let t=e.dataset.sivu,s=k.find(a=>a.id===t),n=xe(s),i=we(t,n);e.querySelectorAll(".dk__choice[data-testi-item]").forEach(a=>{a.addEventListener("click",()=>{if(i.submitted)return;let r=Number(a.dataset.testiItem),o=Number(a.dataset.choice);i.answers[r]=o,e.querySelector(`.dk__testi-item[data-testi-item="${r}"]`)?.querySelectorAll(".dk__choice").forEach(c=>{c.classList.toggle("is-selected",Number(c.dataset.choice)===o)})})}),document.getElementById("dk-testi-submit")?.addEventListener("click",()=>{if(i.submitted)return;i.answers=$t(n,i);let a=0;i.results=n.map((r,o)=>{let d=Z(r,i.answers[o]);return d.correct&&a++,d}),i.scoreCorrect=a,i.submitted=!0,x(t),ue(),requestAnimationFrame(()=>{document.querySelector(".dk__testi-summary")?.scrollIntoView({block:"start",behavior:"smooth"})})}),document.getElementById("dk-testi-reset")?.addEventListener("click",()=>{Y.delete(t),ue()}),document.getElementById("dk-testi-next-sivu")?.addEventListener("click",()=>{let a=y(p.sivuId),r=k[a+1];r&&w(r.id)})}var xt="puheo:dk:itsearvio",q=[{id:"vocab",text:"Hallitsen t\xE4m\xE4n oppitunnin sanaston."},{id:"grammar",text:"Pystyn k\xE4ytt\xE4m\xE4\xE4n uutta kielioppia omissa lauseissani."},{id:"input",text:"Ymm\xE4rr\xE4n aiheen teksti\xE4 ja keskusteluja."},{id:"output",text:"Voin puhua ja kirjoittaa t\xE4st\xE4 aiheesta espanjaksi."}],wt=["heikko","vajaa","kohtuu","vahva","hallitsen"];function ee(){return`${xt}:${p.lang}:${p.kurssiKey}:${p.lessonIndex}`}function te(){try{return JSON.parse(localStorage.getItem(ee())||"null")}catch{return null}}function Ae(e){try{localStorage.setItem(ee(),JSON.stringify(e))}catch{}}var J=new Map;function Ee(e){let t=J.get(e);return t||(t={...te()?.ratings||{}},J.set(e,t)),t}function Le(e){let t=Ee(e.id),s=te(),n=!!s,i=q.map(d=>{let c=n?s.ratings?.[d.id]??0:t[d.id]??0,u=[1,2,3,4,5].map(_=>`
      <button type="button"
              class="dk__arvio-btn ${c===_?"is-chosen":""}"
              data-statement="${l(d.id)}"
              data-value="${_}"
              aria-pressed="${c===_}"
              aria-label="${_}, ${l(wt[_-1])}"
              ${n?"disabled":""}>
      ...
      <div class="dk__arvio-row" data-statement="${l(d.id)}">
       ...
       <button type="button" class="dk__btn dk__btn--primary" id="dk-arvio-back">Takaisin oppimispolulle</button>`:`<button type="button" class="dk__btn dk__btn--primary" id="dk-arvio-submit" ${a?"":"disabled"}>Tallenna arvio</button>`,o=n?St(s):"";return`
    ...
    </section>`}function St(e){let t=e?.ratings||{},s=q.map(a=>t[a.id]).filter(Number.isInteger);if(s.length===0)return"";let n=s.reduce((a,r)=>a+r,0)/s.length,i=n>=4?"Olet vahvalla pohjalla.":n>=3?"Hyv\xE4, suuntaa ty\xF6 heikoimpiin kohtiin.":"Kannattaa kerrata oppitunti ennen seuraavaa.";return`
    <div class="dk__testi-summary" aria-live="polite">
      <div class="dk__testi-summary-score">
    ...
    </div>`}function pe(){let e=y(p.sivuId),t=k[e],s=document.querySelector(".dk__content .dk__arvio");if(!s)return;let n=document.createElement("div");n.innerHTML=Le(t),s.replaceWith(n.firstElementChild),je()}function je(){let e=document.querySelector(".dk__arvio");if(!e)return;let t=e.dataset.sivu,s=Ee(t);e.querySelectorAll(".dk__arvio-btn").forEach(n=>{n.addEventListener("click",()=>{let i=n.dataset.statement,a=Number(n.dataset.value);s[i]=a,e.querySelector(`.dk__arvio-row[data-statement="${i}"]`)?.querySelectorAll(".dk__arvio-btn").forEach(d=>{let c=Number(d.dataset.value)===a;d.classList.toggle("is-chosen",c),d.setAttribute("aria-pressed",c?"true":"false")});let o=document.getElementById("dk-arvio-submit");if(o){let d=q.every(c=>Number.isInteger(s[c.id])&&s[c.id]>0);o.disabled=!d}})}),document.getElementById("dk-arvio-submit")?.addEventListener("click",()=>{let n={ratings:{...s},submittedAt:new Date().toISOString(),lang:p.lang,kurssiKey:p.kurssiKey,lessonIndex:p.lessonIndex};Ae(n),x(t),pe(),K()&&A(`${S}/api/digikirja/itsearvio`,{method:"POST",headers:{"Content-Type":"application/json",...I()},body:JSON.stringify({lang:p.lang,kurssi:p.kurssiKey,lesson:p.lessonIndex,ratings:n.ratings})}).catch(()=>{})}),document.getElementById("dk-arvio-reset")?.addEventListener("click",()=>{try{localStorage.removeItem(ee())}catch{}J.delete(t),pe()}),document.getElementById("dk-arvio-back")?.addEventListener("click",()=>{location.hash=`#/oppimispolku/${p.lang}/${encodeURIComponent(p.kurssiKey)}`})}var G=new Map,L="know",Te="again";function z(e){return(Array.isArray(e?.vocab)?e.vocab:[]).slice(0,ge)}function se(){return`${ot}:${p.lang}:${p.kurssiKey}:${p.lessonIndex}`}function ne(){try{let e=localStorage.getItem(se());return e?JSON.parse(e):{}}catch{return{}}}function It(e,t){try{let s=ne();s[e]=t,localStorage.setItem(se(),JSON.stringify(s))}catch{}}function At(){try{localStorage.removeItem(se())}catch{}}function Ce(e){let t=G.get(e);return t||(t={cardIndex:0,flipped:!1},G.set(e,t)),t}function j(e,t){return e?.es?`${t}:${e.es}`:`${t}`}function Me(e){let t=z(g);if(t.length===0)return'<div class="dk__placeholder"><p>T\xE4m\xE4n oppitunnin sanasto on tyhj\xE4.</p></div>';let s=ne(),n=t.filter((f,h)=>s[j(f,h)]===L).length;if(n===t.length)return`
    ...
    </div>`}function D(){let e=y(p.sivuId),t=k[e],s=document.querySelector(".dk__content .dk__flashpack");if(!s)return;let n=document.createElement("div");n.innerHTML=Me(t),s.replaceWith(n.firstElementChild),qe()}function qe(){let e=document.querySelector(".dk__flashpack");if(!e)return;let t=e.dataset.sivu;if(document.getElementById("dk-flash-reset")?.addEventListener("click",()=>{At(),G.delete(t),D()}),document.getElementById("dk-flash-next-sivu")?.addEventListener("click",()=>{let d=y(p.sivuId),c=k[d+1];c&&w(c.id)}),e.dataset.done==="true")return;let s=Ce(t),n=Number(e.dataset.index),i=z(g)[n];if(!i)return;let a=j(i,n),r=()=>{s.flipped=!s.flipped,D()};document.getElementById("dk-flashcard")?.addEventListener("click",r),document.getElementById("dk-flashcard")?.addEventListener("keydown",d=>{(d.key===" "||d.key==="Enter")&&(d.preventDefault(),r())}),document.getElementById("dk-flash-flip")?.addEventListener("click",r);let o=d=>{It(a,d),s.flipped=!1;let c=ne(),u=z(g);u.length>0&&u.every((h,m)=>c[j(h,m)]===L)&&x(t);let f=Ke(u,c,n+1<u.length?n+1:0);s.cardIndex=f[0]??n,D()};document.getElementById("dk-flash-again")?.addEventListener("click",()=>o(Te)),document.getElementById("dk-flash-know")?.addEventListener("click",()=>o(L))}var _e=new Map,He=new Set(["mc","typed","gap_fill","translate"]);function T(e){if(!e||e.kind!=="tehtava")return null;let t=/^phase-(\d+)$/.exec(e.id);if(!t)return null;let s=Number(t[1]);return(Array.isArray(g?.phases)?g.phases:[])[s]||null}function Oe(e,t){let s=_e.get(e);return s||(s={itemIndex:0,answered:new Array(t.length).fill(null),scoreCorrect:0,scoreTotal:0},_e.set(e,s)),s}function Be(e){let t=T(e);if(!t)return $e(e);let s=Array.isArray(t.items)?t.items:[];if(s.length===0)return'<div class="dk__placeholder"><p>T\xE4ll\xE4 vaiheella ei ole teht\xE4vi\xE4.</p></div>';let n=s[0].item_type;if(n==="writing")return Pe(e,t,s);if(n==="reading_mc")return Fe(e,t,s);if(!He.has(n)){let d=n==="match"?"Yhdist\xE4misteht\xE4v\xE4":`Teht\xE4v\xE4tyyppi "${n}"`;return`
    ...
    <section class="dk__exercise" data-sivu="${l(e.id)}" data-index="${a}">
      ...
      ${Ct(r,o)}
      ${Ot(r,o,a,s.length)}
    </section>`}var ke=new Map;function Ne(e){let t=ke.get(e);return t||(t={promptIdx:0,text:"",submitted:!1,result:null,loading:!1,error:null},ke.set(e,t)),t}function Pe(e,t,s){let n=Ne(e.id),i=s[Math.min(n.promptIdx,s.length-1)]||s[0],a=i.min_words||50,r=i.max_words||120,o=t.instruction||"",d=s.length>1&&!n.submitted?`<div class="dk__writing-switcher" role="tablist" aria-label="Valitse aihe">
         ${s.map((u,_)=>`
           <button type="button"
       ...
       </div>`:"",c=n.submitted&&n.result?jt(n):Lt(i,n,a,r,o);return`
    ...
    <div class="dk__writing-result">
      ...
      <div class="dk__reading-q" data-qi="${v}">
        ...
        ${o&&m.explanation_fi?`<p class="dk__reading-expl ${ae===P?"is-correct":"is-wrong"}">${l(m.explanation_fi)}</p>`:""}
    ...
    </ol>`}function Kt(e,t){let s=e.hint||"",n=t?.userAnswer||"",i=!!t;return`
    ...
    <div class="dk__input-row">
      ...
      <input id="dk-input" type="text" class="dk__input"
             ...
             value="${l(n)}"
             ${i?"disabled":""}>
    </div>`}function qt(e,t){let s=String(e.sentence_template||""),n=(s.match(/\{(\d+)\}/g)||[]).length,i=t?.userAnswer||new Array(n).fill(""),a=!!t,r=0,o=l(s).replace(/\{(\d+)\}/g,()=>{let c=i[r]||"",u=`dk-gap-${r}`;return r++,`<input id="${u}" type="text" class="dk__input dk__input--gap"
    ...
    ${d}`}function Ht(e,t){let s=e.direction==="es_to_fi"?"espanjasta suomeksi":e.direction==="fi_to_es"?"suomesta espanjaksi":"k\xE4\xE4nn\xF6s",n=t?.userAnswer||"",i=!!t;return`
    ...
    <main class="dk__content" id="dk-content" tabindex="-1">
      ...
      ${Pt(n,i,"bottom")}
       ...
       </button>`;return`<div class="${n}">${i}${a}</div>`}function Rt(){return`
    ...
    <div class="dk" id="dk-root" data-sidemenu="open">
    ...
    </div>`}function H(e){let t=document.getElementById("dk-root");if(!t)return;t.dataset.sidemenu=e;let s=document.getElementById("dk-toggle-sidemenu");s&&s.setAttribute("aria-pressed",e===$?"true":"false")}function Ft(){let e=document.getElementById("dk-toggle-sidemenu"),t=document.getElementById("dk-sidemenu-backdrop");e?.addEventListener("click",()=>{let s=document.getElementById("dk-root");if(window.matchMedia("(max-width: 1023px)").matches){let i=s.dataset.sidemenu===E?$:E;H(i)}else{let i=s.dataset.sidemenu===$?E:$;H(i),dt(i)}}),t?.addEventListener("click",()=>{H($)})}function w(e){if(!e||e===p.sivuId||k.findIndex(r=>r.id===e)<0)return;p.sivuId=e;let s=`#/oppitunti/${p.lang}/${encodeURIComponent(p.kurssiKey)}/${p.lessonIndex}/${encodeURIComponent(e)}`;location.hash!==s&&history.replaceState(null,"",s);let n=document.getElementById("dk-root");if(!n)return;let i=n.querySelector(".dk__sidemenu-list");i&&i.querySelectorAll(".dk__row").forEach(r=>{let o=r.dataset.sivu===e;r.classList.toggle("is-active",o),r.setAttribute("aria-current",o?"page":"false")});let a=n.querySelector(".dk__content");if(a){let r=document.createElement("div");r.innerHTML=Ye(),a.replaceWith(r.firstElementChild),Ge()}window.matchMedia("(max-width: 1023px)").matches&&H($),Je(),be(),Q(),document.getElementById("dk-content")?.focus({preventScroll:!1})}function Dt(){document.getElementById("dk-sidemenu-list")?.addEventListener("click",e=>{let t=e.target.closest(".dk__row");t&&w(t.dataset.sivu)})}function Je(){let e=document.getElementById("dk-sidemenu-list");if(!e)return;let t=e.querySelector(".dk__row.is-active");t&&requestAnimationFrame(()=>{try{t.scrollIntoView({block:"nearest",inline:"nearest",behavior:"auto"})}catch{let s=t.offsetTop-e.clientHeight/2+t.clientHeight/2;e.scrollTop=Math.max(0,s)}})}function Ge(){document.querySelectorAll(".dk__content .dk__prevnext-btn[data-sivu]").forEach(e=>{e.addEventListener("click",()=>w(e.dataset.sivu))}),Ue(),qe(),Ie(),je(),Re(),De()}function Wt(){ye=!0}async function Ut(e={}){ye||Wt(),p.lang=e.lang||p.lang,p.kurssiKey=e.kurssiKey||p.kurssiKey,p.lessonIndex=Number(e.lessonIndex)||p.lessonIndex,p.sivuId=e.sivuId||p.sivuId||"teoria";let t=document.getElementById("screen-digikirja");if(!t)return;t.innerHTML=Rt(),re("screen-digikirja"),import("./app-sidebarShell-O3IKFPA5.js").then(n=>n.setSidebarMode("book"));let s=`${p.lang}/${p.kurssiKey}/${p.lessonIndex}`;F=s;try{let[n]=await Promise.all([_t(p),kt(p)]);if(F!==s)return;g=n,k=ht(n),k.some(r=>r.id===p.sivuId)||(p.sivuId=k[0]?.id||"teoria");let a=window.matchMedia("(max-width: 1023px)").matches?$:ct();t.innerHTML=`
      ...
      </div>`,H(a),Ft(),Dt(),vt(),Ge(),Je(),be(),Q()}catch(n){if(F!==s)return;t.innerHTML=Vt(n)}}function Zt(e){let t=/^#\/oppitunti\/([a-z]{2})\/([^/?#]+)\/(\d+)\/([^/?#]+)/i.exec(e||location.hash);return t?(Ut({lang:t[1].toLowerCase(),kurssiKey:decodeURIComponent(t[2]),lessonIndex:Number(t[3]),sivuId:decodeURIComponent(t[4])}),!0):!1}export{Wt as initDigikirja,Ut as showDigikirja,Zt as tryRouteDigikirja};
//# sourceMappingURL=app-digikirja-3AHUMN2P.js.map
