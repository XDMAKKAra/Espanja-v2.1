import{a as c,c as p,e as u,j as d}from"./app-chunk-NZWTLFMY.js";import{b as m}from"./app-chunk-3WC2U67L.js";var f=new Map;function n(e){return String(e??"").replace(/[&<>"']/g,s=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[s])}function _(){let e=/lang=([a-z]{2})/i.exec(location.hash||"");if(e)return e[1].toLowerCase();try{return localStorage.getItem("puheo:lang")||"es"}catch{return"es"}}async function v(e){let s=f.get(e);if(s&&Date.now()-s.ts<6e4)return s.kurssit;try{let o=await d(`${c}/api/curriculum?lang=${encodeURIComponent(e)}`,{headers:{...p()?u():{}}});if(!o.ok)return[];let t=await o.json();if(t?.available===!1)return[];let r=Array.isArray(t?.kurssit)?t.kurssit.filter(a=>a&&typeof a.key=="string"):[];return f.set(e,{ts:Date.now(),kurssit:r}),r}catch{return[]}}function y(e,s,o){let t=!!e.kertausPassed,r=!e.isUnlocked,a=e.lessonsCompleted||0,i=e.lessonCount||10,l=Math.min(100,Math.round(a/i*100)),h=t?"Suoritettu":r?"Lukittu":a>0?`${a} / ${i} oppituntia`:"Aloita \u2192",b=["op-row",t?"is-done":"",r?"is-locked":"is-clickable",a>0&&!t?"is-progress":""].filter(Boolean).join(" "),k=r?"#":`#/oppimispolku/${o}/${encodeURIComponent(e.key)}`;return`
    ...
    </a>`}function $(e,s){let o=s.filter(a=>a.kertausPassed).length,t=s.length;return`
    ...
    </div>`}async function g(e){m("screen-oppimispolku-index");let s=document.getElementById("op-root");if(!s)return;let o=e||_();s.innerHTML='<div class="op-loading" role="status" aria-label="Ladataan kursseja"><span class="sr-only">Ladataan kursseja\u2026</span></div>';let t=await v(o);if(t.length===0){w(s,"Kursseja ei viel\xE4 julkaistu t\xE4lle kielelle.");return}s.innerHTML=$(o,t),s.querySelectorAll(".op-row.is-locked").forEach(r=>{r.addEventListener("click",a=>a.preventDefault())})}function I(e){return/^#\/oppimispolku(\?|$)/.test(e||"")?(g(),!0):!1}export{g as loadOppimispolkuIndex,I as tryRouteOppimispolkuIndex};
//# sourceMappingURL=app-oppimispolkuIndex-6QLRVGW4.js.map
