...
`)}var ae="puheo:dk:sidemenu",j="open",g="collapsed",Ne="puheo:dk:progress";function ie(){return`${Ne}:${u.lang}:${u.kurssiKey}:${u.lessonIndex}`}function B(){try{return JSON.parse(localStorage.getItem(ie())||"{}")}catch{return{}}}function re(e){try{localStorage.setItem(ie(),JSON.stringify(e))}catch{}}function x(e){if(!e)return;let t=B();t[e]!=="done"&&(t[e]="done",re(t),M()&&A(`${S}/api/digikirja/progress`,{method:"POST",headers:{"Content-Type":"application/json",...E()},body:JSON.stringify({lang:u.lang,kurssi:u.kurssiKey,lesson:u.lessonIndex,sivuId:e})}).catch(()=>{}),U())}function U(){let e=B(),t=document.getElementById("dk-sidemenu-list");t&&t.querySelectorAll(".dk__row").forEach(n=>{n.classList.toggle("is-done",e[n.dataset.sivu]==="done")});let s=document.getElementById("dk-progress-chip");if(s){let n=k.filter(a=>e[a.id]==="done").length;s.textContent=`${n} / ${k.length} valmis`,s.dataset.full=n>=k.length?"true":"false"}}var Pe={teoria:"Opetus",tehtava:"Harjoitukset",flashcards:"Kortit",testi:"Testit",itsearviointi:"Itsearvio"};var oe=5,Re="puheo:dk:flashcards",ee={},de=!1,u={lang:"es",kurssiKey:"kurssi_2",lessonIndex:3,sivuId:"teoria"},f=null,k=[],H="";function o(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function qe(){try{return localStorage.getItem(ae)===g?g:j}catch{return j}}function Ve(e){try{localStorage.setItem(ae,e)}catch{}}function Fe(e){return e==="fr"?"Ranska":e==="de"?"Saksa":"Espanja"}function De(e){return`/data/courses/${encodeURIComponent(e.lang)}/${encodeURIComponent(e.kurssiKey)}/lesson_${encodeURIComponent(e.lessonIndex)}.json`}async function Ue(e){let t=De(e),s=await fetch(t,{headers:{accept:"application/json"}});if(!s.ok)throw new Error(`lesson fetch ${s.status}`);return s.json()}async function Je(e){if(M())try{let t=`lang=${encodeURIComponent(e.lang)}&kurssi=${encodeURIComponent(e.kurssiKey)}&lesson=${encodeURIComponent(e.lessonIndex)}`,[s,n]=await Promise.all([A(`${S}/api/digikirja/progress?${t}`,{headers:E()}).catch(()=>null),A(`${S}/api/digikirja/itsearvio?${t}`,{headers:E()}).catch(()=>null)]);if(s?.ok){let{sivut:a}=await s.json().catch(()=>({sivut:{}}));if(a&&typeof a=="object"){let i=B();for(let d of Object.keys(a))i[d]||(i[d]="done");re(i)}}if(n?.ok){let{itsearvio:a}=await n.json().catch(()=>({itsearvio:null}));a?.ratings&&(Y()||he({ratings:a.ratings,submittedAt:a.submittedAt,lang:e.lang,kurssiKey:e.kurssiKey,lessonIndex:e.lessonIndex}))}}catch{}}function Ye(e){let t=[];t.push({id:"teoria",kind:"teoria",num:"",title:e?.meta?.title||"Opetus",meta:"Opetussivu"}),(Array.isArray(e?.phases)?e.phases:[]).forEach((i,d)=>{let c=String(d+1),l=i.title||`Vaihe ${c}`,r=Array.isArray(i.items)?i.items.length:0;t.push({id:`phase-${d}`,kind:"tehtava",num:c,title:l,meta:r?`${r} kohtaa`:"Teht\xE4v\xE4"})});let n=Array.isArray(e?.vocab)?e.vocab:[],a=Math.min(n.length,oe);return a>0&&t.push({id:"kortit-1",kind:"flashcards",num:"",title:`K\xE4\xE4nt\xF6kortit \xB7 ${e?.meta?.title||""}`.trim(),meta:`${a} korttia`}),t}function b(e){let t=k.findIndex(s=>s.id===e);return t>=0?t:0}function Ge(){let t=(f?.meta||{}).title||"Oppitunti";return`
    ...
    </header>`}function We(){let e=[],t=null;for(let d of k){let c=Pe[d.kind]||"Muut";c!==t&&(e.push({title:c,items:[]}),t=c),e[e.length-1].items.push(d)}let s=B(),n=e.map(d=>{let c=`<span class="dk__group-title">${o(d.title)}</span>`,l=d.items.map(r=>{let p=r.id===u.sivuId,_=s[r.id]==="done",h=["dk__row"];p&&h.push("is-active"),_&&h.push("is-done");let m=r.num?`${o(r.num)} `:"";return`
        <button type="button"
                class="${h.join(" ")}"
                data-sivu="${o(r.id)}"
                data-kind="${o(r.kind)}"
                aria-current="${p?"page":"false"}"
                aria-label="${o(r.title)}${_?", suoritettu":""}">
          <span class="dk__row-bullet" aria-hidden="true"></span>
          <span class="dk__row-title">${m}${o(r.title)}</span>
          ${_?'<span class="dk__row-check" aria-hidden="true">\u2713</span>':""}
        </button>`}).join("");return c+l}).join(""),i=typeof K=="function"&&K()||""||"Oma sivu";return`
    <aside class="dk__sidemenu" id="dk-sidemenu" aria-label="Oppitunnin sis\xE4llys">
      <div class="dk__sidemenu-top">
        ...
        <div class="dk__sidemenu-course">${o(Fe(u.lang))} \xB7 ${o(u.kurssiKey||"")}</div>
      </div>
    ...
    <div class="dk__placeholder" data-kind="${o(e.kind)}">
      ...
      </div>`;let s=pe(e.id,t),n=e.testDef?.label||e.title||"Testi",a=t.map((c,l)=>Ze(c,l,s)).join(""),i=s.submitted?Qe(t,s):"",d=s.submitted?`<button type="button" class="dk__btn dk__btn--ghost" id="dk-testi-reset">Tee uudelleen</button>
    ...
    <section class="dk__testi" data-sivu="${o(e.id)}">
      ...
      <div class="dk__exercise-actions dk__testi-actions">${d}</div>
        ...
        </ol>`;break}case"typed":case"translate":{let r=n?a?.userAnswer||"":s.answers[t]||"";c=`
        <p class="dk__exercise-stem dk__testi-stem">${o(i)}</p>
        <div class="dk__input-row">
          ...
          <${e.item_type==="translate"?`textarea rows="2" id="dk-testi-input-${t}" class="dk__input dk__input--multiline"`:`input id="dk-testi-input-${t}" type="text" class="dk__input"`}
        ...
        </div>`;break}case"gap_fill":{let r=String(e.sentence_template||""),p=n?a?.userAnswer||[]:s.answers[t]||[],_=0,h=o(r).replace(/\{(\d+)\}/g,()=>{let v=p[_]||"",$=`dk-testi-${t}-gap-${_}`,Me=_;return _++,`<input id="${$}" type="text" class="dk__input dk__input--gap"
           ...
           </ul>`:"";c=`<p class="dk__exercise-stem dk__exercise-stem--gap dk__testi-stem">${h}</p>${m}`;break}default:c=`<p>${o(i)}</p>`}let l=n?`<div class="dk__testi-reveal">
    ...
    <div class="dk__testi-summary" aria-live="polite">
      ...
      </div>`}).join(""),i=C.every(l=>Number.isInteger(t[l.id])&&t[l.id]>0),d=n?`<button type="button" class="dk__btn dk__btn--ghost" id="dk-arvio-reset">P\xE4ivit\xE4 arvio</button>
    ...
    <div class="dk__testi-summary" aria-live="polite">
      ...
      </section>`;let i=ye(e.id),d=Math.min(i.cardIndex,t.length-1),l=$e(t,s,d)[0]??d,r=t[l],p=I(r,l),_=s[p]||null;return`
    ...
    <div class="dk__input-row">
      ...
      <input id="dk-input" type="text" class="dk__input"
             ...
             value="${o(n)}"
             ${a?"disabled":""}>
    </div>`}function ut(e,t){let s=String(e.sentence_template||""),n=(s.match(/\{(\d+)\}/g)||[]).length,a=t?.userAnswer||new Array(n).fill(""),i=!!t,d=0,c=o(s).replace(/\{(\d+)\}/g,()=>{let r=a[d]||"",p=`dk-gap-${d}`;return d++,`<input id="${p}" type="text" class="dk__input dk__input--gap"
    ...
    <div class="dk__input-row">
      ...
      </div>`;let a=t.correct,i=a?"is-correct":"is-wrong",d=a?"Oikein":"Viel\xE4 ei aivan",c=kt(e,t),l=s>=n-1;return`
    ...
    </div>`}function kt(e,t){let s=Ae(e),n=s?`<p class="dk__feedback-expected"><span>Oikea vastaus:</span> ${o(s)}</p>`:"",a=e.explanation?`<p class="dk__feedback-text">${o(e.explanation)}</p>`:"",i=t?.accentHint?`<p class="dk__feedback-text dk__feedback-hint">${o(t.accentHint)}</p>`:"";return`${n}${i}${a}`}function Ae(e){switch(e.item_type){case"mc":return Array.isArray(e.choices)&&Number.isInteger(e.correct_index)?e.choices[e.correct_index]:"";case"typed":return Array.isArray(e.accept)&&e.accept[0]||"";case"translate":return Array.isArray(e.accept)&&e.accept[0]||"";case"gap_fill":{let t=String(e.sentence_template||""),s=Array.isArray(e.answers)?e.answers:[],n=0;return t.replace(/\{(\d+)\}/g,()=>{let a=s[n++];return Array.isArray(a)&&a[0]||"\u2014"})}default:return""}}function D(e,t){switch(e.item_type){case"mc":{let s=Number(t);return{correct:s===e.correct_index,choiceIndex:s}}case"typed":case"translate":{let s=String(t||"").trim(),n=Array.isArray(e.accept)?e.accept:[];for(let a of n){let i=O(s,a,u.lang||"es");if(i.ok)return{correct:!0,userAnswer:s,accentHint:i.hint||null}}return{correct:!1,userAnswer:s}}case"gap_fill":{let s=Array.isArray(t)?t:[],n=Array.isArray(e.answers)?e.answers:[],a=!0;for(let i=0;i<n.length;i++){let d=Array.isArray(n[i])?n[i]:[n[i]],c=String(s[i]||"").trim(),l=!1;for(let r of d)if(O(c,String(r),u.lang||"es").ok){l=!0;break}if(!l){a=!1;break}}return{correct:a,userAnswer:s}}default:return{correct:!1}}}function ht(e){let t=document.querySelector(".dk__exercise");if(!t)return null;switch(e.item_type){case"typed":case"translate":{let s=t.querySelector("#dk-input");return s?s.value:""}case"gap_fill":return[...t.querySelectorAll(".dk__input--gap")].map(s=>s.value);default:return null}}function P(){let e=b(u.sivuId),t=k[e],s=document.querySelector(".dk__content .dk__exercise");if(!s)return;let n=document.createElement("div");n.innerHTML=Ee(t),s.replaceWith(n.firstElementChild),je()}function je(){let e=document.querySelector(".dk__exercise");if(!e)return;let t=e.dataset.sivu,s=k.find(r=>r.id===t),n=Ie(s);if(!n)return;let a=n.items||[],i=Number(e.dataset.index),d=a[i];if(!d)return;let c=Se(t,a);e.querySelectorAll(".dk__choice").forEach(r=>{r.addEventListener("click",()=>{if(c.answered[i])return;let p=Number(r.dataset.choice),_=D(d,p);c.answered[i]=_,_.correct&&c.scoreCorrect++,c.scoreTotal++,c.scoreTotal>=a.length&&x(t),P()})}),document.getElementById("dk-check")?.addEventListener("click",()=>{if(c.answered[i])return;let r=ht(d),p=D(d,r);c.answered[i]=p,p.correct&&c.scoreCorrect++,c.scoreTotal++,c.scoreTotal>=a.length&&x(t),P()}),e.querySelector("#dk-input")?.addEventListener("keydown",r=>{r.key==="Enter"&&!r.shiftKey&&!c.answered[i]&&(r.preventDefault(),document.getElementById("dk-check")?.click())}),e.querySelectorAll(".dk__wordbank-chip").forEach(r=>{r.addEventListener("click",()=>{if(c.answered[i])return;let p=r.dataset.word||"",_=[...e.querySelectorAll(".dk__input--gap")],h=_.find(m=>!m.value.trim())||_[_.length-1];h&&(h.value=p,h.focus())})}),document.getElementById("dk-next-item")?.addEventListener("click",()=>{if(i<a.length-1)c.itemIndex=i+1,P();else{let r=b(u.sivuId),p=k[r+1];p&&T(p.id)}})}function Ce(){let e=f?.meta||{},t=b(u.sivuId),s=k[t],n=t>0?k[t-1]:null,a=t<k.length-1?k[t+1]:null,i=[e.course_key||u.kurssiKey,`Oppitunti ${e.lesson_index||u.lessonIndex}`].filter(Boolean).join(" \xB7 "),d=s.kind==="teoria"?`<em>${o(s.title)}</em>`:`${s.num?`${o(s.num)} \xB7 `:""}${o(s.title)}`,c=s.kind==="teoria"?Xe():s.kind==="tehtava"?Ee(s):s.kind==="flashcards"?be(s):s.kind==="testi"?_e(s):s.kind==="itsearviointi"?fe(s):le(s);return`
...
//# sourceMappingURL=app-digikirja-ONSPFIQU.js.map
