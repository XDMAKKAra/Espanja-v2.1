import{a as R,b as L}from"./app-chunk-EVWRWXV5.js";import{b as H}from"./app-chunk-BSBZXMKL.js";import{b as B}from"./app-chunk-AE7C6F2Z.js";import{c as E}from"./app-chunk-2SASPNNN.js";import{b as q}from"./app-chunk-CTTO4TQX.js";import{a as w,c as y,e as x,j as P}from"./app-chunk-T52YLBP4.js";function N(t,e){let s=t?.mastery_threshold;return s?typeof s[e]=="number"?s[e]:s.B??.7:.7}function F(t,e){let s=t?.skip_for_targets;return Array.isArray(s)&&s.includes(e)}function g(t){return String(t||"").toLowerCase().trim().normalize("NFD").replace(/[̀-ͯ]/g,"").replace(/\s+/g," ")}function A(t,e,s="es"){let a=e||[],n=g(t);if(n&&a.some(i=>g(i)===n))return{ok:!0,hint:null};for(let i of a){let r=B(t,i,s);if(r.ok)return r}return{ok:!1,hint:null}}var S="lr-review-v411";function D(t){let e=(t.items||[]).map(s=>({...s,topics:Array.isArray(s.topics)&&s.topics.length?s.topics:s._concept?[s._concept]:[]}));return{phase_id:S,phase_type:"review_mixed",title:"Kertaus",instruction:"Aloitetaan kertaamalla pari kohtaa jotka kaipasivat viel\xE4 toistoa.",mastery_threshold:{I:.5,A:.5,B:.5,C:.5,M:.5,E:.5,L:.5},skip_for_targets:[],items:e,_isReview:!0}}var _="lesson-runner-root",U="puheo:lessonProgress:";function V(t){let e=t&&typeof t.language=="string"&&t.language||"es",s=t&&t.kurssiKey?encodeURIComponent(t.kurssiKey):"",a=s?`#/oppimispolku/${e}/${s}`:`#/oppimispolku?lang=${e}`;location.hash!==a&&(location.hash=a)}var $=new Map;async function Y(t){if(!t)return[];if($.has(t))return $.get(t);try{let{API:e,isLoggedIn:s,authHeader:a,apiFetch:n}=await import("./app-api-GLA7PASW.js"),i=await n(`${e}/api/curriculum/${encodeURIComponent(t)}`,{headers:{...s()?a():{}}});if(!i.ok)return $.set(t,[]),[];let r=await i.json(),l=Array.isArray(r.lessons)?r.lessons.slice().sort((d,c)=>(d.sortOrder||0)-(c.sortOrder||0)):[];return $.set(t,l),l}catch{return[]}}function C(t){let e=Array.isArray(t.phases)?t.phases:[];if(e.length===0)return`<aside class="lr-toc" aria-label="Teht\xE4v\xE4t">
  ...
  </aside>`}function j(t,e){t.querySelectorAll(".lr-toc__item:not(.is-current)").forEach(s=>{let a=()=>{let n=Number(s.dataset.phase);!Number.isFinite(n)||n===e.currentPhaseIdx||(v(e),e.currentPhaseIdx=n,e.currentItemIdx=0,e.correctInPhase=0,e.answeredInPhase=0,v(e),b(t,e))};s.addEventListener("click",a),s.addEventListener("keydown",n=>{(n.key==="Enter"||n.key===" ")&&(n.preventDefault(),a())})})}function K(t){let e=t.lesson.meta||{},s=e.kurssi_title||e.course_title||e.course_key||"Kurssi",a=e.lesson_index||t.lessonIndex,n=e.title||"Oppitunti";return`<nav class="lr-breadcrumb" aria-label="Sijainti">
  ...
  </nav>`}function T(t,e){return`${U}${t}:${e}`}function v(t){if(!(!t||!t.kurssiKey||t.finished)&&!(t.currentPhaseIdx===0&&t.currentItemIdx===0&&t.answeredInPhase===0))try{sessionStorage.setItem(T(t.kurssiKey,t.lessonIndex),JSON.stringify({currentPhaseIdx:t.currentPhaseIdx,currentItemIdx:t.currentItemIdx,correctInPhase:t.correctInPhase,answeredInPhase:t.answeredInPhase,phaseResults:t.phaseResults,targetGrade:t.targetGrade,phaseSignature:t.phases.map(e=>`${e.phase_id}:${e.items.length}`).join("|"),savedAt:Date.now()}))}catch{}}function G(t,e,s){try{let a=sessionStorage.getItem(T(t,e));if(!a)return null;let n=JSON.parse(a);if(!n||typeof n!="object")return null;let i=s.phases.map(r=>`${r.phase_id}:${r.items.length}`).join("|");return n.phaseSignature!==i||n.savedAt&&Date.now()-n.savedAt>1440*60*1e3?null:n}catch{return null}}function Q(t,e){try{sessionStorage.removeItem(T(t,e))}catch{}}function u(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function X(t,e,s,a){let n=(t.phases||[]).filter(i=>!F(i,e));return{lesson:t,kurssiKey:s,lessonIndex:a,targetGrade:e,language:typeof E.language=="string"&&E.language||"es",phases:n,currentPhaseIdx:0,currentItemIdx:0,correctInPhase:0,answeredInPhase:0,gradedItems:[],phaseResults:[],sidePanelOpen:!1,sidePanelOpenMs:0,sidePanelOpenedAt:0,startedAt:Date.now(),finished:!1}}async function Z(t){try{let e=await P(`${w}/api/curriculum/review-queue?lang=${encodeURIComponent(t||"es")}&limit=8`,{headers:{...x()}});if(!e||!e.ok)return null;let s=await e.json();return s&&!s.locked&&Array.isArray(s.items)&&s.items.length?s:null}catch{return null}}async function ee(t){try{if(typeof y=="function"&&!y())return;let e=await Promise.race([Z(t.language),new Promise(s=>setTimeout(()=>s(null),1500))]);e&&Array.isArray(e.items)&&e.items.length&&(t.phases=[D(e),...t.phases])}catch{}}async function je(t,e,s,a){let n=t.pregenerated||t;te(),q("screen-lesson");let i=document.getElementById(_);if(!i)return;let r=X(n,a||"B",e,s);try{sessionStorage.setItem("currentLesson",JSON.stringify({kurssiKey:e,lessonIndex:s,lessonFocus:n.meta?.title||"",lessonType:n.meta?.lesson_type||"",targetGrade:r.targetGrade,isPregenerated:!0}));let d=n.teaching||{},c=String(d.intro_md||"").trim(),o=Array.isArray(d.key_points)?d.key_points:[],p=[c,o.length?`## Avainkohdat
`+o.map(h=>`- ${h}`).join(`
`):""].filter(Boolean).join(`

`);p?sessionStorage.setItem("currentLessonTeachingMd",p):sessionStorage.removeItem("currentLessonTeachingMd")}catch{}r._courseLessons=[],Y(e).then(d=>{r._courseLessons=d;let c=document.querySelector(".lr-shell .lr-toc");if(c&&c.parentElement){let o=document.createElement("div");o.innerHTML=C(r),c.parentElement.replaceChild(o.firstElementChild,c),j(document.getElementById(_),r)}}),await ee(r);let l=G(e,s,r);if(l){r.currentPhaseIdx=l.currentPhaseIdx,r.currentItemIdx=l.currentItemIdx,r.correctInPhase=l.correctInPhase||0,r.answeredInPhase=l.answeredInPhase||0,r.phaseResults=Array.isArray(l.phaseResults)?l.phaseResults:[],r.startedAt=Date.now(),b(i,r);return}se(i,r)}function te(){let t=document.getElementById("screen-lesson");t||(t=document.createElement("div"),t.id="screen-lesson",t.className="screen",document.querySelector(".app-main")?.appendChild(t)||document.body.appendChild(t)),document.getElementById(_)||(t.innerHTML=`<div id="${_}"></div>`)}function se(t,e){let s=e.lesson.meta||{},a=e.lesson.teaching||{},n=W(a.intro_md||""),i=Array.isArray(a.key_points)?a.key_points:[],r=i.length?`<ul class="lr-keypoints">${i.map(l=>`<li>${u(l)}</li>`).join("")}</ul>`:"";t.innerHTML=`
    ...
    </div>`,j(t,e),document.getElementById("lr-back")?.addEventListener("click",()=>{V(e)}),document.getElementById("lr-start")?.addEventListener("click",()=>{e.startedAt=Date.now(),b(t,e)})}function b(t,e){let s=e.phases[e.currentPhaseIdx];if(!s)return J(t,e);let a=e.phases.length,n=ne(e),i=s.items[e.currentItemIdx];if(!i)return M(t,e,s,"completed");t.innerHTML=`
    <div class="lr-shell lr-shell--exercise lr-shell--three">
      ...
      ${ve(e)}
    </div>`,ue(t,e,i),j(t,e)}function ne(t){return`<span class="lr-stepper" aria-hidden="true">${t.phases.map((s,a)=>a<t.currentPhaseIdx?'<span class="lr-step lr-step--done" aria-hidden="true">\u25CF</span>':a===t.currentPhaseIdx?'<span class="lr-step lr-step--current" aria-hidden="true">\u25CF</span>':'<span class="lr-step" aria-hidden="true">\u25CB</span>').join("")}</span>`}function re(t,e){switch(t.item_type){case"mc":return ae(t);case"typed":return z(t);case"translate":return ie(t);case"match":return le(t);case"gap_fill":return ce(t);case"writing":return oe(t);case"reading_mc":return de(t);default:return`<p class="lr-unsupported">Teht\xE4v\xE4tyyppi\xE4 "${u(t.item_type||"?")}" ei tueta, ohitetaan.</p>
    ...
    <div class="lr-typed">
      ...
      <div class="lr-feedback" id="lr-feedback" hidden></div>
    </div>`}function ie(t){return z({...t,prompt:t.source})}function le(t){let e=(t.pairs||[]).map(a=>a.left),s=ye((t.pairs||[]).map(a=>a.right));return`
    ...
    <div class="lr-gap">
      ...
      <div class="lr-feedback" id="lr-feedback" hidden></div>
    </div>`}function oe(t){let e=Number(t.min_chars)||L(t.min_words),s=Number(t.max_chars)||L(t.max_words);return`
  ...
  `).join("");return`
    ...
    </div>`,document.getElementById("lr-next")?.addEventListener("click",()=>{e.currentPhaseIdx+=1,e.currentItemIdx=0,e.correctInPhase=0,e.answeredInPhase=0,e.currentPhaseIdx>=e.phases.length?J(t,e):(v(e),b(t,e))})}function ge(t,e){if(!window.confirm("Ohita t\xE4m\xE4 vaihe? Sanat palaavat kertaussessioon my\xF6hemmin."))return;let a=e.phases[e.currentPhaseIdx];M(t,e,a,"skipped")}function J(t,e){if(e.finished)return;e.finished=!0,Q(e.kurssiKey,e.lessonIndex);let s=e.phaseResults.filter(o=>o.phaseId!==S),a=s.reduce((o,p)=>o+(p.skipped?0:p.correct),0),n=s.reduce((o,p)=>o+(p.skipped?0:p.total),0),i=Math.max(1,Math.round((Date.now()-e.startedAt)/6e4)),r=e.targetGrade,l=ke(r,e.phaseResults),d=e.lesson.meta?.yo_relevance||"",c=e.phaseResults.map(o=>{let p=o.skipped?"lr-result-skipped":o.mastered?"lr-result-mastered":"lr-result-almost",h=o.skipped?"Ohitettu":o.mastered?"Hallitset":"L\xE4hell\xE4";return`<li class="lr-result-row ${p}">
