import{b as O}from"./app-chunk-AE7C6F2Z.js";import{a as S,c as M,d as K,e as E,j as A}from"./app-chunk-NZWTLFMY.js";import{b as z}from"./app-chunk-3WC2U67L.js";function Be(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function y(e){let t=Be(e);return t=t.replace(/`([^`]+)`/g,'<code class="dk__teoria-code">$1</code>'),t=t.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),t=t.replace(/\*([^*\n]+)\*/g,"<em>$1</em>"),t=t.replace(/(^|\s)_([^_\n]+)_(?=\s|[.,;:!?]|$)/g,"$1<em>$2</em>"),t}function X(e){return/^\s*\|?\s*:?-{2,}:?(\s*\|\s*:?-{2,}:?\s*)+\|?\s*$/.test(e)}function Z(e){return e.trim().replace(/^\|/,"").replace(/\|$/,"").split("|").map(s=>s.trim())}function Ke(e,t){let s=Z(e),n=t.map(Z),a=s.map(c=>`<th>${y(c)}</th>`).join(""),i=n.map(c=>`<tr>${c.map(l=>`<td>${y(l)}</td>`).join("")}</tr>`).join("");return`<div class="dk__bilingual${s.length===2?" dk__bilingual--2col":""}"><table><thead><tr>${a}</tr></thead><tbody>${i}</tbody></table></div>`}function Oe(e){return`
    ...
    </aside>`}function He(e){return`<ul class="dk__teoria-ul">${e.map(s=>`<li>${y(s.replace(/^[-*]\s+/,""))}</li>`).join("")}</ul>`}function Q(e){if(!e||typeof e!="string")return"";let t=e.replace(/\r\n?/g,`
`).split(`
`),s=[],n=0,a=!1;for(;n<t.length;){let i=t[n];if(/^\s*$/.test(i)){n++;continue}let d=/^(#{1,6})\s+(.*)$/.exec(i);if(d){let o=d[1].length,l=d[2].trim();if(o===1&&!a){a=!0,n++;continue}o===2?s.push(`<h3 class="dk__teoria-h2">${y(l)}</h3>`):o===3?s.push(`<h4 class="dk__teoria-h3">${y(l)}</h4>`):s.push(`<h${Math.min(o+1,6)} class="dk__teoria-h">${y(l)}</h${Math.min(o+1,6)}>`),n++;continue}if(/^\s*>\s?/.test(i)){let o=[];for(;n<t.length&&/^\s*>\s?/.test(t[n]);)o.push(t[n]),n++;s.push(Oe(o));continue}if(/^\s*\|/.test(i)&&n+1<t.length&&X(t[n+1])){let o=i,l=[];for(n+=2;n<t.length&&/^\s*\|/.test(t[n]);)l.push(t[n]),n++;s.push(Ke(o,l));continue}if(/^\s*[-*]\s+/.test(i)){let o=[];for(;n<t.length&&/^\s*[-*]\s+/.test(t[n]);)o.push(t[n]),n++;s.push(He(o));continue}let c=[i];for(n++;n<t.length&&!/^\s*$/.test(t[n])&&!/^(#{1,6})\s+/.test(t[n])&&!/^\s*>\s?/.test(t[n])&&!/^\s*[-*]\s+/.test(t[n])&&!(/^\s*\|/.test(t[n])&&n+1<t.length&&X(t[n+1]));)c.push(t[n]),n++;s.push(`<p class="dk__teoria-p">${y(c.join(" "))}</p>`)}return s.join(`
`)}var ae="puheo:dk:sidemenu",j="open",g="collapsed",Ne="puheo:dk:progress";function ie(){return`${Ne}:${u.lang}:${u.kurssiKey}:${u.lessonIndex}`}function B(){try{return JSON.parse(localStorage.getItem(ie())||"{}")}catch{return{}}}function re(e){try{localStorage.setItem(ie(),JSON.stringify(e))}catch{}}function x(e){if(!e)return;let t=B();t[e]!=="done"&&(t[e]="done",re(t),M()&&A(`${S}/api/digikirja/progress`,{method:"POST",headers:{"Content-Type":"application/json",...E()},body:JSON.stringify({lang:u.lang,kurssi:u.kurssiKey,lesson:u.lessonIndex,sivuId:e})}).catch(()=>{}),U())}function U(){let e=B(),t=document.getElementById("dk-sidemenu-list");t&&t.querySelectorAll(".dk__row").forEach(n=>{n.classList.toggle("is-done",e[n.dataset.sivu]==="done")});let s=document.getElementById("dk-progress-chip");if(s){let n=k.filter(a=>e[a.id]==="done").length;s.textContent=`${n} / ${k.length} valmis`,s.dataset.full=n>=k.length?"true":"false"}}var Pe={teoria:"Opetus",tehtava:"Harjoitukset",flashcards:"Kortit",testi:"Testit",itsearviointi:"Itsearvio"};var oe=5,Re="puheo:dk:flashcards",ee={},de=!1,u={lang:"es",kurssiKey:"kurssi_2",lessonIndex:3,sivuId:"teoria"},v=null,k=[],H="";function r(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function qe(){try{return localStorage.getItem(ae)===g?g:j}catch{return j}}function Ve(e){try{localStorage.setItem(ae,e)}catch{}}function Fe(e){return e==="fr"?"Ranska":e==="de"?"Saksa":"Espanja"}function De(e){return`/data/courses/${encodeURIComponent(e.lang)}/${encodeURIComponent(e.kurssiKey)}/lesson_${encodeURIComponent(e.lessonIndex)}.json`}async function Ue(e){let t=De(e),s=await fetch(t,{headers:{accept:"application/json"}});if(!s.ok)throw new Error(`lesson fetch ${s.status}`);return s.json()}async function Je(e){if(M())try{let t=`lang=${encodeURIComponent(e.lang)}&kurssi=${encodeURIComponent(e.kurssiKey)}&lesson=${encodeURIComponent(e.lessonIndex)}`,[s,n]=await Promise.all([A(`${S}/api/digikirja/progress?${t}`,{headers:E()}).catch(()=>null),A(`${S}/api/digikirja/itsearvio?${t}`,{headers:E()}).catch(()=>null)]);if(s?.ok){let{sivut:a}=await s.json().catch(()=>({sivut:{}}));if(a&&typeof a=="object"){let i=B();for(let d of Object.keys(a))i[d]||(i[d]="done");re(i)}}if(n?.ok){let{itsearvio:a}=await n.json().catch(()=>({itsearvio:null}));a?.ratings&&(Y()||he({ratings:a.ratings,submittedAt:a.submittedAt,lang:e.lang,kurssiKey:e.kurssiKey,lessonIndex:e.lessonIndex}))}}catch{}}function Ye(e){let t=[];t.push({id:"teoria",kind:"teoria",num:"",title:e?.meta?.title||"Opetus",meta:"Opetussivu"}),(Array.isArray(e?.phases)?e.phases:[]).forEach((i,d)=>{let c=String(d+1),o=i.title||`Vaihe ${c}`,l=Array.isArray(i.items)?i.items.length:0;t.push({id:`phase-${d}`,kind:"tehtava",num:c,title:o,meta:l?`${l} kohtaa`:"Teht\xE4v\xE4"})});let n=Array.isArray(e?.vocab)?e.vocab:[],a=Math.min(n.length,oe);return a>0&&t.push({id:"kortit-1",kind:"flashcards",num:"",title:`K\xE4\xE4nt\xF6kortit \xB7 ${e?.meta?.title||""}`.trim(),meta:`${a} korttia`}),t}function b(e){let t=k.findIndex(s=>s.id===e);return t>=0?t:0}function Ge(){let t=(v?.meta||{}).title||"Oppitunti";return`
    <header class="dk__topbar" role="banner">
      ...
      <div class="dk__tools">
    ...
    </header>`}function We(){let e=[],t=null;for(let c of k){let o=Pe[c.kind]||"Muut";o!==t&&(e.push({title:o,items:[]}),t=o),e[e.length-1].items.push(c)}let s=B(),n=e.map(c=>{let o=`<span class="dk__group-title">${r(c.title)}</span>`,l=c.items.map(p=>{let _=p.id===u.sivuId,h=s[p.id]==="done",m=["dk__row"];_&&m.push("is-active"),h&&m.push("is-done");let f=p.num?`${r(p.num)} `:"";return`
        <button type="button"
                class="${m.join(" ")}"
                data-sivu="${r(p.id)}"
                data-kind="${r(p.kind)}"
        ...
        </button>`}).join("");return o+l}).join(""),a="";try{a=(localStorage.getItem("puheo:nickname")||"").trim()}catch{}let i=typeof K=="function"&&K()||"",d=a||i||"Oma sivu";return`
    <aside class="dk__sidemenu" id="dk-sidemenu" aria-label="Oppitunnin sis\xE4llys">
      <div class="dk__sidemenu-top">
        ...
        <div class="dk__sidemenu-course">${r(Fe(u.lang))} \xB7 ${r(u.kurssiKey||"")}</div>
      </div>
    ...
    <div class="dk__placeholder" data-kind="${r(e.kind)}">
      ...
      <p>${r(t.body)}</p>
    </div>`}var R=new Map;function ue(e){let t=e?.testDef;return t?((Array.isArray(v?.phases)?v.phases:[])[t.sourcePhase]?.items||[]).slice(0,t.count).filter(a=>we.has(a.item_type)):[]}function pe(e,t){let s=R.get(e);return s||(s={submitted:!1,answers:t.map(n=>n.item_type==="mc"?null:n.item_type==="gap_fill"?new Array((String(n.sentence_template||"").match(/\{\d+\}/g)||[]).length).fill(""):""),results:t.map(()=>null),scoreCorrect:0},R.set(e,s)),s}function _e(e){let t=ue(e);if(t.length===0)return`
      <div class="dk__placeholder" data-kind="testi">
        <p>T\xE4ll\xE4 testill\xE4 ei ole viel\xE4 kohtia.</p>
      </div>`;let s=pe(e.id,t),n=e.testDef?.label||e.title||"Testi",a=t.map((c,o)=>Ze(c,o,s)).join(""),i=s.submitted?Qe(t,s):"",d=s.submitted?`<button type="button" class="dk__btn dk__btn--ghost" id="dk-testi-reset">Tee uudelleen</button>
    ...
    <section class="dk__testi" data-sivu="${r(e.id)}">
      ...
      <div class="dk__exercise-actions dk__testi-actions">${d}</div>
    </section>`}function Ze(e,t,s){let n=s.submitted,a=s.results[t],i=e.item_type==="translate"?e.source||"":e.item_type==="typed"?e.prompt||"":e.item_type==="gap_fill"?null:e.stem||"",d=n?`<span class="dk__feedback-chip ${a?.correct?"is-correct":"is-wrong"}">${a?.correct?"Oikein":"Viel\xE4 ei"}</span>`:`<span class="dk__testi-itemnum">${t+1}</span>`,c="";switch(e.item_type){case"mc":{let l=n?a?.choiceIndex:s.answers[t]===null?-1:s.answers[t],p=e.correct_index;c=`
        <p class="dk__exercise-stem dk__testi-stem">${r(i)}</p>
        <ol class="dk__choices" role="radiogroup" aria-label="Vaihtoehdot">
          ${(e.choices||[]).map((_,h)=>{let m=l===h,f=h===p,$=["dk__choice"];return n?m&&f?$.push("is-correct"):m&&!f?$.push("is-wrong"):f&&$.push("is-revealed"):m&&$.push("is-selected"),`
              <li>
        ...
        </div>`;break}case"gap_fill":{let l=String(e.sentence_template||""),p=n?a?.userAnswer||[]:s.answers[t]||[],_=0,h=r(l).replace(/\{(\d+)\}/g,()=>{let f=p[_]||"",$=`dk-testi-${t}-gap-${_}`,Me=_;return _++,`<input id="${$}" type="text" class="dk__input dk__input--gap"
    ...
    <div class="dk__testi-summary" aria-live="polite">
    ...
    </div>`}function et(e,t){return e.map((n,a)=>{switch(n.item_type){case"mc":return t.answers[a];case"typed":case"translate":{let i=document.getElementById(`dk-testi-input-${a}`);return i?i.value:""}case"gap_fill":return[...document.querySelectorAll(`[data-testi-item="${a}"][data-testi-gap]`)].map(d=>d.value);default:return null}})}function te(){let e=b(u.sivuId),t=k[e],s=document.querySelector(".dk__content .dk__testi");if(!s)return;let n=document.createElement("div");n.innerHTML=_e(t),s.replaceWith(n.firstElementChild),ke()}function ke(){let e=document.querySelector(".dk__testi");if(!e)return;let t=e.dataset.sivu,s=k.find(i=>i.id===t),n=ue(s),a=pe(t,n);e.querySelectorAll(".dk__choice[data-testi-item]").forEach(i=>{i.addEventListener("click",()=>{if(a.submitted)return;let d=Number(i.dataset.testiItem),c=Number(i.dataset.choice);a.answers[d]=c,e.querySelector(`.dk__testi-item[data-testi-item="${d}"]`)?.querySelectorAll(".dk__choice").forEach(l=>{l.classList.toggle("is-selected",Number(l.dataset.choice)===c)})})}),document.getElementById("dk-testi-submit")?.addEventListener("click",()=>{if(a.submitted)return;a.answers=et(n,a);let i=0;a.results=n.map((d,c)=>{let o=D(d,a.answers[c]);return o.correct&&i++,o}),a.scoreCorrect=i,a.submitted=!0,x(t),te(),requestAnimationFrame(()=>{document.querySelector(".dk__testi-summary")?.scrollIntoView({block:"start",behavior:"smooth"})})}),document.getElementById("dk-testi-reset")?.addEventListener("click",()=>{R.delete(t),te()}),document.getElementById("dk-testi-next-sivu")?.addEventListener("click",()=>{let i=b(u.sivuId),d=k[i+1];d&&T(d.id)})}var tt="puheo:dk:itsearvio",C=[{id:"vocab",text:"Hallitsen t\xE4m\xE4n oppitunnin sanaston."},{id:"grammar",text:"Pystyn k\xE4ytt\xE4m\xE4\xE4n uutta kielioppia omissa lauseissani."},{id:"input",text:"Ymm\xE4rr\xE4n aiheen teksti\xE4 ja keskusteluja."},{id:"output",text:"Voin puhua ja kirjoittaa t\xE4st\xE4 aiheesta espanjaksi."}],st=["heikko","vajaa","kohtuu","vahva","hallitsen"];function J(){return`${tt}:${u.lang}:${u.kurssiKey}:${u.lessonIndex}`}function Y(){try{return JSON.parse(localStorage.getItem(J())||"null")}catch{return null}}function he(e){try{localStorage.setItem(J(),JSON.stringify(e))}catch{}}var q=new Map;function me(e){let t=q.get(e);return t||(t={...Y()?.ratings||{}},q.set(e,t)),t}function fe(e){let t=me(e.id),s=Y(),n=!!s,a=C.map(o=>{let l=n?s.ratings?.[o.id]??0:t[o.id]??0,p=[1,2,3,4,5].map(_=>`
      <button type="button"
              class="dk__arvio-btn ${l===_?"is-chosen":""}"
              data-statement="${r(o.id)}"
              data-value="${_}"
              aria-pressed="${l===_}"
              aria-label="${_}, ${r(st[_-1])}"
              ${n?"disabled":""}>
      ...
      </div>`}).join(""),i=C.every(o=>Number.isInteger(t[o.id])&&t[o.id]>0),d=n?`<button type="button" class="dk__btn dk__btn--ghost" id="dk-arvio-reset">P\xE4ivit\xE4 arvio</button>
       <button type="button" class="dk__btn dk__btn--primary" id="dk-arvio-back">Takaisin oppimispolulle</button>`:`<button type="button" class="dk__btn dk__btn--primary" id="dk-arvio-submit" ${i?"":"disabled"}>Tallenna arvio</button>`,c=n?nt(s):"";return`
    <section class="dk__arvio" data-sivu="${r(e.id)}">
      <header class="dk__exercise-head">
        <span class="dk__exercise-eyebrow">Itsearviointi</span>
        <span class="dk__exercise-score">${C.length} v\xE4itt\xE4m\xE4\xE4</span>
      </header>
    ...
    </section>`}function nt(e){let t=e?.ratings||{},s=C.map(i=>t[i.id]).filter(Number.isInteger);if(s.length===0)return"";let n=s.reduce((i,d)=>i+d,0)/s.length,a=n>=4?"Olet vahvalla pohjalla.":n>=3?"Hyv\xE4, suuntaa ty\xF6 heikoimpiin kohtiin.":"Kannattaa kerrata oppitunti ennen seuraavaa.";return`
    <div class="dk__testi-summary" aria-live="polite">
      <div class="dk__testi-summary-score">
    ...
    </div>`}function se(){let e=b(u.sivuId),t=k[e],s=document.querySelector(".dk__content .dk__arvio");if(!s)return;let n=document.createElement("div");n.innerHTML=fe(t),s.replaceWith(n.firstElementChild),ve()}function ve(){let e=document.querySelector(".dk__arvio");if(!e)return;let t=e.dataset.sivu,s=me(t);e.querySelectorAll(".dk__arvio-btn").forEach(n=>{n.addEventListener("click",()=>{let a=n.dataset.statement,i=Number(n.dataset.value);s[a]=i,e.querySelector(`.dk__arvio-row[data-statement="${a}"]`)?.querySelectorAll(".dk__arvio-btn").forEach(o=>{let l=Number(o.dataset.value)===i;o.classList.toggle("is-chosen",l),o.setAttribute("aria-pressed",l?"true":"false")});let c=document.getElementById("dk-arvio-submit");if(c){let o=C.every(l=>Number.isInteger(s[l.id])&&s[l.id]>0);c.disabled=!o}})}),document.getElementById("dk-arvio-submit")?.addEventListener("click",()=>{let n={ratings:{...s},submittedAt:new Date().toISOString(),lang:u.lang,kurssiKey:u.kurssiKey,lessonIndex:u.lessonIndex};he(n),x(t),se(),M()&&A(`${S}/api/digikirja/itsearvio`,{method:"POST",headers:{"Content-Type":"application/json",...E()},body:JSON.stringify({lang:u.lang,kurssi:u.kurssiKey,lesson:u.lessonIndex,ratings:n.ratings})}).catch(()=>{})}),document.getElementById("dk-arvio-reset")?.addEventListener("click",()=>{try{localStorage.removeItem(J())}catch{}q.delete(t),se()}),document.getElementById("dk-arvio-back")?.addEventListener("click",()=>{location.hash=`#/oppimispolku/${u.lang}/${encodeURIComponent(u.kurssiKey)}`})}var V=new Map,w="know",ge="again";function F(e){return(Array.isArray(e?.vocab)?e.vocab:[]).slice(0,oe)}function G(){return`${Re}:${u.lang}:${u.kurssiKey}:${u.lessonIndex}`}function W(){try{let e=localStorage.getItem(G());return e?JSON.parse(e):{}}catch{return{}}}function at(e,t){try{let s=W();s[e]=t,localStorage.setItem(G(),JSON.stringify(s))}catch{}}function it(){try{localStorage.removeItem(G())}catch{}}function ye(e){let t=V.get(e);return t||(t={cardIndex:0,flipped:!1},V.set(e,t)),t}function I(e,t){return e?.es?`${t}:${e.es}`:`${t}`}function be(e){let t=F(v);if(t.length===0)return'<div class="dk__placeholder"><p>T\xE4m\xE4n oppitunnin sanasto on tyhj\xE4.</p></div>';let s=W(),n=t.filter((h,m)=>s[I(h,m)]===w).length;if(n===t.length)return`
      ...
      </section>`;let i=ye(e.id),d=Math.min(i.cardIndex,t.length-1),o=$e(t,s,d)[0]??d,l=t[o],p=I(l,o),_=s[p]||null;return`
    ...
    </section>`}function $e(e,t,s){let n=[];for(let a=s;a<e.length;a++){let i=I(e[a],a);t[i]!==w&&n.push(a)}for(let a=0;a<s;a++){let i=I(e[a],a);t[i]!==w&&n.push(a)}return n}function rt(e,t,s,n){let a=e.gender?`<span class="dk__flashcard-tag">${r(e.gender==="m"?"Maskuliini":e.gender==="f"?"Feminiini":e.gender)}</span>`:"",i=n===ge?'<span class="dk__flashcard-tag dk__flashcard-tag--again">Harjoittelussa</span>':n===w?'<span class="dk__flashcard-tag dk__flashcard-tag--know">Hallinnassa</span>':"";return`
    ...
    </div>`}function N(){let e=b(u.sivuId),t=k[e],s=document.querySelector(".dk__content .dk__flashpack");if(!s)return;let n=document.createElement("div");n.innerHTML=be(t),s.replaceWith(n.firstElementChild),xe()}function xe(){let e=document.querySelector(".dk__flashpack");if(!e)return;let t=e.dataset.sivu;if(document.getElementById("dk-flash-reset")?.addEventListener("click",()=>{it(),V.delete(t),N()}),document.getElementById("dk-flash-next-sivu")?.addEventListener("click",()=>{let o=b(u.sivuId),l=k[o+1];l&&T(l.id)}),e.dataset.done==="true")return;let s=ye(t),n=Number(e.dataset.index),a=F(v)[n];if(!a)return;let i=I(a,n),d=()=>{s.flipped=!s.flipped,N()};document.getElementById("dk-flashcard")?.addEventListener("click",d),document.getElementById("dk-flashcard")?.addEventListener("keydown",o=>{(o.key===" "||o.key==="Enter")&&(o.preventDefault(),d())}),document.getElementById("dk-flash-flip")?.addEventListener("click",d);let c=o=>{at(i,o),s.flipped=!1;let l=W(),p=F(v);p.length>0&&p.every((m,f)=>l[I(m,f)]===w)&&x(t);let h=$e(p,l,n+1<p.length?n+1:0);s.cardIndex=h[0]??n,N()};document.getElementById("dk-flash-again")?.addEventListener("click",()=>c(ge)),document.getElementById("dk-flash-know")?.addEventListener("click",()=>c(w))}var ne=new Map,we=new Set(["mc","typed","gap_fill","translate"]);function Ie(e){if(!e||e.kind!=="tehtava")return null;let t=/^phase-(\d+)$/.exec(e.id);if(!t)return null;let s=Number(t[1]);return(Array.isArray(v?.phases)?v.phases:[])[s]||null}function Se(e,t){let s=ne.get(e);return s||(s={itemIndex:0,answered:new Array(t.length).fill(null),scoreCorrect:0,scoreTotal:0},ne.set(e,s)),s}function Ee(e){let t=Ie(e);if(!t)return le(e);let s=Array.isArray(t.items)?t.items:[];if(s.length===0)return'<div class="dk__placeholder"><p>T\xE4ll\xE4 vaiheella ei ole teht\xE4vi\xE4.</p></div>';let n=s[0].item_type;if(!we.has(n)){let o=n==="match"?"Yhdist\xE4misteht\xE4v\xE4, tulossa PR 4b":n==="writing"?"Kirjoitusteht\xE4v\xE4, tulossa PR 7":`Teht\xE4v\xE4tyyppi "${n}" tulossa my\xF6hemmin`;return`
      <div class="dk__placeholder" data-kind="tehtava">
        <p class="dk__placeholder-kind">${r(o)}</p>
        <p>Vaihe ${r(String((ot(e)??0)+1))}: ${r(t.title||"")}. Vaiheessa ${s.length} teht\xE4v\xE4\xE4 t\xE4t\xE4 tyyppi\xE4.</p>
    ...
    <section class="dk__exercise" data-sivu="${r(e.id)}" data-index="${i}">
      ...
      ${dt(d,c)}
      ${_t(d,c,i,s.length)}
    </section>`}function ot(e){let t=/^phase-(\d+)$/.exec(e.id);return t?Number(t[1]):null}function dt(e,t){switch(e.item_type){case"mc":return ct(e,t);case"typed":return lt(e,t);case"gap_fill":return ut(e,t);case"translate":return pt(e,t);default:return`<p class="dk__teoria-p">Teht\xE4v\xE4tyyppi \u201D${r(e.item_type)}\u201D ei ole viel\xE4 k\xE4ytett\xE4viss\xE4.</p>`}}function ct(e,t){let s=Array.isArray(e.choices)?e.choices:[],n=Number.isInteger(e.correct_index)?e.correct_index:-1,a=!!t,i=t?.choiceIndex;return`
    ...
    <div class="dk__input-row">
      ...
      <input id="dk-input" type="text" class="dk__input"
             ...
             value="${r(n)}"
             ${a?"disabled":""}>
    </div>`}function ut(e,t){let s=String(e.sentence_template||""),n=(s.match(/\{(\d+)\}/g)||[]).length,a=t?.userAnswer||new Array(n).fill(""),i=!!t,d=0,c=r(s).replace(/\{(\d+)\}/g,()=>{let l=a[d]||"",p=`dk-gap-${d}`;return d++,`<input id="${p}" type="text" class="dk__input dk__input--gap"
    ...
    ${o}`}function pt(e,t){let s=e.direction==="es_to_fi"?"espanjasta suomeksi":e.direction==="fi_to_es"?"suomesta espanjaksi":"k\xE4\xE4nn\xF6s",n=t?.userAnswer||"",a=!!t;return`
    ...
    <div class="dk__input-row">
      ...
      </div>`;let a=t.correct,i=a?"is-correct":"is-wrong",d=a?"Oikein":"Viel\xE4 ei aivan",c=kt(e,t),o=s>=n-1;return`
    ...
    </div>`}function kt(e,t){let s=Ae(e),n=s?`<p class="dk__feedback-expected"><span>Oikea vastaus:</span> ${r(s)}</p>`:"",a=e.explanation?`<p class="dk__feedback-text">${r(e.explanation)}</p>`:"",i=t?.accentHint?`<p class="dk__feedback-text dk__feedback-hint">${r(t.accentHint)}</p>`:"";return`${n}${i}${a}`}function Ae(e){switch(e.item_type){case"mc":return Array.isArray(e.choices)&&Number.isInteger(e.correct_index)?e.choices[e.correct_index]:"";case"typed":return Array.isArray(e.accept)&&e.accept[0]||"";case"translate":return Array.isArray(e.accept)&&e.accept[0]||"";case"gap_fill":{let t=String(e.sentence_template||""),s=Array.isArray(e.answers)?e.answers:[],n=0;return t.replace(/\{(\d+)\}/g,()=>{let a=s[n++];return Array.isArray(a)&&a[0]||"\u2014"})}default:return""}}function D(e,t){switch(e.item_type){case"mc":{let s=Number(t);return{correct:s===e.correct_index,choiceIndex:s}}case"typed":case"translate":{let s=String(t||"").trim(),n=Array.isArray(e.accept)?e.accept:[];for(let a of n){let i=O(s,a,u.lang||"es");if(i.ok)return{correct:!0,userAnswer:s,accentHint:i.hint||null}}return{correct:!1,userAnswer:s}}case"gap_fill":{let s=Array.isArray(t)?t:[],n=Array.isArray(e.answers)?e.answers:[],a=!0;for(let i=0;i<n.length;i++){let d=Array.isArray(n[i])?n[i]:[n[i]],c=String(s[i]||"").trim(),o=!1;for(let l of d)if(O(c,String(l),u.lang||"es").ok){o=!0;break}if(!o){a=!1;break}}return{correct:a,userAnswer:s}}default:return{correct:!1}}}function ht(e){let t=document.querySelector(".dk__exercise");if(!t)return null;switch(e.item_type){case"typed":case"translate":{let s=t.querySelector("#dk-input");return s?s.value:""}case"gap_fill":return[...t.querySelectorAll(".dk__input--gap")].map(s=>s.value);default:return null}}function P(){let e=b(u.sivuId),t=k[e],s=document.querySelector(".dk__content .dk__exercise");if(!s)return;let n=document.createElement("div");n.innerHTML=Ee(t),s.replaceWith(n.firstElementChild),je()}function je(){let e=document.querySelector(".dk__exercise");if(!e)return;let t=e.dataset.sivu,s=k.find(l=>l.id===t),n=Ie(s);if(!n)return;let a=n.items||[],i=Number(e.dataset.index),d=a[i];if(!d)return;let c=Se(t,a);e.querySelectorAll(".dk__choice").forEach(l=>{l.addEventListener("click",()=>{if(c.answered[i])return;let p=Number(l.dataset.choice),_=D(d,p);c.answered[i]=_,_.correct&&c.scoreCorrect++,c.scoreTotal++,c.scoreTotal>=a.length&&x(t),P()})}),document.getElementById("dk-check")?.addEventListener("click",()=>{if(c.answered[i])return;let l=ht(d),p=D(d,l);c.answered[i]=p,p.correct&&c.scoreCorrect++,c.scoreTotal++,c.scoreTotal>=a.length&&x(t),P()}),e.querySelector("#dk-input")?.addEventListener("keydown",l=>{l.key==="Enter"&&!l.shiftKey&&!c.answered[i]&&(l.preventDefault(),document.getElementById("dk-check")?.click())}),e.querySelectorAll(".dk__wordbank-chip").forEach(l=>{l.addEventListener("click",()=>{if(c.answered[i])return;let p=l.dataset.word||"",_=[...e.querySelectorAll(".dk__input--gap")],h=_.find(m=>!m.value.trim())||_[_.length-1];h&&(h.value=p,h.focus())})}),document.getElementById("dk-next-item")?.addEventListener("click",()=>{if(i<a.length-1)c.itemIndex=i+1,P();else{let l=b(u.sivuId),p=k[l+1];p&&T(p.id)}})}function Ce(){let e=v?.meta||{},t=b(u.sivuId),s=k[t],n=t>0?k[t-1]:null,a=t<k.length-1?k[t+1]:null,i=[e.course_key||u.kurssiKey,`Oppitunti ${e.lesson_index||u.lessonIndex}`].filter(Boolean).join(" \xB7 "),d=s.kind==="teoria"?`<em>${r(s.title)}</em>`:`${s.num?`${r(s.num)} \xB7 `:""}${r(s.title)}`,c=s.kind==="teoria"?Xe():s.kind==="tehtava"?Ee(s):s.kind==="flashcards"?be(s):s.kind==="testi"?_e(s):s.kind==="itsearviointi"?fe(s):le(s);return`
    <main class="dk__content" id="dk-content" tabindex="-1">
      <p class="dk__page-meta">${r(i)}</p>
      <h2 class="dk__page-title">${d}</h2>
      ${c}
      ${mt(n,a,"bottom")}
    </main>`}function mt(e,t,s){let n=`dk__prevnext dk__prevnext--${s}`,a=e?`<button type="button" class="dk__prevnext-btn dk__prevnext-btn--prev" data-sivu="${r(e.id)}">
    ...
    </div>`}function L(e){let t=document.getElementById("dk-root");if(!t)return;t.dataset.sidemenu=e;let s=document.getElementById("dk-toggle-sidemenu");s&&s.setAttribute("aria-pressed",e===g?"true":"false")}function gt(){let e=document.getElementById("dk-toggle-sidemenu"),t=document.getElementById("dk-sidemenu-backdrop");e?.addEventListener("click",()=>{let s=document.getElementById("dk-root");if(window.matchMedia("(max-width: 1023px)").matches){let a=s.dataset.sidemenu===j?g:j;L(a)}else{let a=s.dataset.sidemenu===g?j:g;L(a),Ve(a)}}),t?.addEventListener("click",()=>{L(g)})}function T(e){if(!e||e===u.sivuId||k.findIndex(d=>d.id===e)<0)return;u.sivuId=e;let s=`#/oppitunti/${u.lang}/${encodeURIComponent(u.kurssiKey)}/${u.lessonIndex}/${encodeURIComponent(e)}`;location.hash!==s&&history.replaceState(null,"",s);let n=document.getElementById("dk-root");if(!n)return;let a=n.querySelector(".dk__sidemenu-list");a&&a.querySelectorAll(".dk__row").forEach(d=>{let c=d.dataset.sivu===e;d.classList.toggle("is-active",c),d.setAttribute("aria-current",c?"page":"false")});let i=n.querySelector(".dk__content");if(i){let d=document.createElement("div");d.innerHTML=Ce(),i.replaceWith(d.firstElementChild),Te()}window.matchMedia("(max-width: 1023px)").matches&&L(g),Le(),ce(),U(),document.getElementById("dk-content")?.focus({preventScroll:!1})}function yt(){document.getElementById("dk-sidemenu-list")?.addEventListener("click",e=>{let t=e.target.closest(".dk__row");t&&T(t.dataset.sivu)})}function Le(){let e=document.getElementById("dk-sidemenu-list");if(!e)return;let t=e.querySelector(".dk__row.is-active");t&&requestAnimationFrame(()=>{try{t.scrollIntoView({block:"nearest",inline:"nearest",behavior:"auto"})}catch{let s=t.offsetTop-e.clientHeight/2+t.clientHeight/2;e.scrollTop=Math.max(0,s)}})}function Te(){document.querySelectorAll(".dk__content .dk__prevnext-btn[data-sivu]").forEach(e=>{e.addEventListener("click",()=>T(e.dataset.sivu))}),je(),xe(),ke(),ve()}function bt(){de=!0}async function $t(e={}){de||bt(),u.lang=e.lang||u.lang,u.kurssiKey=e.kurssiKey||u.kurssiKey,u.lessonIndex=Number(e.lessonIndex)||u.lessonIndex,u.sivuId=e.sivuId||u.sivuId||"teoria";let t=document.getElementById("screen-digikirja");if(!t)return;t.innerHTML=ft(),z("screen-digikirja");let s=`${u.lang}/${u.kurssiKey}/${u.lessonIndex}`;H=s;try{let[n]=await Promise.all([Ue(u),Je(u)]);if(H!==s)return;v=n,k=Ye(n),k.some(d=>d.id===u.sivuId)||(u.sivuId=k[0]?.id||"teoria");let i=window.matchMedia("(max-width: 1023px)").matches?g:qe();t.innerHTML=`
