...
`).split(`
`),s=[],n=0,a=!1;for(;n<t.length;){let i=t[n];if(/^\s*$/.test(i)){n++;continue}let d=/^(#{1,6})\s+(.*)$/.exec(i);if(d){let l=d[1].length,r=d[2].trim();if(l===1&&!a){a=!0,n++;continue}l===2?s.push(`<h3 class="dk__teoria-h2">${g(r)}</h3>`):l===3?s.push(`<h4 class="dk__teoria-h3">${g(r)}</h4>`):s.push(`<h${Math.min(l+1,6)} class="dk__teoria-h">${g(r)}</h${Math.min(l+1,6)}>`),n++;continue}if(/^\s*>\s?/.test(i)){let l=[];for(;n<t.length&&/^\s*>\s?/.test(t[n]);)l.push(t[n]),n++;s.push(Pe(l));continue}if(/^\s*\|/.test(i)&&n+1<t.length&&z(t[n+1])){let l=i,r=[];for(n+=2;n<t.length&&/^\s*\|/.test(t[n]);)r.push(t[n]),n++;s.push(Be(l,r));continue}if(/^\s*[-*]\s+/.test(i)){let l=[];for(;n<t.length&&/^\s*[-*]\s+/.test(t[n]);)l.push(t[n]),n++;s.push(Re(l));continue}let c=[i];for(n++;n<t.length&&!/^\s*$/.test(t[n])&&!/^(#{1,6})\s+/.test(t[n])&&!/^\s*>\s?/.test(t[n])&&!/^\s*[-*]\s+/.test(t[n])&&!(/^\s*\|/.test(t[n])&&n+1<t.length&&z(t[n+1]));)c.push(t[n]),n++;s.push(`<p class="dk__teoria-p">${g(c.join(" "))}</p>`)}return s.join(`
`)}var ie="puheo:dk:sidemenu",T="open",y="collapsed",Ne="puheo:dk:progress";function re(){return`${Ne}:${u.lang}:${u.kurssiKey}:${u.lessonIndex}`}function M(){try{return JSON.parse(localStorage.getItem(re())||"{}")}catch{return{}}}function oe(e){try{localStorage.setItem(re(),JSON.stringify(e))}catch{}}function x(e){if(!e)return;let t=M();t[e]!=="done"&&(t[e]="done",oe(t),K()&&A(`${S}/api/digikirja/progress`,{method:"POST",headers:{"Content-Type":"application/json",...E()},body:JSON.stringify({lang:u.lang,kurssi:u.kurssiKey,lesson:u.lessonIndex,sivuId:e})}).catch(()=>{}),V())}function V(){let e=M(),t=document.getElementById("dk-sidemenu-list");t&&t.querySelectorAll(".dk__row").forEach(n=>{n.classList.toggle("is-done",e[n.dataset.sivu]==="done")});let s=document.getElementById("dk-progress-chip");if(s){let n=k.filter(a=>e[a.id]==="done").length;s.textContent=`${n} / ${k.length} valmis`,s.dataset.full=n>=k.length?"true":"false"}}var He={teoria:"Opetus",tehtava:"Harjoitukset",flashcards:"Kortit",testi:"Testit",itsearviointi:"Itsearvio"},Q={teoria:`<svg class="dk__row-glyph" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
  ...
  </svg>`};function qe(e){return Q[e]||Q.tehtava}var ce=5,De="puheo:dk:flashcards",ee={},de=!1,u={lang:"es",kurssiKey:"kurssi_2",lessonIndex:3,sivuId:"teoria"},f=null,k=[],B="";function o(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function Fe(){try{return localStorage.getItem(ie)===y?y:T}catch{return T}}function Ve(e){try{localStorage.setItem(ie,e)}catch{}}function Ue(e){return e==="fr"?"Ranska":e==="de"?"Saksa":"Espanja"}function Je(e){return`/data/courses/${encodeURIComponent(e.lang)}/${encodeURIComponent(e.kurssiKey)}/lesson_${encodeURIComponent(e.lessonIndex)}.json`}async function Ye(e){let t=Je(e),s=await fetch(t,{headers:{accept:"application/json"}});if(!s.ok)throw new Error(`lesson fetch ${s.status}`);return s.json()}async function Ge(e){if(K())try{let t=`lang=${encodeURIComponent(e.lang)}&kurssi=${encodeURIComponent(e.kurssiKey)}&lesson=${encodeURIComponent(e.lessonIndex)}`,[s,n]=await Promise.all([A(`${S}/api/digikirja/progress?${t}`,{headers:E()}).catch(()=>null),A(`${S}/api/digikirja/itsearvio?${t}`,{headers:E()}).catch(()=>null)]);if(s?.ok){let{sivut:a}=await s.json().catch(()=>({sivut:{}}));if(a&&typeof a=="object"){let i=M();for(let d of Object.keys(a))i[d]||(i[d]="done");oe(i)}}if(n?.ok){let{itsearvio:a}=await n.json().catch(()=>({itsearvio:null}));a?.ratings&&(J()||me({ratings:a.ratings,submittedAt:a.submittedAt,lang:e.lang,kurssiKey:e.kurssiKey,lessonIndex:e.lessonIndex}))}}catch{}}function We(e){let t=[];t.push({id:"teoria",kind:"teoria",num:"",title:e?.meta?.title||"Opetus",meta:"Opetussivu"});let s=Array.isArray(e?.phases)?e.phases:[];s.forEach((r,p)=>{let _=String(p+1),h=r.title||`Vaihe ${_}`,m=Array.isArray(r.items)?r.items.length:0;t.push({id:`phase-${p}`,kind:"tehtava",num:_,title:h,meta:m?`${m} kohtaa`:"Teht\xE4v\xE4"})});let n=Array.isArray(e?.vocab)?e.vocab:[],a=Math.min(n.length,ce);a>0&&t.push({id:"kortit-1",kind:"flashcards",num:"",title:`K\xE4\xE4nt\xF6kortit \xB7 ${e?.meta?.title||""}`.trim(),meta:`${a} korttia`});let i=r=>s.findIndex(p=>Array.isArray(p.items)&&p.items[0]?.item_type===r),d=i("translate"),c=i("mc"),l=6;if(d>=0){let r=Math.min(l,(s[d].items||[]).length);t.push({id:"test-1",kind:"testi",num:"T1",title:"Test 1 \xB7 K\xE4\xE4nn\xE4",meta:`${r} kohtaa`,testDef:{sourcePhase:d,count:r,label:"K\xE4\xE4nn\xE4 espanjaksi"}})}if(c>=0){let r=Math.min(l,(s[c].items||[]).length);t.push({id:"test-2",kind:"testi",num:"T2",title:"Test 2 \xB7 Valitse",meta:`${r} kohtaa`,testDef:{sourcePhase:c,count:r,label:"Valitse oikea vaihtoehto"}})}return t.push({id:"arvio",kind:"itsearviointi",num:"",title:"Arvioi omia taitojasi",meta:"Itsearvio"}),t}function b(e){let t=k.findIndex(s=>s.id===e);return t>=0?t:0}function ze(){let e=f?.meta||{},t=e.course_key||u.kurssiKey||"",s=e.title||"Oppitunti";return`
    <header class="dk__topbar" role="banner">
      ...
      <nav class="dk__breadcrumb" aria-label="Navigointi">
        ...
        <a href="#/oppimispolku?lang=${o(u.lang)}">${o(Ue(u.lang))}</a>
        <span class="dk__breadcrumb-sep">/</span>
    ...
    </header>`}function Xe(){let e=[],t=null;for(let a of k){let i=He[a.kind]||"Muut";i!==t&&(e.push({title:i,items:[]}),t=i),e[e.length-1].items.push(a)}let s=M(),n=e.map(a=>{let i=`<span class="dk__group-title">${o(a.title)}</span>`,d=a.items.map(c=>{let l=c.id===u.sivuId,r=s[c.id]==="done",p=c.num||"\xB7",_=["dk__row"];return l&&_.push("is-active"),r&&_.push("is-done"),`
        <button type="button"
                class="${_.join(" ")}"
                data-sivu="${o(c.id)}"
                data-kind="${o(c.kind)}"
                aria-current="${l?"page":"false"}"
                aria-label="${o(c.title)}${r?", suoritettu":""}">
          <span class="dk__row-glyph-wrap" aria-hidden="true">${qe(c.kind)}</span>
          <span class="dk__row-num">${o(p)}</span>
          <span class="dk__row-title">${o(c.title)}</span>
          <span class="dk__row-meta">${o(c.meta||"")}</span>
    ...
    <div class="dk__placeholder" data-kind="${o(e.kind)}">
      ...
      </div>`;let s=_e(e.id,t),n=e.testDef?.label||e.title||"Testi",a=t.map((c,l)=>Qe(c,l,s)).join(""),i=s.submitted?et(t,s):"",d=s.submitted?`<button type="button" class="dk__btn dk__btn--ghost" id="dk-testi-reset">Tee uudelleen</button>
    ...
    <section class="dk__testi" data-sivu="${o(e.id)}">
      ...
      <div class="dk__exercise-actions dk__testi-actions">${d}</div>
        ...
        <div class="dk__input-row">
         ...
         ${a?.correct?"":`<p class="dk__feedback-expected"><span>Oikea vastaus:</span> ${o(Te(e)||"")}</p>`}
    ...
    <div class="dk__testi-summary" aria-live="polite">
      ...
      <div class="dk__arvio-row" data-statement="${o(l.id)}">
       ...
       <button type="button" class="dk__btn dk__btn--primary" id="dk-arvio-back">Takaisin oppimispolulle</button>`:`<button type="button" class="dk__btn dk__btn--primary" id="dk-arvio-submit" ${i?"":"disabled"}>Tallenna arvio</button>`,c=n?at(s):"";return`
    ...
    <div class="dk__testi-summary" aria-live="polite">
      ...
      </section>`;let i=be(e.id),d=Math.min(i.cardIndex,t.length-1),l=xe(t,s,d)[0]??d,r=t[l],p=I(r,l),_=s[p]||null;return`
    ...
    </section>`}function xe(e,t,s){let n=[];for(let a=s;a<e.length;a++){let i=I(e[a],a);t[i]!==w&&n.push(a)}for(let a=0;a<s;a++){let i=I(e[a],a);t[i]!==w&&n.push(a)}return n}function ot(e,t,s,n){let a=e.gender?`<span class="dk__flashcard-tag">${o(e.gender==="m"?"Maskuliini":e.gender==="f"?"Feminiini":e.gender)}</span>`:"",i=n===ge?'<span class="dk__flashcard-tag dk__flashcard-tag--again">Harjoittelussa</span>':n===w?'<span class="dk__flashcard-tag dk__flashcard-tag--know">Hallinnassa</span>':"";return`
    ...
    </div>`}function P(){let e=b(u.sivuId),t=k[e],s=document.querySelector(".dk__content .dk__flashpack");if(!s)return;let n=document.createElement("div");n.innerHTML=$e(t),s.replaceWith(n.firstElementChild),we()}function we(){let e=document.querySelector(".dk__flashpack");if(!e)return;let t=e.dataset.sivu;if(document.getElementById("dk-flash-reset")?.addEventListener("click",()=>{rt(),q.delete(t),P()}),document.getElementById("dk-flash-next-sivu")?.addEventListener("click",()=>{let l=b(u.sivuId),r=k[l+1];r&&C(r.id)}),e.dataset.done==="true")return;let s=be(t),n=Number(e.dataset.index),a=D(f)[n];if(!a)return;let i=I(a,n),d=()=>{s.flipped=!s.flipped,P()};document.getElementById("dk-flashcard")?.addEventListener("click",d),document.getElementById("dk-flashcard")?.addEventListener("keydown",l=>{(l.key===" "||l.key==="Enter")&&(l.preventDefault(),d())}),document.getElementById("dk-flash-flip")?.addEventListener("click",d);let c=l=>{it(i,l),s.flipped=!1;let r=G(),p=D(f);p.length>0&&p.every((m,v)=>r[I(m,v)]===w)&&x(t);let h=xe(p,r,n+1<p.length?n+1:0);s.cardIndex=h[0]??n,P()};document.getElementById("dk-flash-again")?.addEventListener("click",()=>c(ge)),document.getElementById("dk-flash-know")?.addEventListener("click",()=>c(w))}var ne=new Map,Ie=new Set(["mc","typed","gap_fill","translate"]);function Se(e){if(!e||e.kind!=="tehtava")return null;let t=/^phase-(\d+)$/.exec(e.id);if(!t)return null;let s=Number(t[1]);return(Array.isArray(f?.phases)?f.phases:[])[s]||null}function Ee(e,t){let s=ne.get(e);return s||(s={itemIndex:0,answered:new Array(t.length).fill(null),scoreCorrect:0,scoreTotal:0},ne.set(e,s)),s}function Ae(e){let t=Se(e);if(!t)return ue(e);let s=Array.isArray(t.items)?t.items:[];if(s.length===0)return'<div class="dk__placeholder"><p>T\xE4ll\xE4 vaiheella ei ole teht\xE4vi\xE4.</p></div>';let n=s[0].item_type;if(!Ie.has(n)){let l=n==="match"?"Yhdist\xE4misteht\xE4v\xE4, tulossa PR 4b":n==="writing"?"Kirjoitusteht\xE4v\xE4, tulossa PR 7":`Teht\xE4v\xE4tyyppi "${n}" tulossa my\xF6hemmin`;return`
      <div class="dk__placeholder" data-kind="tehtava">
        <p class="dk__placeholder-kind">${o(l)}</p>
        <p>Vaihe ${o(String((ct(e)??0)+1))}: ${o(t.title||"")}. Vaiheessa ${s.length} teht\xE4v\xE4\xE4 t\xE4t\xE4 tyyppi\xE4.</p>
      </div>`}let a=Ee(e.id,s),i=Math.min(a.itemIndex,s.length-1),d=s[i],c=a.answered[i];return`
    ...
    <div class="dk__input-row">
      ...
      <input id="dk-input" type="text" class="dk__input"
             ...
             value="${o(n)}"
             ${a?"disabled":""}>
    </div>`}function pt(e,t){let s=String(e.sentence_template||""),n=(s.match(/\{(\d+)\}/g)||[]).length,a=t?.userAnswer||new Array(n).fill(""),i=!!t,d=0,c=o(s).replace(/\{(\d+)\}/g,()=>{let r=a[d]||"",p=`dk-gap-${d}`;return d++,`<input id="${p}" type="text" class="dk__input dk__input--gap"
    ...
    <div class="dk__input-row">
      ...
      </div>`;let a=t.correct,i=a?"is-correct":"is-wrong",d=a?"Oikein":"Viel\xE4 ei aivan",c=ht(e,t),l=s>=n-1;return`
    ...
    <main class="dk__content" id="dk-content" tabindex="-1">
      ...
      ${c}
      ${ae(n,a,"bottom")}
    </main>`}function ae(e,t,s){let n=`dk__prevnext dk__prevnext--${s}`,a=e?`<button type="button" class="dk__prevnext-btn dk__prevnext-btn--prev" data-sivu="${o(e.id)}">
    ...
    </div>`}function L(e){let t=document.getElementById("dk-root");if(!t)return;t.dataset.sidemenu=e;let s=document.getElementById("dk-toggle-sidemenu");s&&s.setAttribute("aria-pressed",e===y?"true":"false")}function yt(){let e=document.getElementById("dk-toggle-sidemenu"),t=document.getElementById("dk-sidemenu-backdrop");e?.addEventListener("click",()=>{let s=document.getElementById("dk-root");if(window.matchMedia("(max-width: 1023px)").matches){let a=s.dataset.sidemenu===T?y:T;L(a)}else{let a=s.dataset.sidemenu===y?T:y;L(a),Ve(a)}}),t?.addEventListener("click",()=>{L(y)})}function C(e){if(!e||e===u.sivuId||k.findIndex(d=>d.id===e)<0)return;u.sivuId=e;let s=`#/oppitunti/${u.lang}/${encodeURIComponent(u.kurssiKey)}/${u.lessonIndex}/${encodeURIComponent(e)}`;location.hash!==s&&history.replaceState(null,"",s);let n=document.getElementById("dk-root");if(!n)return;let a=n.querySelector(".dk__sidemenu-list");a&&a.querySelectorAll(".dk__row").forEach(d=>{let c=d.dataset.sivu===e;d.classList.toggle("is-active",c),d.setAttribute("aria-current",c?"page":"false")});let i=n.querySelector(".dk__content");if(i){let d=document.createElement("div");d.innerHTML=Le(),i.replaceWith(d.firstElementChild),Ke()}window.matchMedia("(max-width: 1023px)").matches&&L(y),Ce(),le(),V(),document.getElementById("dk-content")?.focus({preventScroll:!1})}function gt(){document.getElementById("dk-sidemenu-list")?.addEventListener("click",e=>{let t=e.target.closest(".dk__row");t&&C(t.dataset.sivu)})}function Ce(){let e=document.getElementById("dk-sidemenu-list");if(!e)return;let t=e.querySelector(".dk__row.is-active");t&&requestAnimationFrame(()=>{try{t.scrollIntoView({block:"nearest",inline:"nearest",behavior:"auto"})}catch{let s=t.offsetTop-e.clientHeight/2+t.clientHeight/2;e.scrollTop=Math.max(0,s)}})}function Ke(){document.querySelectorAll(".dk__content .dk__prevnext-btn[data-sivu]").forEach(e=>{e.addEventListener("click",()=>C(e.dataset.sivu))}),je(),we(),he(),ye()}function bt(){de=!0}async function $t(e={}){de||bt(),u.lang=e.lang||u.lang,u.kurssiKey=e.kurssiKey||u.kurssiKey,u.lessonIndex=Number(e.lessonIndex)||u.lessonIndex,u.sivuId=e.sivuId||u.sivuId||"teoria";let t=document.getElementById("screen-digikirja");if(!t)return;t.innerHTML=ft(),W("screen-digikirja");let s=`${u.lang}/${u.kurssiKey}/${u.lessonIndex}`;B=s;try{let[n]=await Promise.all([Ye(u),Ge(u)]);if(B!==s)return;f=n,k=We(n),k.some(d=>d.id===u.sivuId)||(u.sivuId=k[0]?.id||"teoria");let i=window.matchMedia("(max-width: 1023px)").matches?y:Fe();t.innerHTML=`
