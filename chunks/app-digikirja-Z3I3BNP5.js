...
`)}var ae="puheo:dk:sidemenu",x="open",y="collapsed",Ne="puheo:dk:progress";function ie(){return`${Ne}:${u.lang}:${u.kurssiKey}:${u.lessonIndex}`}function B(){try{return JSON.parse(localStorage.getItem(ie())||"{}")}catch{return{}}}function re(e){try{localStorage.setItem(ie(),JSON.stringify(e))}catch{}}function w(e){if(!e)return;let t=B();t[e]!=="done"&&(t[e]="done",re(t),M()&&j(`${E}/api/digikirja/progress`,{method:"POST",headers:{"Content-Type":"application/json",...A()},body:JSON.stringify({lang:u.lang,kurssi:u.kurssiKey,lesson:u.lessonIndex,sivuId:e})}).catch(()=>{}),U())}function U(){let e=B(),t=document.getElementById("dk-sidemenu-list");t&&t.querySelectorAll(".dk__row").forEach(n=>{n.classList.toggle("is-done",e[n.dataset.sivu]==="done")});let s=document.getElementById("dk-progress-chip");if(s){let n=k.filter(a=>e[a.id]==="done").length;s.textContent=`${n} / ${k.length} valmis`,s.dataset.full=n>=k.length?"true":"false"}}var Pe={teoria:"Opetus",tehtava:"Harjoitukset",flashcards:"Kortit",testi:"Testit",itsearviointi:"Itsearvio"};var oe=5,Re="puheo:dk:flashcards",ee={},de=!1,u={lang:"es",kurssiKey:"kurssi_2",lessonIndex:3,sivuId:"teoria"},v=null,k=[],H="";function r(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function qe(){try{return localStorage.getItem(ae)===x?x:y}catch{return x}}function Ve(e){try{localStorage.setItem(ae,e)}catch{}}function Fe(e){return e==="fr"?"Ranska":e==="de"?"Saksa":"Espanja"}function De(e){return`/data/courses/${encodeURIComponent(e.lang)}/${encodeURIComponent(e.kurssiKey)}/lesson_${encodeURIComponent(e.lessonIndex)}.json`}async function Ue(e){let t=De(e),s=await fetch(t,{headers:{accept:"application/json"}});if(!s.ok)throw new Error(`lesson fetch ${s.status}`);return s.json()}async function Je(e){if(M())try{let t=`lang=${encodeURIComponent(e.lang)}&kurssi=${encodeURIComponent(e.kurssiKey)}&lesson=${encodeURIComponent(e.lessonIndex)}`,[s,n]=await Promise.all([j(`${E}/api/digikirja/progress?${t}`,{headers:A()}).catch(()=>null),j(`${E}/api/digikirja/itsearvio?${t}`,{headers:A()}).catch(()=>null)]);if(s?.ok){let{sivut:a}=await s.json().catch(()=>({sivut:{}}));if(a&&typeof a=="object"){let i=B();for(let d of Object.keys(a))i[d]||(i[d]="done");re(i)}}if(n?.ok){let{itsearvio:a}=await n.json().catch(()=>({itsearvio:null}));a?.ratings&&(Y()||he({ratings:a.ratings,submittedAt:a.submittedAt,lang:e.lang,kurssiKey:e.kurssiKey,lessonIndex:e.lessonIndex}))}}catch{}}function Ye(e){let t=[];t.push({id:"teoria",kind:"teoria",num:"",title:e?.meta?.title||"Opetus",meta:"Opetussivu"}),(Array.isArray(e?.phases)?e.phases:[]).forEach((i,d)=>{let c=String(d+1),o=i.title||`Vaihe ${c}`,l=Array.isArray(i.items)?i.items.length:0;t.push({id:`phase-${d}`,kind:"tehtava",num:c,title:o,meta:l?`${l} kohtaa`:"Teht\xE4v\xE4"})});let n=Array.isArray(e?.vocab)?e.vocab:[],a=Math.min(n.length,oe);return a>0&&t.push({id:"kortit-1",kind:"flashcards",num:"",title:`K\xE4\xE4nt\xF6kortit \xB7 ${e?.meta?.title||""}`.trim(),meta:`${a} korttia`}),t}function b(e){let t=k.findIndex(s=>s.id===e);return t>=0?t:0}function Ge(){let t=(v?.meta||{}).title||"Oppitunti";return`
    ...
    <div class="dk__testi-summary" aria-live="polite">
    ...
    </div>`}function se(){let e=b(u.sivuId),t=k[e],s=document.querySelector(".dk__content .dk__arvio");if(!s)return;let n=document.createElement("div");n.innerHTML=fe(t),s.replaceWith(n.firstElementChild),ve()}function ve(){let e=document.querySelector(".dk__arvio");if(!e)return;let t=e.dataset.sivu,s=me(t);e.querySelectorAll(".dk__arvio-btn").forEach(n=>{n.addEventListener("click",()=>{let a=n.dataset.statement,i=Number(n.dataset.value);s[a]=i,e.querySelector(`.dk__arvio-row[data-statement="${a}"]`)?.querySelectorAll(".dk__arvio-btn").forEach(o=>{let l=Number(o.dataset.value)===i;o.classList.toggle("is-chosen",l),o.setAttribute("aria-pressed",l?"true":"false")});let c=document.getElementById("dk-arvio-submit");if(c){let o=C.every(l=>Number.isInteger(s[l.id])&&s[l.id]>0);c.disabled=!o}})}),document.getElementById("dk-arvio-submit")?.addEventListener("click",()=>{let n={ratings:{...s},submittedAt:new Date().toISOString(),lang:u.lang,kurssiKey:u.kurssiKey,lessonIndex:u.lessonIndex};he(n),w(t),se(),M()&&j(`${E}/api/digikirja/itsearvio`,{method:"POST",headers:{"Content-Type":"application/json",...A()},body:JSON.stringify({lang:u.lang,kurssi:u.kurssiKey,lesson:u.lessonIndex,ratings:n.ratings})}).catch(()=>{})}),document.getElementById("dk-arvio-reset")?.addEventListener("click",()=>{try{localStorage.removeItem(J())}catch{}q.delete(t),se()}),document.getElementById("dk-arvio-back")?.addEventListener("click",()=>{location.hash=`#/oppimispolku/${u.lang}/${encodeURIComponent(u.kurssiKey)}`})}var V=new Map,I="know",ge="again";function F(e){return(Array.isArray(e?.vocab)?e.vocab:[]).slice(0,oe)}function G(){return`${Re}:${u.lang}:${u.kurssiKey}:${u.lessonIndex}`}function W(){try{let e=localStorage.getItem(G());return e?JSON.parse(e):{}}catch{return{}}}function at(e,t){try{let s=W();s[e]=t,localStorage.setItem(G(),JSON.stringify(s))}catch{}}function it(){try{localStorage.removeItem(G())}catch{}}function ye(e){let t=V.get(e);return t||(t={cardIndex:0,flipped:!1},V.set(e,t)),t}function S(e,t){return e?.es?`${t}:${e.es}`:`${t}`}function be(e){let t=F(v);if(t.length===0)return'<div class="dk__placeholder"><p>T\xE4m\xE4n oppitunnin sanasto on tyhj\xE4.</p></div>';let s=W(),n=t.filter((h,m)=>s[S(h,m)]===I).length;if(n===t.length)return`
      ...
      </section>`;let i=ye(e.id),d=Math.min(i.cardIndex,t.length-1),o=$e(t,s,d)[0]??d,l=t[o],p=S(l,o),_=s[p]||null;return`
    ...
    </section>`}function $e(e,t,s){let n=[];for(let a=s;a<e.length;a++){let i=S(e[a],a);t[i]!==I&&n.push(a)}for(let a=0;a<s;a++){let i=S(e[a],a);t[i]!==I&&n.push(a)}return n}function rt(e,t,s,n){let a=e.gender?`<span class="dk__flashcard-tag">${r(e.gender==="m"?"Maskuliini":e.gender==="f"?"Feminiini":e.gender)}</span>`:"",i=n===ge?'<span class="dk__flashcard-tag dk__flashcard-tag--again">Harjoittelussa</span>':n===I?'<span class="dk__flashcard-tag dk__flashcard-tag--know">Hallinnassa</span>':"";return`
...
//# sourceMappingURL=app-digikirja-Z3I3BNP5.js.map
